...
$var_pag[23] = 'N_PHPR_DB_TYPE';
$var_pag[24] = 'N_PHPR_DB_NAME';
$var_pag[25] = 'database_esistente';
$var_pag[26] = 'N_PHPR_DB_HOST';
$var_pag[27] = 'N_PHPR_DB_PORT';
$var_pag[28] = 'N_PHPR_DB_USER';
$var_pag[29] = 'N_PHPR_DB_PASS';
$var_pag[30] = 'N_PHPR_LOAD_EXT';
$var_pag[31] = 'tempdatabase';
$var_pag[32] = 'N_PHPR_TAB_PRE';
$n_var_pag = 33;
...
if (defined("C_UTENTE_BACKUP_ESTERNO") and C_UTENTE_BACKUP_ESTERNO != "" and isset($utente_backup) and $utente_backup == C_UTENTE_BACKUP_ESTERNO and (!defined('C_VERSIONE_ATTUALE') or !defined('C_PHPR_VERSIONE_NUM') or (float) C_VERSIONE_ATTUALE == (float) C_PHPR_VERSIONE_NUM)) $id_utente = "b";
...
$ripristina_backup = "";
$lingua_mex = "ita";
if (!empty($crea_backup) and defined("C_MAX_ANNI_BACKUP_ESTERNO") and C_MAX_ANNI_BACKUP_ESTERNO != "") {
$anni_backup = "sel";
for ($num1 = 0 ; $num1 < C_MAX_ANNI_BACKUP_ESTERNO ; $num1++) ${'anno_sel'.($anno - $num1)} = 1;
} # fine (!empty($crea_backup) and defined("C_MAX_ANNI_BACKUP_ESTERNO") and...
} # fine if ($id_utente == "b")
...
if (isset($compresso) and $compresso == "SI" and @is_file($file_compresso)) unlink($file_compresso);
...
$file_backup = C_DATI_PATH."/hoteld_doc_backup.php";
if (strstr($lingua,"-")) {
$locale_lingua = substr(strstr($lingua,"-"),1);
$lingua = str_replace("-$locale_lingua","",$lingua);
if (!is_dir("./includes/lang/$lingua/locales/$locale_lingua") or strlen($locale_lingua) > 3 or str_replace("/","",$locale_lingua) != $lingua) $locale_lingua = "";
...
if ($locale_lingua) $file_copia = "./includes/lang/$lingua/locales/$locale_lingua/hoteld_doc_backup.php";
...
if (!empty($versione_file) and $versione_file == $versione_corrente) {
...
<input type=\"hidden\" name=\"ripristina_backup\" value=\"1\">
...
if (isset($dati_conn) and $dati_conn == "attuali") {
...
} # fine if (isset($dati_conn) and $dati_conn == "attuali")

if (isset($continua) and $continua == "SI" and ((!defined('C_RESTRIZIONI_DEMO_ADMIN') or C_RESTRIZIONI_DEMO_ADMIN != "SI") or (defined('C_PASS_DEMO_ADMIN') and C_PASS_DEMO_ADMIN == fixset($pass_demo_admin)))) ripristina_backup($file,"NO",$pag,$numconnessione,fixset($database_esistente),fixset($tempdatabase),$PHPR_DB_TYPE,$PHPR_DB_NAME,$PHPR_DB_HOST,$PHPR_DB_PORT,$PHPR_DB_USER,$PHPR_DB_PASS,$PHPR_LOAD_EXT,$PHPR_TAB_PRE,$N_PHPR_DB_TYPE,$N_PHPR_DB_NAME,$N_PHPR_DB_HOST,$N_PHPR_DB_PORT,$N_PHPR_DB_USER,$N_PHPR_DB_PASS,$N_PHPR_LOAD_EXT,$N_PHPR_TAB_PRE,$ext_pgsql_caricata,$ext_mysql_caricata,fixset($mantieni_anni));
...
if ($ini_lingua == $lingua_mex and !$locale_mex) $selected = " selected";
else $selected = "";
...
if (is_file($file_doc_backup)) echo "<option value=\"$ini_lingua\"$selected>$nome_lingua</option>";
...
if (preg_replace("/[a-z]*/i","",$ini_locale) == "" and strlen($ini_locale) < 4 and is_file("./includes/lang/$ini_lingua/locales/$ini_locale/hoteld_doc_backup.php")) {
...
if ("$ini_lingua-$ini_locale" == "$lingua_mex-$locale_mex") $selected = " selected";
else $selected = "";
...
} # fine if (preg_replace("/[a-z]*/i","",$ini_lingua) == "" and strlen($ini_lingua) < 4 and...
