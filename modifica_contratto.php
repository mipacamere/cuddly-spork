...
$var_pag[75] = 'upload_iimg';
$var_pag[76] = 'cid';
$var_pag[77] = 'file_iimg_upload';
$var_pag[78] = 'num_elimina_iimg';
$var_pag[79] = 'elimina_iimg';
$var_pag[80] = 'manda_plain_text';
$var_pag[81] = 'aggiungi_iimg';
$var_pag[82] = 'num_iimg';
$var_pag[83] = 'agg_pag_escludi';
$var_pag[84] = 'pag_escludi';
$var_pag[85] = 'pag_esc_elim';
$n_var_pag = 86;
...
if ($anno_utente_attivato == "SI" and $priv_mod_doc == "s" and $modifica_pers != "NO" and ($attiva_contratti_consentiti == "n" or $contratti_consentiti_vett[(int) $num_contratto] == "SI")) {
...
$contrhtm = esegui_query("select tipo from $tablecontratti where numero = '".aggslashdb($num_contratto)."' and (tipo = 'contrhtm' or (tipo = 'opzeml' and testo $LIKE 'html;%')) ");
...
$num_contratto_int = (int) $num_contratto;
...
$iimg_esist = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
if (numlin_query($iimg_esist)) {
esegui_query("delete from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
$iimg_esist = (string) risul_query($iimg_esist,0,'testo');
$iimg_esist = explode(",",$iimg_esist);
$n_iimg_esist = (count($iimg_esist) - 1);
for ($num1 = 1 ; $num1 < $n_iimg_esist ; $num1++) {
$num_iimg = str_replace(strstr($iimg_esist[$num1],":"),"",$iimg_esist[$num1]);
$altre_iimg = esegui_query("select * from $tablecontratti where testo $LIKE '%,".aggslashdb($num_iimg).":%' and tipo = 'img_inln' ");
if (!numlin_query($altre_iimg)) esegui_query("delete from $tablecontratti where numero = '".aggslashdb($num_iimg)."' and tipo = 'file_iim' ");
} # fine for $num1
} # fine if (numlin_query($iimg_esist))
...
$nuova_dir_salva = htmlspecialchars($nuova_dir_salva,ENT_COMPAT);
...
$mostra_form_iniziale = "NO";
$nome_file_salva = htmlspecialchars($nome_file_salva,ENT_COMPAT);
...
if (!empty($agg_pag_escludi) and !empty($pag_escludi)) {
if ($pag_escludi == "tab_mese" or $pag_escludi == "tab_prenota" or $pag_escludi == "tab_clienti" or $pag_escludi == "messaggi" or $pag_escludi == "contr_disp" or $pag_escludi == "mod_prenota" or $pag_escludi == "mod_cliente" or $pag_escludi == "pos" or $pag_escludi == "casse") {
$pag_escluse = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'pag_esc'");
if (numlin_query($pag_escluse)) $str_pag_esc = (string) risul_query($pag_escluse,0,'testo');
else $str_pag_esc = "";
if (!strstr($str_pag_esc,",$pag_escludi,")) {
$mostra_form_iniziale = "NO";
if (numlin_query($pag_escluse)) esegui_query("update $tablecontratti set testo = '".aggslashdb("$str_pag_esc$pag_escludi,")."' where numero = '$num_contratto' and tipo = 'pag_esc' ");
else esegui_query("insert into $tablecontratti (numero,tipo,testo) values ('$num_contratto','pag_esc',',$pag_escludi,') ");
echo mex("È stata aggiunta la pagina dove non mostrare il documento",$pag).".<br>";
} # fine if (!strstr($str_pag_esc,",$pag_escludi,"))
} # fine if ($pag_escludi == "tab_mese" or $pag_escludi == "tab_prenota" or...
} # fine if (!empty($agg_pag_escludi) and !empty($pag_escludi))

if (!empty($pag_esc_elim)) {
if ($pag_esc_elim == "tab_mese" or $pag_esc_elim == "tab_prenota" or $pag_escludi == "tab_clienti" or $pag_esc_elim == "messaggi" or $pag_esc_elim == "contr_disp" or $pag_esc_elim == "mod_prenota" or $pag_esc_elim == "mod_cliente" or $pag_esc_elim == "pos" or $pag_esc_elim == "casse") {
$pag_escluse = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'pag_esc'");
if (numlin_query($pag_escluse)) $str_pag_esc = (string) risul_query($pag_escluse,0,'testo');
else $str_pag_esc = "";
if (strstr($str_pag_esc,",$pag_esc_elim,")) {
$mostra_form_iniziale = "NO";
$str_pag_esc = str_replace(",$pag_esc_elim,",",",$str_pag_esc);
if ($str_pag_esc != ",") esegui_query("update $tablecontratti set testo = '".aggslashdb("$str_pag_esc")."' where numero = '$num_contratto' and tipo = 'pag_esc' ");
else esegui_query("delete from $tablecontratti where numero = '$num_contratto' and tipo = 'pag_esc' ");
echo mex("È stata eliminata la pagina dove non mostrare il documento",$pag).".<br>";
} # fine if (strstr($str_pag_esc,",$pag_esc_elim,"))
} # fine if ($pag_esc_elim == "tab_mese" or $pag_esc_elim == "tab_prenota" or...
} # fine if (!empty($pag_esc_elim))
...
$nuovo_oggetto = htmlspecialchars($nuovo_oggetto,ENT_COMPAT);
$tabelle_lock = array($tablecontratti);
$tabelle_lock = lock_tabelle($tabelle_lock);
...
if (!empty($upload_iimg)) {
$errore = "NO";
$file_tmp = C_DATI_PATH."/iimg_contr$num_contratto.tmp";
if (empty($file_iimg_upload)) {
if (!empty($HTTP_POST_FILES['file_iimg_upload']['tmp_name'])) {
$file_iimg_upload = $HTTP_POST_FILES['file_iimg_upload']['tmp_name'];
$file_iimg_upload_name = $HTTP_POST_FILES['file_iimg_upload']['name'];
$file_iimg_upload_type = $HTTP_POST_FILES['file_iimg_upload']['type'];
} # fine if (!empty($HTTP_POST_FILES['file_iimg_upload']['tmp_name']))
else {
if (!empty($_FILES['file_iimg_upload']['tmp_name'])) {
$file_iimg_upload = $_FILES['file_iimg_upload']['tmp_name'];
$file_iimg_upload_name = $_FILES['file_iimg_upload']['name'];
$file_iimg_upload_type = $_FILES['file_iimg_upload']['type'];
} # fine if (!empty($_FILES['file_iimg_upload']['tmp_name']))
} # fine else if (!empty($HTTP_POST_FILES['file_iimg_upload']['tmp_name']))
} # fine if (empty($file_iimg_upload))
if (!move_uploaded_file(fixstr($file_iimg_upload),$file_tmp)) $errore = "SI";
if ($errore == "NO") {
if (!defined("C_MASSIMO_NUM_BYTE_UPLOAD") or C_MASSIMO_NUM_BYTE_UPLOAD == 0 or filesize($file_tmp) <= C_MASSIMO_NUM_BYTE_UPLOAD) {
$iimg = "";
$f_iimg = fopen($file_tmp, 'r');
while (true) {
$dati = fread($f_iimg, 8192);
...
if ($tipo_contratto == "contreml") {
$errore = 0;
$iimg_esist = esegui_query("select testo from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln'");
$numlin_iimg_esist = numlin_query($iimg_esist);
if ($numlin_iimg_esist) $iimg_esist = (string) risul_query($iimg_esist,0,'testo');
else $iimg_esist = "";
if (substr_count($iimg_esist,",") > $num_allegati_max) $errore = 1;
$max_fii = esegui_query("select max(numero) from $tablecontratti where tipo = 'file_iim' ");
if (numlin_query($max_fii)) $max_fii = risul_query($max_fii,0,0) + 1;
else $max_fii = 1;
if (!$errore) {
$mostra_form_iniziale = "NO";
$iimg = chunk_split(base64_encode($iimg));
$fiimg_esist = esegui_query("select testo from $tablecontratti where testo $LIKE '%,".aggslashdb($iimg)."' and tipo = 'file_iim'");
if (numlin_query($fiimg_esist)) {
$errore = 1;
$fiimg_esist = explode(",",(string) risul_query($fiimg_esist,0,'testo'));
echo "".mex("È stata trovata una immagine uguale",$pag)." (<em class=\"colred\">".htmlspecialchars($fiimg_esist[0])."</em>), ".mex("riutilizzare l'immagine esistente per evitare di ingombrare il database",$pag).".<br>";;
} # fine if (numlin_query($fiimg_esist))
if (empty($cid) or !preg_match("/^[a-z][a-z0-9_]{2,30}$/i",$cid) or strstr($iimg_esist,":$cid,")) {
$errore = 1;
echo "<em class=\"colred\">".mex("Content-ID errato",$pag)."</em>.<br>";
} # fine if (empty($cid) or !preg_match("/^[a-z][a-z0-9_]{2,20}$/i",$cid) or...
if (empty($file_iimg_upload_name) and !empty($file_iimg_upload_type)) {
if (strstr($file_iimg_upload_type,"jpeg")) $file_iimg_upload_name = mex("Immagine",$pag).".jpeg";
if (strstr($file_iimg_upload_type,"gif")) $file_iimg_upload_name = mex("Immagine",$pag).".gif";
if (strstr($file_iimg_upload_type,"png")) $file_iimg_upload_name = mex("Immagine",$pag).".png";
if (strstr($file_iimg_upload_type,"webp")) $file_iimg_upload_name = mex("Immagine",$pag).".webp";
} # fine (empty($file_iimg_upload_name) and !empty($file_iimg_upload_type))
$file_iimg_upload_name = trim(fixstr($file_iimg_upload_name));
$lowfn = strtolower($file_iimg_upload_name);
if (substr($lowfn,-4) != ".jpg" and substr($lowfn,-5) != ".jpeg" and substr($lowfn,-4) != ".gif" and substr($lowfn,-4) != ".png" and substr($lowfn,-5) != ".webp") $errore = 1;
elseif (strstr($file_iimg_upload_name,"<") or strstr($file_iimg_upload_name,">") or strstr($file_iimg_upload_name,"\"") or strstr($file_iimg_upload_name,",") or preg_match("/^[a-z ]+:/i",$file_iimg_upload_name)) $errore = 1;
if (!$errore) {
if (substr($lowfn,-4) == ".jpg" or substr($lowfn,-5) == ".jpeg") $file_iimg_upload_type = "image/jpeg";
if (substr($lowfn,-4) == ".gif") $file_iimg_upload_type = "image/gif";
if (substr($lowfn,-4) == ".png") $file_iimg_upload_type = "image/png";
if (substr($lowfn,-5) == ".webp") $file_iimg_upload_type = "image/webp";
$iimg = htmlspecialchars($file_iimg_upload_name,ENT_COMPAT).",".$file_iimg_upload_type.",".$iimg;
esegui_query("insert into $tablecontratti (numero,tipo,testo) values ('$max_fii','file_iim','".aggslashdb($iimg)."')");
if ($iimg_esist) $str_iimg = "$iimg_esist$max_fii:$cid,";
else $str_iimg = ",$max_fii:$cid,";
if ($numlin_iimg_esist) esegui_query("update $tablecontratti set testo = '$str_iimg' where numero = '$num_contratto' and tipo = 'img_inln' ");
else esegui_query("insert into $tablecontratti (numero,tipo,testo) values ('$num_contratto','img_inln','$str_iimg')");
echo "".mex("Immagine salvata",$pag).".<br>";
} # fine if (!$errore)
} # fine if (!$errore)
} # fine if ($tipo_contratto == "contreml")
unlock_tabelle($tabelle_lock);
...
} # fine if (!empty($upload_iimg))

if (!empty($aggiungi_iimg)) {
$tabelle_lock = array($tablecontratti);
$tabelle_lock = lock_tabelle($tabelle_lock);
$dati_contratto = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo $LIKE 'contr%'");
$tipo_contratto = risul_query($dati_contratto,0,'tipo');
if ($tipo_contratto == "contreml") {
if (controlla_num_pos($num_iimg) == "NO") $num_iimg = 0;
$iimg_esistente = esegui_query("select * from $tablecontratti where numero = '".aggslashdb($num_iimg)."' and tipo = 'file_iim' ");
if (numlin_query($iimg_esistente)) {
$iimg_esist = esegui_query("select testo from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
$numlin_iimg_esist = numlin_query($iimg_esist);
if ($numlin_iimg_esist) $iimg_esist = (string) risul_query($iimg_esist,0,'testo');
else $iimg_esist = "";
if (!strstr($iimg_esist,",$num_iimg:")) {
$errore = 0;
if (substr_count($iimg_esist,",") > $num_allegati_max) $errore = 1;
if (empty($cid) or !preg_match("/^[a-z][a-z0-9_]{2,30}$/i",$cid) or strstr($iimg_esist,":$cid,")) {
$errore = 1;
...
if (!$errore) {
$mostra_form_iniziale = "NO";
if ($numlin_iimg_esist) {
if (!$iimg_esist) $iimg_esist = ",";
esegui_query("update $tablecontratti set testo = '$iimg_esist$num_iimg:$cid,' where numero = '$num_contratto' and tipo = 'img_inln' ");
} # fine if ($numlin_iimg_esist)
else esegui_query("insert into $tablecontratti (numero,tipo,testo) values ('$num_contratto','img_inln',',$num_iimg:$cid,') ");
echo "".mex("Immagine salvata",$pag).".<br>";
} # fine if (!$errore)
} # fine if (!strstr($iimg_esist,",$num_iimg:"))
} # fine if (numlin_query($iimg_esistente))
} # fine if ($tipo_contratto == "contreml")
unlock_tabelle($tabelle_lock);
} # fine if (!empty($aggiungi_iimg))

if (!empty($elimina_iimg)) {
$tabelle_lock = array($tablecontratti);
$tabelle_lock = lock_tabelle($tabelle_lock);
$iimg_esist = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
if (numlin_query($iimg_esist)) $iimg_esist = (string) risul_query($iimg_esist,0,'testo');
else $iimg_esist = "";
$num_iimg = fixstr($num_elimina_iimg);
if ($iimg_esist and strstr($iimg_esist,",$num_iimg:")) {
$mostra_form_iniziale = "NO";
$iimg_esist = preg_replace("/,$num_iimg:[a-z][a-z0-9_]*,/i",",",$iimg_esist);
if (strlen($iimg_esist) < 3) esegui_query("delete from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
else esegui_query("update $tablecontratti set testo = '$iimg_esist' where numero = '$num_contratto' and tipo = 'img_inln' ");
$altre_iimg = esegui_query("select * from $tablecontratti where testo $LIKE '%,".aggslashdb($num_iimg).":%' and tipo = 'img_inln' ");
if (!numlin_query($altre_iimg)) esegui_query("delete from $tablecontratti where numero = '".aggslashdb($num_iimg)."' and tipo = 'file_iim' ");
echo "".mex("Immagine eliminata",$pag).".<br>";
} # fine if ($iimg_esist and strstr($iimg_esist,",$num_iimg,"))
unlock_tabelle($tabelle_lock);
} # fine if (!empty($elimina_iimg))
...
if (!move_uploaded_file(fixset($file_allegato_upload),$file_tmp)) $errore = "SI";
if ($errore == "NO") {
if (!defined("C_MASSIMO_NUM_BYTE_UPLOAD") or C_MASSIMO_NUM_BYTE_UPLOAD == 0 or filesize($file_tmp) <= C_MASSIMO_NUM_BYTE_UPLOAD) {
$allegato = "";
...
$allegato = htmlspecialchars($file_allegato_upload_name,ENT_COMPAT).",".$file_allegato_upload_type.",".$allegato;
...
$tipo_contratto = risul_query($dati_contratto,0,'tipo');
if ($tipo_contratto == "contreml") {
if (controlla_num_pos($num_allegato) == "NO") $num_allegato = 0;
...
$mpt = 0;
if (fixstr($formato_email) == "html" and $priv_mod_doc_html == "s") {
$n_opz = "html;";
if ($opz_eml[0] != "html") {
$mpt = 1;
$testo_eml = (string) risul_query($contratto,0,'testo');
...
if (!empty($manda_plain_text)) $mpt = 1;
} # fine if (fixstr($formato_email) == "html" and $priv_mod_doc_html == "s") 
else {
$n_opz = ";";
if ($opz_eml[0] == "html") {
$iimg_esist = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
...
} # fine else if (fixstr($formato_email) == "html" and $priv_mod_doc_html == "s") 
...
if (isset($opz_eml[3])) $n_opz .= ";".$opz_eml[3];
esegui_query("update $tablecontratti set testo = '$n_opz' where numero = '$num_contratto' and tipo = 'opzeml' ");
...
if (defined('C_RESTRIZIONI_DEMO_ADMIN') and C_RESTRIZIONI_DEMO_ADMIN == "SI") $n_contratto = htmlspecialchars($n_contratto,ENT_COMPAT);
...
$contr_controlla_int = preg_replace("/\\[c +[A-Za-z]+[A-Za-z0-9_]*(\\((('[^'\\]\\(]*')|([A-Za-z]+[A-Za-z0-9_]*))\\))? *(!?=|!?%|>|<) *(\"[^\"]*\"|[A-Za-z]+[A-Za-z0-9_]*(\\((('[^'\\]\\(]*')|([A-Za-z]+[A-Za-z0-9_]*))\\))?) *((\\||&) *[A-Za-z]+[A-Za-z0-9_]*(\\((('[^'\\]\\(]*')|([A-Za-z]+[A-Za-z0-9_]*))\\))? *(!?=|!?%|>|<) *(\"[^\"]*\"|[A-Za-z]+[A-Za-z0-9_]*(\\((('[^'\\]\\(]*')|([A-Za-z]+[A-Za-z0-9_]*))\\))?) *)*\\]/","[c]",$contr_controlla_int);
$contr_controlla = explode("[c]",$contr_controlla_int);
$num_contr_controlla = count($contr_controlla);
$parte = explode("[/c]",$contr_controlla[0]);
if (count($parte) != 1) $errore = "SI";
for ($num1 = 1 ; $num1 < $num_contr_controlla ; $num1++) {
...
#$pos_curs += (2 * substr_count(substr($n_contratto,0,$pos_curs),"–"));
$testo_contratto_mod = tronca_testo($n_contratto,0,$pos_curs)."[".$var_agg."]".tronca_testo($n_contratto,$pos_curs);
...
$testo_agg2 = "\n".mex("SOSTITUISCI CON PARTE DA MOSTRARE SOLO SE E' SODDISFATTA LA CONDIZIONE PRECEDENTE",$pag).": ".mex("nome_variabile",$pag)." =".mex("valore",$pag).", ".mex("OPPURE",$pag)." !=".mex("valore",$pag).", %".mex("valore",$pag)." (".mex("contiene",'modifica_var_contr.php')."), !%".mex("valore",$pag).", >".mex("valore",$pag).", <".mex("valore",$pag).". ";
$testo_agg2 .= mex("Il valore può essere un testo tra virgolette \" o un'altra variabile",$pag).". ";
$testo_agg2 .= mex("Unisci più condizioni con",$pag)." & (AND) ".strtolower(mex("OPPURE",$pag))." | (OR): ".mex("nome_variabile",$pag)."1!=\"\" | ".mex("nome_variabile",$pag)."2>\"0\" &... ".mex("considerate sempre come raggruppate a destra",$pag).": ".mex("condizione",$pag)."1 & (".mex("condizione",$pag)."2 | (".mex("condizione",$pag)."3 & ".mex("condizione",$pag)."4))\n";
$testo_agg3 = "[/c]";
} # fine if ($tipo_rip == 8)
if (!strcmp((string) $pos_curs,"")) $testo_contratto_mod = $n_contratto.$testo_agg1.$testo_agg2.$testo_agg3;
else {
#$pos_curs = strlen(utf8_encode(substr(utf8_decode($n_contratto),0,$pos_curs)));
if (!strcmp((string) $pos_fine_sel,"")) $testo_contratto_mod = tronca_testo($n_contratto,0,$pos_curs).$testo_agg1.$testo_agg2.$testo_agg3.tronca_testo($n_contratto,$pos_curs);
else {
#$pos_fine_sel = strlen(utf8_encode(substr(utf8_decode($n_contratto),0,$pos_fine_sel)));
$testo_contratto_mod = tronca_testo($n_contratto,0,$pos_curs).$testo_agg1.tronca_testo($n_contratto,$pos_curs,($pos_fine_sel - $pos_curs)).$testo_agg3.tronca_testo($n_contratto,$pos_fine_sel);
...
<div style=\"padding-top: 6px;\"><em>".mex("Nota",$pag)."</em>: ".mex("aggiungendo",$pag)." <em>&del=1</em> (".mex("o",$pag)." <em>&del=2</em>) ".mex("alla URL verranno cercate solo (o anche) le prenotazioni cancellate, anche nelle URL successive",$pag).".</div>
...
<div style=\"padding-top: 6px;\">".mex("Altri esempi dell'uso della variabile",$pag)." <em>res_num</em>: <em>res_num=3</em> / <em>res_num=3-</em> / <em>res_num=3,7</em> / <em>res_num=3,6-9</em> / <em>res_num=".($anno - 1).".90</em> (".mex("numero del",$pag)." ".($anno - 1).", ".mex("prima di essere importata nel",$pag)." $anno)</div>
...
$pag_escluse = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'pag_esc'");
if (numlin_query($pag_escluse)) $pag_escluse = (string) risul_query($pag_escluse,0,'testo');
else $pag_escluse = "";
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"$pag\"><div style=\"line-height: 1.6;\">
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"num_contratto\" value=\"$num_contratto\">
<input type=\"hidden\" name=\"cambia_qualcosa\" value=\"SI\">
".mex("Pagine dove non mostrare questo documento",$pag).": ";
$pag_escluse_vett = explode(",",$pag_escluse);
$num_pag_escluse = count($pag_escluse_vett);
$str_pag_escluse = "";
for ($num1 = 1 ; $num1 < ($num_pag_escluse - 1) ; $num1++) {
$pag_esc_corr = "";
$span_style = " style=\"border: 1px solid grey; border-radius: 4px; padding: 0 3px 0 2px; white-space: nowrap;\"";
if ($pag_escluse_vett[$num1] == "tab_mese") $pag_esc_corr = "<span$span_style><em>".mex("tabella del mese",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=tab_mese\">x</a></span>";
if ($pag_escluse_vett[$num1] == "tab_prenota") $pag_esc_corr = "<span$span_style><em>".mex("tabella delle prenotazioni",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=tab_prenota\">x</a></span>";
if ($pag_escluse_vett[$num1] == "tab_clienti") $pag_esc_corr = "<span$span_style><em>".mex("tabella dei clienti",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=tab_clienti\">x</a></span>";
if ($pag_escluse_vett[$num1] == "messaggi") $pag_esc_corr = "<span$span_style><em>".mex("tabella dei messaggi",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=messaggi\">x</a></span>";
if ($pag_escluse_vett[$num1] == "contr_disp") $pag_esc_corr = "<span$span_style><em>".mex("controllo disponibilità",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=contr_disp\">x</a></span>";
if ($pag_escluse_vett[$num1] == "mod_prenota") $pag_esc_corr = "<span$span_style><em>".mex("modifica della prenotazione",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=mod_prenota\">x</a></span>";
if ($pag_escluse_vett[$num1] == "mod_cliente") $pag_esc_corr = "<span$span_style><em>".mex("modifica del cliente",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=mod_cliente\">x</a></span>";
if ($pag_escluse_vett[$num1] == "pos") $pag_esc_corr = "<span$span_style><em>".mex("punto vendita",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=pos\">x</a></span>";
if ($pag_escluse_vett[$num1] == "casse") $pag_esc_corr = "<span$span_style><em>".mex("casse",$pag)."</em> <a href=\"./$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;num_contratto=$num_contratto&amp;cambia_qualcosa=SI&amp;pag_esc_elim=casse\">x</a></span>";
if ($pag_esc_corr) {
if ($str_pag_escluse) $str_pag_escluse .= ", ";
$str_pag_escluse .= $pag_esc_corr;
} # fine if ($pag_esc_corr)
} # fine for $num1
if ($str_pag_escluse) echo "$str_pag_escluse. ";
if ($num_pag_escluse < 11) {
echo "<span style=\"white-space: nowrap;\"><select name=\"pag_escludi\">";
if (!strstr($pag_escluse,",tab_mese,")) echo "<option value=\"tab_mese\">".mex("tabella del mese",$pag)."</option>";
if (!strstr($pag_escluse,",tab_prenota,")) echo "<option value=\"tab_prenota\">".mex("tabella delle prenotazioni",$pag)."</option>";
if (!strstr($pag_escluse,",tab_clienti,")) echo "<option value=\"tab_clienti\">".mex("tabella dei clienti",$pag)."</option>";
if (!strstr($pag_escluse,",messaggi,")) echo "<option value=\"messaggi\">".mex("tabella dei messaggi",$pag)."</option>";
if (!strstr($pag_escluse,",contr_disp,")) echo "<option value=\"contr_disp\">".mex("controllo disponibilità",$pag)."</option>";
if (!strstr($pag_escluse,",mod_prenota,")) echo "<option value=\"mod_prenota\">".mex("modifica della prenotazione",$pag)."</option>";
if (!strstr($pag_escluse,",mod_cliente,")) echo "<option value=\"mod_cliente\">".mex("modifica del cliente",$pag)."</option>";
if (!strstr($pag_escluse,",pos,")) echo "<option value=\"pos\">".mex("punto vendita",$pag)."</option>";
if (!strstr($pag_escluse,",casse,")) echo "<option value=\"casse\">".mex("casse",$pag)."</option>";
echo "</select>
<input class=\"sbutton\" type=\"submit\" name=\"agg_pag_escludi\" value=\"".mex("Aggiungi",$pag)."\"></span>";
} # fine if ($num_pag_escluse < 11)
echo "</div></form><br>";
...
echo "<table cellspacing=0 cellpadding=0 style=\"line-height: 1;\"><tr><td valign=\"top\" style=\"padding-top: 4px;\">
".mex("Formato dell'email",$pag).":</td><td>
...
if ($priv_mod_doc_html == "s") echo "<option value=\"html\"$sel>".mex("html",$pag)."</option>";
echo "</select>";
if ($opz_eml[0] == "html") {
if (!empty($opz_eml[3])) $checked_mpt = " checked";
else $checked_mpt = "";
echo "<br><label><input type=\"checkbox\" name=\"manda_plain_text\" value=\"SI\"$checked_mpt>".mex("Invia anche la versione alternativa in testo semplice",$pag)." (".mex("etichette html rimosse",$pag).")</label><br>";
} # fine if ($opz_eml[0] == "html")
echo " <input class=\"sbutton\" type=\"submit\" name=\"cambia_formato_email\" value=\"".mex("Cambia",$pag)."\">
</div></form>";
if ($opz_eml[0] == "html") {
$img_inline = esegui_query("select * from $tablecontratti where numero = '$num_contratto' and tipo = 'img_inln' ");
if (numlin_query($img_inline)) $img_inline = (string) risul_query($img_inline,0,'testo');
else $img_inline = "";
$img_inline_orig = $img_inline;
$d_num_iimg = 0;
$iimg_presente = array();
if ($img_inline) {
$img_inline = explode(",",$img_inline);
$d_num_iimg = (count($img_inline) - 2);
for ($num1 = 1 ; $num1 <= $d_num_iimg ; $num1++) {
$cid = substr(strstr($img_inline[$num1],":"),1);
$img_inline[$num1] = str_replace(":$cid","",$img_inline[$num1]);
$iimg_presente[$img_inline[$num1]] = $cid;
} # fine for $num1
} # fine if ($img_inline)
$file_iimg = esegui_query("select * from $tablecontratti where tipo = 'file_iim' ");
$num_file_iimg = numlin_query($file_iimg);
$opt_iimg = "";
for ($num1 = 0 ; $num1 < $num_file_iimg ; $num1++) {
$num_iimg = risul_query($file_iimg,$num1,'numero');
if (!isset($iimg_presente[$num_iimg])) {
$nome_iimg = explode(",",risul_query($file_iimg,$num1,'testo'));
$opt_iimg .= "<option value=\"$num_iimg\">".$nome_iimg[0]."</option>";
} # fine if (!isset($iimg_presente[$num_iimg]))
} # fine for $num1
if ($img_inline) {
for ($num1 = 1 ; $num1 <= $d_num_iimg ; $num1++) {
echo "<table><tr><td>".mex("Immagine inline",$pag)." (".mex("incorporata",$pag).") $num1:&nbsp;</td><td>";
$num_iimg = $img_inline[$num1];
$iimg = esegui_query("select * from $tablecontratti where numero = '$num_iimg' and tipo = 'file_iim' ");
$iimg = risul_query($iimg,0,'testo');
$iimg = explode(",",$iimg);
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"$pag\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"num_contratto\" value=\"$num_contratto\">
<input type=\"hidden\" name=\"cambia_qualcosa\" value=\"SI\">
<input type=\"hidden\" name=\"num_elimina_iimg\" value=\"$num_iimg\">
<b>".$iimg[0]."</b> (cid: <em>".$iimg_presente[$num_iimg]."</em>)";
echo "<input class=\"sbutton\" type=\"submit\" name=\"elimina_iimg\" value=\"".mex("Elimina",$pag)."\">
</div></form></td></tr>";
echo "</table>";
} # fine for $num1
} # fine if ($img_inline)
if ($d_num_iimg < $num_allegati_max) {
$ncid = ($d_num_iimg + 1);
while (strstr($img_inline_orig,":img$ncid,")) $ncid++;
echo "<table cellspacing=\"0\"><tr><td>
".mex("Immagine inline",$pag).":</td><td>
...
<input type=\"hidden\" name=\"num_contratto\" value=\"$num_contratto\">
<input type=\"hidden\" name=\"cambia_qualcosa\" value=\"SI\">
 <input name=\"file_iimg_upload\" type=\"file\">&nbsp;
 Content-ID (cid): <input type=\"text\" name=\"cid\" value=\"img$ncid\" size=\"8\" maxlength=\"20\">
 <input class=\"sbutton\" type=\"submit\" name=\"upload_iimg\" value=\"".mex("Aggiungi",$pag)."\">
</div></form></td></tr>";
if ($opt_iimg) {
echo "<tr><td></td><td>
<form accept-charset=\"utf-8\" enctype=\"multipart/form-data\" method=\"post\" action=\"$pag\"><div>
...
<input type=\"hidden\" name=\"num_contratto\" value=\"$num_contratto\">
<input type=\"hidden\" name=\"cambia_qualcosa\" value=\"SI\">
<input type=\"hidden\" name=\"aggiungi_iimg\" value=\"SI\">
<select name=\"num_iimg\">
$opt_iimg</select>
 cid: <input type=\"text\" name=\"cid\" value=\"img$ncid\" size=\"8\" maxlength=\"20\">
<input class=\"sbutton\" type=\"submit\" value=\"".mex("Aggiungi",$pag)."\">
</div></form></td></tr>";
} # fine if ($opt_iimg)
echo "</table>";
} # fine if ($d_num_iimg < $num_allegati_max)
} # fine if ($opz_eml[0] == "html")
echo "</td></tr></table>";
...
if (numlin_query($testo_contratto)) $testo_contratto = (string) risul_query($testo_contratto,0,'testo');
