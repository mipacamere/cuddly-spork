...
if (defined('C_CREA_ULTIMO_ACCESSO') and C_CREA_ULTIMO_ACCESSO == "SI") $filelock_ua = crea_lock_file(C_DATI_PATH."/ultimo_accesso");
...
$lingua = fixstr($lingua);
$locale_lingua = "";
if (strstr($lingua,"-")) {
$lingua = explode("-",$lingua);
if ($lingua[1] and preg_replace("/[a-z]*/i","",$lingua[1]) == "" and strlen($lingua[1]) < 4) $locale_lingua = $lingua[1];
$lingua = $lingua[0];
} # fine if (strstr($lingua,"-"))
if (!$lingua or preg_replace("/[a-z]*/i","",$lingua) != "" or strlen($lingua) > 3) $lingua = "en";
if (!is_dir("./includes/lang/$lingua")) $lingua = "ita";
if ($locale_lingua and ($lingua == "ita" or !is_file("./includes/lang/$lingua/locales/$locale_lingua/l_n"))) $locale_lingua = "";
if ($locale_lingua) $lingua_locale = "$lingua-$locale_lingua";
else $lingua_locale = $lingua;
if ($lingua == "ita") include("./includes/locale.php");
else {
if ($locale_lingua and is_file("./includes/lang/$lingua/locales/$locale_lingua/locale.php")) include("./includes/lang/$lingua/locales/$locale_lingua/locale.php");
else include("./includes/lang/$lingua/locale.php");
...
if ($tipo_db and $tipo_db != "mysqli" and $tipo_db != "postgresql" and $tipo_db != "sqlite") $tipo_db = "mysql";
...
$numapp_default = 0;
if ((empty($numappartamenti) and empty($numletti)) or controlla_num_pos($numappartamenti) == "NO") {
$numappartamenti = 5;
$numapp_default = 1;
} # fine if ((empty($numappartamenti) and empty($numletti)) or controlla_num_pos($numappartamenti) == "NO")
...
$prefisso_tab = trim(fixstr($prefisso_tab));
if (!$prefisso_tab or (preg_match('/^[a-z][_0-9a-z]*$/',$prefisso_tab) and strlen($prefisso_tab) <= 12)) {
$numconnessione = "";
$host_phprdb = substr(preg_replace("/[^a-z0-9-.]/i","",fixstr($host_phprdb)),0,253);
$port_phprdb = trim(fixstr($port_phprdb));
if ($port_phprdb and (controlla_num_pos($port_phprdb) != "SI" or $port_phprdb > "65535")) $port_phprdb = "";
$user_phprdb = substr(trim(str_replace(" ","",fixstr($user_phprdb))),0,128);
$password_phprdb = substr(trim(str_replace(" ","",fixstr($password_phprdb))),0,128);

if ($tipo_db == "postgresql") {
if ($carica_estensione == "SI") dl("pgsql.so");
$tempdatabase = preg_replace("/[^a-z0-9_]/i","",fixstr($tempdatabase));
if ($database_esistente == "SI") $tempdatabase = $database_phprdb;
if ($tempdatabase) $numconnessione = pg_connect("dbname=$tempdatabase host=$host_phprdb port=$port_phprdb user=$user_phprdb password=$password_phprdb ");
...
if ($tipo_db == "mysqli") $link_mysqli = $numconnessione;
...
if (isset($locale['currency_name']) and (string) $locale['currency_name'] != "") $Euro = $locale['currency_name'];
else $Euro = mex2("Euro",$pag,$lingua);
esegui_query("insert into $tablepersonalizza (idpersonalizza,idutente,valpersonalizza) values ('valuta','1','".aggslashdb($Euro)."')");
$arrotond_predef = formatta_soldi(fixset($locale['prices_percentage_rounding']));
if ((float) $arrotond_predef == 0 or controlla_soldi($arrotond_predef) != "SI")  $arrotond_predef = "1";
esegui_query("insert into $tablepersonalizza (idpersonalizza,idutente,valpersonalizza) values ('arrotond_predef','1','".aggslashdb($arrotond_predef)."')");
$arrotond_tasse = formatta_soldi(fixset($locale['taxes_rounding']));
if ((float) $arrotond_tasse == 0 or controlla_soldi($arrotond_tasse) != "SI") $arrotond_tasse = "0.01";
esegui_query("insert into $tablepersonalizza (idpersonalizza,idutente,valpersonalizza) values ('arrotond_tasse','1','".aggslashdb($arrotond_tasse)."')");
if (isset($locale['money_format']) and ($locale['money_format'] == "usa" or $locale['money_format'] == "europa")) $stile_soldi = $locale['money_format'];
else {
if ($lingua == "ita" or $lingua == "es" or $lingua == "fr" or $lingua == "de" or $lingua == "pt") $stile_soldi = "europa";
else $stile_soldi = "usa";
} # fine else if (isset($locale['money_format']) and ($locale['money_format'] == "usa" or...
...
if (isset($locale['date_format']) and ($locale['date_format'] == "usa" or $locale['date_format'] == "europa")) $stile_data = $locale['date_format'];
else $stile_data = "europa";
esegui_query("insert into $tablepersonalizza (idpersonalizza,idutente,valpersonalizza) values ('stile_data','1','$stile_data')");
...
if ($numappartamenti > 12) $attiva_checkin = "SI";
else $attiva_checkin = "NO";
esegui_query("insert into $tablepersonalizza (idpersonalizza,idutente,valpersonalizza) values ('attiva_checkin','1','$attiva_checkin')");
...
if ($locale_lingua and is_file($f_pre."./includes/lang/$lingua/locales/$locale_lingua/hoteld_doc_backup.php")) $file_contr_backup = $f_pre."./includes/lang/$lingua/locales/$locale_lingua/hoteld_doc_backup.php";
else {
...
} # fine else if ($locale_lingua and is_file($f_pre."./includes/lang/$lingua/locales/$locale_lingua/hoteld_doc_backup.php"))
} # fine else if ($lingua == "ita")
...
$diff_tz = "";
if (!empty($locale['timezone']) and function_exists('timezone_identifiers_list') and function_exists('date_default_timezone_set')) {
$tzlist = timezone_identifiers_list();
foreach ($tzlist as $id => $val) if ($val == $locale['timezone']) $diff_tz = $locale['timezone'];
unset($tzlist);
} # fine if (!empty($locale['timezone']) and function_exists('timezone_identifiers_list') and...
fwrite($fileaperto,"<?php
define('C_VERSIONE_ATTUALE',".C_PHPR_VERSIONE_NUM.");
define('C_DIFF_ORE',0);
");
if ($diff_tz) fwrite($fileaperto,"define('C_DIFF_TZ','$diff_tz');
");
fwrite($fileaperto,"define('C_MIN_SESSIONE',90);
...
else $num_app_max = $numappartamenti + ceil((float) $numletti / 26);
...
} # fine if (!$prefisso_tab or (preg_match('/^[_a-z][_0-9a-z]*$/',$prefisso_tab) and...
else {
...
$torna_indietro = "SI";
} # fine else if (!$prefisso_tab or (preg_match('/^[_a-z][_0-9a-z]*$/',$prefisso_tab) and...
...
<input type=\"hidden\" name=\"lingua\" value=\"$lingua_locale\">
...
} # fine if (!empty($app_vicini_vett[$num]))
} # fine for $num
} # fine if ($num_camera_l)

if (defined('C_UTILIZZA_SEMPRE_DEFAULTS') and C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO" and @is_file(C_DATI_PATH."/ini.php")) {
$filelock = crea_lock_file(C_DATI_PATH."/ini.php");
if (@is_file(C_DATI_PATH."/ini.php")) {
...
esegui_query("update $tableutenti set nome_utente = '".aggslashdb($admin)."' where idutenti = '1'",1);
$passw = "";
...
esegui_query("update $tableutenti set password = '$passw', salt = '$salt', tipo_pass = '5' where idutenti = '1'",1);
...
} # fine if (@is_file(C_DATI_PATH."/ini.php"))
distruggi_lock_file($filelock,C_DATI_PATH."/ini.php");
...
if (defined('C_CREA_ULTIMO_ACCESSO') and C_CREA_ULTIMO_ACCESSO == "SI") distruggi_lock_file($filelock_ua,C_DATI_PATH."/ultimo_accesso");
