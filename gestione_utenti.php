...
if (htmlspecialchars(${"nome".$id},ENT_COMPAT) != ${"nome".$id}) ${"nome".$id} = $nome;
...
$n_tipo_pass = ${"tipo_pass".$id};
if ($n_tipo_pass and $tipo_pass != $n_tipo_pass and $n_tipo_pass != "n") {
if (!${"prima_pass".$id} or ${"prima_pass".$id} != ${"seconda_pass".$id} or ${"prima_pass".$id} != str_replace("&","",${"prima_pass".$id})) {
$continua = "NO";
echo mex("Nuova password dell'utente",$pag)." $id ".mex("<div style=\"display: inline; color: red;\">non</div> inserita correttamente",$pag).".<br>";
} # fine if (!${"prima_pass".$id} or ${"prima_pass".$id} != ${"seconda_pass".$id} or...
else {
$lung_min = 4;
if (defined('C_MINIMA_LUNGHEZZA_PASS') and C_MINIMA_LUNGHEZZA_PASS > 0 and controlla_num_pos(C_MINIMA_LUNGHEZZA_PASS) == "SI") $lung_min = C_MINIMA_LUNGHEZZA_PASS;
$lung_pass = strlen(${"prima_pass".$id});
if ($lung_pass < $lung_min) {
$continua = "NO";
echo "<span style=\"color: red;\">".mex("Password troppo debole",$pag)."</span> (".mex("almeno",$pag)." $lung_min ".mex("caratteri",$pag).").<br>";
} # fine if ($lung_pass < $lung_min)
else {
$pass_debole = 0;
if (strstr("012345678901234567890",${"prima_pass".$id}) or strstr("098765432109876543210",${"prima_pass".$id})) $pass_debole = 1;
else {
$max_lung_ripetizioni = 1;
$ripetizione_corr = 1;
$ultimo_car = substr(${"prima_pass".$id},0,1);
for ($num2 = 1 ; $num2 < $lung_pass ; $num2++) {
$car_corr = substr(${"prima_pass".$id},$num2,1);
if ($car_corr == $ultimo_car) $ripetizione_corr++;
else $ripetizione_corr = 1;
if ($ripetizione_corr > $max_lung_ripetizioni) $max_lung_ripetizioni = $ripetizione_corr;
$ultimo_car = $car_corr;
} # fine for $num2
if ($max_lung_ripetizioni > ceil($lung_pass / 2) and $max_lung_ripetizioni > ($lung_pass - $lung_min)) $pass_debole = 1;
} # fine else if (strstr("012345678901234567890",${"prima_pass".$id}) or...
if ($pass_debole) {
$continua = "NO";
echo "<span style=\"color: red;\">".mex("Password troppo debole",$pag)."</span>.<br>";
} # fine if ($pass_debole)
} # fine else if ($lung_pass < $lung_min)
} # fine else if (!${"prima_pass".$id} or ${"prima_pass".$id} != ${"seconda_pass".$id} or...
} # fine if ($n_tipo_pass and $tipo_pass != $n_tipo_pass and...
} # fine for $num1
if ($continua == "NO") {
...
$salt = "";
if ($n_tipo_pass == "5") {
srand((int) ((float) microtime() * 1000000.0));
...
if (htmlspecialchars($nome,ENT_COMPAT) != $nome) $nome = "";
if ($nome) {
...
} # fine if (substr($file_tema[$num1],0,2) ==..
...
if ($prima_pass != $seconda_pass or $prima_pass == "" or $prima_pass != str_replace("&","",$prima_pass)) {
unset($continua);
echo mex("Le nuove password non coincidono",$pag).".<br><br>";
} # fine if ($prima_pass != $seconda_pass or...
else {
$lung_min = 4;
if (defined('C_MINIMA_LUNGHEZZA_PASS') and C_MINIMA_LUNGHEZZA_PASS > 0 and controlla_num_pos(C_MINIMA_LUNGHEZZA_PASS) == "SI") $lung_min = C_MINIMA_LUNGHEZZA_PASS;
$lung_pass = strlen($prima_pass);
if ($lung_pass < $lung_min) {
unset($continua);
echo "<span style=\"color: red;\">".mex("Password troppo debole",$pag)."</span> (".mex("almeno",$pag)." $lung_min ".mex("caratteri",$pag).").<br><br>";
} # fine if ($lung_pass < $lung_min)
else {
$pass_debole = 0;
if (strstr("012345678901234567890",$prima_pass) or strstr("098765432109876543210",$prima_pass)) $pass_debole = 1;
else {
$max_lung_ripetizioni = 1;
$ripetizione_corr = 1;
$ultimo_car = substr($prima_pass,0,1);
for ($num2 = 1 ; $num2 < $lung_pass ; $num2++) {
$car_corr = substr($prima_pass,$num2,1);
if ($car_corr == $ultimo_car) $ripetizione_corr++;
else $ripetizione_corr = 1;
if ($ripetizione_corr > $max_lung_ripetizioni) $max_lung_ripetizioni = $ripetizione_corr;
$ultimo_car = $car_corr;
...
if ($max_lung_ripetizioni > ceil($lung_pass / 2) and $max_lung_ripetizioni > ($lung_pass - $lung_min)) $pass_debole = 1;
} # fine else if (strstr("012345678901234567890",$prima_pass) or...
if ($pass_debole) {
unset($continua);
echo "<span style=\"color: red;\">".mex("Password troppo debole",$pag)."</span>.<br><br>";
} # fine if ($pass_debole)
} # fine else if ($lung_pass < $lung_min)
} # fine else if (prima_pass != $seconda_pass or...
...
if (htmlspecialchars($nuovo_gruppo,ENT_COMPAT) != $nuovo_gruppo) $nuovo_gruppo = "";
...
if (isset($gruppi_utente[$id_gruppo]) and $gruppi_utente[$id_gruppo] == "SI") { $checked = " checked"; $tag_b = "<b>"; $slash_b = "</b>"; }
...
for ($num1 = 0 ; $num1 < $num_lista_gruppi ; $num1++) {
$id_gruppo = risul_query($lista_gruppi,$num1,'idgruppi');
if ((!isset($gruppi_utente[$id_gruppo]) or $gruppi_utente[$id_gruppo] != "SI") and isset(${"gruppo".$id_gruppo}) and ${"gruppo".$id_gruppo} == "SI") {
...
} # fine if ((!isset($gruppi_utente[$id_gruppo]) or $gruppi_utente[$id_gruppo] != "SI") and isset(${"gruppo".$id_gruppo}) and...
if (isset($gruppi_utente[$id_gruppo]) and $gruppi_utente[$id_gruppo] == "SI" and (!isset(${"gruppo".$id_gruppo}) or ${"gruppo".$id_gruppo} != "SI")) {
...
} # fine if (isset($gruppi_utente[$id_gruppo]) and $gruppi_utente[$id_gruppo] == "SI" and (!isset(${"gruppo".$id_gruppo}) or...
} # fine for $num1
if ($nuovo_gruppo) {
...
$locale_mex = substr(strstr($lingua[$id_utente_importa],"-"),1);
$lingua_mex = str_replace("-$locale_mex","",$lingua[$id_utente_importa]);
