...
$var_pag[92] = 'ripristina';
$var_pag[93] = 'ripristina_c';
$n_var_pag = 94;
$num2 = 0;
...
$priv_app_gruppi = "NO";
if ($priv_mod_checkin == "s" and (!empty($ins_checkin) or !empty($ins_checkout))) {
...
$tariffe_consentite_vett_gr = array();
if ($attiva_tariffe_consentite == "s") $tariffe_consentite_vett_gr[$id_utente] = $tariffe_consentite_vett;
...
} # fine if ($priv_mod_checkin == "s" and (!empty($ins_checkin) or !empty($ins_checkout)))
...
if ((!empty($cancella) and $priv_canc_prenotazioni != "n")) $iddf = 1;
else $iddf = 0;
...
controlla_id_prenota($id_prenota,$id_prenota_idpr,$num_id_prenota,$id_prenota_int,$priv_mod_prenotazioni,$anno,$PHPR_TAB_PRE,$pcanc,$iddf);
...
if ($priv_canc_prenotazioni == "f" or ($priv_ins_periodi_passati != "s" and $pcanc)) $id_periodo_corrente = calcola_id_periodo_corrente($anno);
...
if ((!empty($cancella) and $priv_canc_prenotazioni != "n") or (!empty($ripristina) and $pcanc and $priv_ins_nuove_prenota != "n")) {
...
$ripristina = "";
$prenota_interconn = trova_prenota_interconn($lista_interconn,$lista_utenti_mess_interconn,$num_id_prenota,$id_prenota_idpr,$anno,$tableinterconnessioni);

if (!isset($cancella_c) or $cancella_c != "SI") {
if ($prenota_interconn) {
echo "<br><span class=\"colwarn\">".mex("Attenzione",$pag)."</span>: ".mex("cancellare manualmente una prenotazione proveniente dalle interconnessioni",$pag);
if ($num_id_prenota > 1) echo " (<em>$prenota_interconn</em>)";
echo ", ".mex("senza aspettare che venga cancellata dall'interconnessione",$pag).",  <span class=\"colred\">".mex("può creare overbooking",$pag)."</span>.<br><br>";
} # fine if ($prenota_interconn)
...
$cognome_cliente = esegui_query("select cognome,cognome2 from $tableclienti where idclienti = '$cognome_cliente' ");
$cognome2 = (string) risul_query($cognome_cliente,0,'cognome2');
$cognome_cliente = risul_query($cognome_cliente,0,'cognome');
if (strcmp($cognome2,"")) $cognome_cliente .= " $cognome2";
...
for ($num_idpr = 0 ; $num_idpr < $num_id_prenota ; $num_idpr++) {
$id_prenota = $id_prenota_idpr[$num_idpr];
$testo = cancella_prenotazione($id_prenota,fixset($cancella_pagato),$metodo_pagamento,$priv_mod_pagato,1,$anno,$PHPR_TAB_PRE,$id_utente,$stato_canc_corr,"",1,$modo_canc,$cassa_pagamenti);
if ($stato_canc_corr['canc'] != "NO") $stato_canc = "SI";
echo $testo;
} # fine for $num_idpr

if ($prenota_interconn) {
$testo = "<br><span class=\"colwarn\">".mex("Attenzione",$pag)."</span>: ".mex("cancellare manualmente una prenotazione proveniente dalle interconnessioni",$pag)." (<em>$prenota_interconn</em>)".", ".mex("senza aspettare che venga cancellata dall'interconnessione",$pag).",  <span class=\"colred\">".mex("può creare overbooking",$pag)."</span>.<br><br>";
spedisci_mess_interconn($testo,$prenota_interconn,$lista_interconn,$lista_utenti_mess_interconn,$anno,$tableutenti,$tablerelgruppi,$tablemessaggi);
} # fine if ($prenota_interconn)
...
if (!empty($ripristina) and $pcanc and $priv_ins_nuove_prenota != "n") {
$inserire = 1;
...
if ($id_periodo_corrente >= $id_inizio_prenota) {
$inserire = 0;
echo "<br>".mex("Non si possono inserire prenotazioni nel passato",$pag).".<br><br>";
break;
} # fine if ($id_periodo_corrente >= $id_inizio_prenota)
} # fine for $num_idpr
} # fine if ($priv_ins_periodi_passati != "s")
if ($attiva_regole1_consentite == "s" and $inserire) {
...
if (empty($condizioni_regole1_consentite)) {
$inserire = 0;
echo "<br>".mex("Non c'è nussun periodo delle regole 1 in cui sia consentito inserire prenotazioni per l'utente",'clienti.php')." $nome_utente.<br><br>";
} # fine if (empty($condizioni_regole1_consentite))
else {
...
$d_appartamento = (string) risul_query($dati_prenota,0,'idappartamenti');
$d_idinizioprenota = risul_query($dati_prenota,0,'iddatainizio');
$d_idfineprenota = risul_query($dati_prenota,0,'iddatafine');
$app_consentito = esegui_query("select idregole,iddatainizio,iddatafine from $tableregole where app_agenzia = '".aggslashdb($d_appartamento)."' and (motivazione2 != 'x' or motivazione2 is NULL) and iddatainizio <= '$d_idfineprenota' and iddatafine >= '$d_idinizioprenota' and $condizioni_regole1_consentite order by iddatainizio");
for ($num2 = 0 ; $num2 < numlin_query($app_consentito) ; $num2++) {
$iddatainizio_regola = risul_query($app_consentito,$num2,'iddatainizio');
$iddatafine_regola = risul_query($app_consentito,$num2,'iddatafine');
if ($num2 == 0) {
...
if (!numlin_query($app_consentito) or $iddatainizio_regole_tot > $d_idinizioprenota or $iddatafine_regole_tot < $d_idfineprenota) {
$inserire = 0;
echo "<br>".mex("Non c'è nessun appartamento tra quelli richiesti in cui sia consentito inserire prenotazioni per l'utente",'unit.php')." $nome_utente.<br><br>";
break;
} # fine if (!numlin_query($app_consentito) or $iddatainizio_regole_tot > $d_idinizioprenota or $iddatafine_regole_tot < $d_idfineprenota)
} # fine for $num_idpr
} # fine else if (empty($condizioni_regole1_consentite))
} # fine if ($attiva_regole1_consentite == "s" and $inserire)
if (!$inserire) {
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"$pag\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
...
if (!isset($ripristina_c) or $ripristina_c != "SI") {
if ($applica_regole1 == "s" or $applica_regole1 == "n") {
$condizioni_regole1_non_sel = "";
if ($applica_regole1 == "n") {
...
$d_idinizioprenota = risul_query($dati_prenota,0,'iddatainizio');
$d_idfineprenota = risul_query($dati_prenota,0,'iddatafine');
$app_agenzia = esegui_query("select * from $tableregole where app_agenzia = '".aggslashdb($d_appartamento)."' and iddatainizio <= '$d_idfineprenota' and iddatafine >= '$d_idinizioprenota' and (motivazione2 != 'x' or motivazione2 is NULL)$condizioni_regole1_non_sel");
$num_app_agenzia = numlin_query($app_agenzia);
if ($num_app_agenzia) {
if (!$fr_spost) echo "<br>".mex("Si devono fare i seguenti spostamenti nei periodi della <div style=\"display: inline; color: blue;\">regola di assegnazione 1</div>",'clienti.php').":<br>";
$fr_spost = 1;
$motivazione_app_agenzia = risul_query($app_agenzia,0,'motivazione');
if ($motivazione_app_agenzia) $motivazione_app_agenzia = " (<b>$motivazione_app_agenzia</b>)";
echo mex("La nuova prenotazione verrà inserita nell'appartamento",'unit.php')." <b class=\"colblu\">$d_appartamento</b>$motivazione_app_agenzia.<br>";
} # fine if ($num_app_agenzia)
} # fine for $num_idpr
if ($fr_spost) echo "<br>";
} # fine if ($applica_regole1 == "s" or $applica_regole1 == "n")
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"modifica_prenota.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"mese\" value=\"$mese\">
...
$modo_canc = array('ripristina' => 1, 'controlla_app_chiusi' => 1);
$id_prenota_int_c = $id_prenota_int;

for ($num_idpr = 0 ; $num_idpr < $num_id_prenota ; $num_idpr++) {
$id_prenota = $id_prenota_idpr[$num_idpr];
$testo = cancella_prenotazione($id_prenota,"","",$priv_mod_pagato,1,$anno,$PHPR_TAB_PRE,$id_utente,$stato_canc_corr,"",1,$modo_canc);
echo $testo;
if ($stato_canc_corr['canc'] != "NO") {
$stato_canc = "SI";
$id_prenota_int_c = substr(str_replace(",$id_prenota,",",",",$id_prenota_int_c,"),1,-1);
...
<input type=\"hidden\" name=\"pcanc\" value=\"1\">";
} # fine if ($id_prenota_int_c)
else echo "<input type=\"hidden\" name=\"id_prenota\" value=\"$id_prenota_int\">";
echo "<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"mese\" value=\"$mese\">
<button class=\"cont\" type=\"submit\"><div>".mex("OK",$pag)."</div></button>
...
} # fine if (!empty($ripristina) and $pcanc and $priv_ins_nuove_prenota != "n")
...
} # fine if ((!empty($cancella) and $priv_canc_prenotazioni != "n" or (!empty($ripristina) and...
...
$dati_ospite = esegui_query("select cognome,cognome2,nome,utente_inserimento from $tableclienti where idclienti = '$id_clienti_osp' ");
$utente_ospite = risul_query($dati_ospite,0,'utente_inserimento');
$dati_osp .= "<option value=\"$id_clienti_osp\">";
if (($vedi_clienti == "PROPRI" and $utente_ospite != $id_utente) or ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_ospite]))) $dati_osp .= mex("Cliente",$pag)." $id_clienti_osp";
else {
$dati_osp .= risul_query($dati_ospite,0,'cognome')." ";
$cognome2_osp = (string) risul_query($dati_ospite,0,'cognome2');
if (strcmp($cognome2_osp,"")) $dati_osp .= "$cognome2_osp ";
$dati_osp .= risul_query($dati_ospite,0,'nome');
} # fine else if (($vedi_clienti == "PROPRI" and $utente_ospite != $id_utente) or...
$dati_osp .= "</option>";
} # fine for $num1
} # fine for $num_idpr
if ($dati_osp) {
...
if (($vedi_clienti == "PROPRI" and $utente_inserimento != $id_utente) or ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento]))) echo mex("Cliente",$pag)." <b>$id_clienti</b><br><br>";
...
".mex("a",$pag).":<br>
<table><tr><td style=\"width: 30px;\"></td><td>";
mostra_dati_cliente($dati_nuovo_cliente,$dcognome,$dcognome2,$dnome,$dsoprannome,$dtitolo_cli,$dsesso,$ddatanascita,$ddatanascita_f,$dnazionenascita,$dcittanascita,$dregionenascita,$ddocumento,$dscadenzadoc,$dscadenzadoc_f,$dtipodoc,$dnazionedoc,$dregionedoc,$dcittadoc,$dnazionalita,$dlingua_cli,$dnazione,$dregione,$dcitta,$dvia,$dnumcivico,$dtelefono,$dtelefono2,$dtelefono3,$dfax,$dcap,$demail,$demail2,$demail_cert,$dcod_fiscale,$dpartita_iva,"",$priv_ins_clienti);
echo "</td></tr></table><br>";
if ($pagamenti['e']) {
...
$n_caparra = (float) formatta_soldi($n_caparra) / (float) $altre_valute[$valuta]['cambio'];
$n_caparra = $n_caparra / (float) $dati_tariffe['tasse_arrotond'];
...
$n_commento = htmlspecialchars(fixstr($n_commento),ENT_COMPAT);
$nomecostofinale0 = htmlspecialchars(fixstr($nomecostofinale0),ENT_COMPAT);
#$n_met_paga_caparra = htmlspecialchars($n_met_paga_caparra,ENT_COMPAT);
$n_origine_prenota = htmlspecialchars(fixstr($n_origine_prenota),ENT_COMPAT);
...
if ($dati_cap[$numca]['letto'] == "s" and $dati_cat_pers['num']) $dati_form_modifica .= "<input type=\"hidden\" name=\"catpers_costoaggiuntivo$idca\" value=\"".htmlspecialchars(fixstr(${"catpers_costoaggiuntivo".$idca}))."\">";
} # fine for $numca
} # fine for $num_idpr
...
$prenota_interconn = "";
if ($n_inizioperiodo or $n_fineperiodo) {
$inizioperiodo = "";
$fineperiodo = "";
if ($n_inizioperiodo) {
$idinizioperiodo = esegui_query("select idperiodi from $tableperiodi where datainizio = '".aggslashdb($n_inizioperiodo)."' ");
...
if ($n_fineperiodo) {
$idfineperiodo = esegui_query("select idperiodi from $tableperiodi where datafine = '".aggslashdb($n_fineperiodo)."' ");
...
$id_prenota_idpr['iddi'] = $d_id_data_inizio_idpr;
$id_prenota_idpr['iddf'] = $d_id_data_fine_idpr;
$prenota_interconn = trova_prenota_interconn($lista_interconn,$lista_utenti_mess_interconn,$num_id_prenota,$id_prenota_idpr,$anno,$tableinterconnessioni,$inizioperiodo,$fineperiodo);
unset($id_prenota_idpr['iddi']);
unset($id_prenota_idpr['iddf']);
} # fine if ($n_inizioperiodo or $n_fineperiodo)
if ($prenota_interconn) {
echo "<br><span class=\"colwarn\">".mex("Attenzione",$pag)."</span>: ".mex("rimpicciolire manualmente una prenotazione proveniente dalle interconnessioni",$pag);
if ($num_id_prenota > 1) echo " (<em>$prenota_interconn</em>)";
echo ", ".mex("senza aspettare che venga modificata dall'interconnessione",$pag).",  <span class=\"colred\">".mex("può creare overbooking",$pag)."</span>.<br><br>";
} # fine if ($prenota_interconn)
...
if ($id_nuovo_utente_inserimento != "" and ($priv_mod_utente_ins == "s" or ($priv_mod_utente_ins == "g" and !empty($utenti_gruppi[$id_nuovo_utente_inserimento]))) and controlla_num_pos($id_nuovo_utente_inserimento) == "SI") {
...
$n_lista_app = htmlspecialchars(fixstr($n_lista_app),ENT_COMPAT);
...
$n_nometipotariffa = aggslashdb(str_replace("\"","",str_replace(">","",str_replace("<","",fixstr($n_nometipotariffa)))));
...
for ($num1 = 0 ; $num1 < $d_cat_persone['num'] ; $num1++) {
$cat_pers = $d_cat_persone['ord'][$num1];
if (fixset($d_cat_persone[$cat_pers]['esist']) != ($num1 + 1)) {
if (strcmp(fixstr(${"n_excat$cat_pers"."_numpers"}),"")) $cp_molt = ${"n_excat$cat_pers"."_numpers"};
...
} # fine if (fixset($d_cat_persone[$cat_pers]['esist']) != ($num1 + 1))
} # fine for $num1
...
if (empty($frase_da2)) $frase_da2 = "<b>".mex("mobile",'unit.php')."</b> ".mex("in",$pag)." <b>$d_appartamento</b>";
...
if ($app_incomp_costi_mod == "SI" and (!isset($app_richiesti[$d_appartamento]) or $app_richiesti[$d_appartamento] != "SI")) $verifica_app = "SI";
...
if ($inserire == "NO" and !empty($app_richiesti_copia)) $app_richiesti = $app_richiesti_copia;
...
if (!empty($num_costi_agg_letti[$num1])) $numca = $num_costi_agg_letti[$num1];
...
} # fine if ($attiva_costi_agg_consentiti == "n" or (isset($costi_agg_consentiti_vett[$dati_ca[$num1]['id']]) and...
...
if ($idclienti_camb) {
$cliente_camb = esegui_query("select cognome,cognome2 from $tableclienti where idclienti = '$idclienti_camb'");
$cognome2_camb = (string) risul_query($cliente_camb,0,'cognome2');
$cliente_camb = risul_query($cliente_camb,0,'cognome');
if (strcmp($cognome2_osp,"")) $cliente_camb .= " $cognome2_camb";
} # fine if ($idclienti_camb)
else $cliente_camb = "?";
...
$n_nometipotariffa_idpr = array();
$n_tariffesettimanali_idpr = array();
$n_costo_tariffa_idpr = array();
$costi_aggiuntivi_modificati_int_idpr = array();
$costi_aggiuntivi_da_inserire_int_idpr = array();
$n_sconto_caparra_idpr = array();
$n_pagato_idpr = array();
$n_confermato_idpr = array();
$n_costo_tot_idpr = array();
...
elseif (!empty($comm_pers_presenti[$tipo_commento])) echo mex("Il commento",$pag)." <b>\"<em>$tipo_commento</em>\"</b> ".mex("verrà cambiato",$pag).".<br>";
...
if ($cat_persone[$num1]['perc'] == "100") $coeff_cat_persone['i'] += (float) $cat_persone[$num1]['molt'];
else $coeff_cat_persone['p'] += ((float) $cat_persone[$num1]['molt'] * ((float) $cat_persone[$num1]['perc'] / 100));
} # fine if ($cat_persone[$num1]['molt'])
} # fine for $num1
for ($num1 = 0 ; $num1 < $d_cat_persone['num'] ; $num1++) {
if ($d_cat_persone[$num1]['molt']) {
if ($d_cat_persone[$num1]['perc'] == "100") $d_coeff_cat_persone += (float) $d_cat_persone[$num1]['molt'];
else $d_coeff_cat_persone += ((float) $d_cat_persone[$num1]['molt'] * ((float) $d_cat_persone[$num1]['perc'] / 100));
...
$d_tariffesett_vett[$num1] = (float) $d_tariffesett[($num1 - $d_id_data_inizio)];
if ($d_tariffesettp) {
$d_tariffesett_vett[$num1] = $d_tariffesett_vett[$num1] - (float) $d_tariffesettp[($num1 - $d_id_data_inizio)];
if ($d_tariffesettpo) $d_tariffesett_vett['p'][$num1] = (float) $d_tariffesettpo[($num1 - $d_id_data_inizio)];
else {
if (!$dati_cat_pers['num']) $d_tariffesett_vett['p'][$num1] = (float) $d_tariffesettp[($num1 - $d_id_data_inizio)] / (float) $d_num_persone;
else $d_tariffesett_vett['p'][$num1] = round(((float) $d_tariffesettp[($num1 - $d_id_data_inizio)] / (float) $d_coeff_cat_persone),2);
...
if (!$dati_cat_pers['num']) $costo_tariffap_settimana = (float) $costo_tariffap_settimana * (float) $numpersone;
else $costo_tariffap_settimana = (round(((float) $costo_tariffap_settimana * $coeff_cat_persone['p']) / $cat_persone['arrotond']) * $cat_persone['arrotond']) + ((float) $costo_tariffap_settimana * $coeff_cat_persone['i']);
...
for ($num1 = $d_id_data_inizio; $num1 <= $d_id_data_fine; $num1++) {
if ($priv_mod_costi_agg != "s") ${$molt_costoaggiuntivo."_sett".$num1} = fixset($dati_cap[$numca]['moltiplica_costo_sett'][$num1]);
...
if (!isset(${$molt_costoaggiuntivo."_sett".$num1}) or ${$molt_costoaggiuntivo."_sett".$num1} == "" or $num1 < $d_id_data_inizio or $num1 > $d_id_data_fine) ${$molt_costoaggiuntivo."_sett".$num1} = $moltiplica_default;
...
if ($letto_agg0 != "s" or !$dati_cat_pers['num'] or !isset($catpers_ca0)) $catpers_ca0 = "";
elseif (!strcmp(fixstr($dati_cat_pers[$catpers_ca0]['n_plur']),"")) $catpers_ca0 = 0;
...
for ($num1 = $inizioperiodo; $num1 <= $fineperiodo; $num1++) $num_letti_agg[$num1] = fixset($num_letti_agg[$num1]) + $nummoltiplica_cf0;
...
if (isset($dati_ca[$num1]["tipo_associa_".$tipotariffa]) and $dati_ca[$num1]["tipo_associa_".$tipotariffa] == "s") $associa_costo_sempre = "SI";
if (isset($dati_ca[$num1]["tipo_associa_".$tipotariffa]) and $dati_ca[$num1]["tipo_associa_".$tipotariffa] == "r") {
...
} # fine if (isset($dati_ca[$num1]["tipo_associa_".$tipotariffa]) and $dati_ca[$num1]["tipo_associa_".$tipotariffa] == "r")
...
if (!empty($dati_ca[$num_costo]['tipo_beniinv'])) $num_ripetizioni_costo[$num_reg] = $nrc;
...
$n_prezzo_costo_agg[$numca] = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$inizioperiodo,$fineperiodo,$n_settimane_costo[$numca],$n_moltiplica_costo[$numca],$costo_tariffa,$tariffesettimanali,1,$caparra,$numpersone,$dati_cat_pers_corr,$catpers_ca_corr);
...
$n_prezzo_costo_agg[$num_reg] = (float) calcola_prezzo_totale_costo($dati_ca,$num_costo,$inizioperiodo,$fineperiodo,$n_settimane_costo[$num_reg],$n_moltiplica_costo[$num_reg],$costo_tariffa,$tariffesettimanali,1,$caparra,$numpersone,$dati_cat_pers,fixset(${"catpers_ca$numca"}));
...
if ($dati_ca[$num_costo]['var_tariffea'] == "s") $vartariffeassociate_ca = fixset($dati_ca[$num_costo]["tipo_associa_".$tipotariffa]).fixset($dati_ca[$num_costo][$tipotariffa]);
...
$prezzo_totale_imposto = (float) $n_sconto;
$n_sconto = (float) $costo_totale_provvisorio - (float) $n_sconto;
} # fine if ($tipo_val_sconto == "tot")
if ($tipo_val_sconto == "tot_sett") {
$prezzo_totale_imposto = (float) $n_sconto * (float) $lunghezza_periodo;
$n_sconto = (float) $costo_totale_provvisorio - ((float) $n_sconto * (float) $lunghezza_periodo);
} # fine if ($tipo_val_sconto == "tot")
if ($tipo_val_sconto == "tar") $n_sconto = (float) $costo_tariffa - (float) $n_sconto;
if ($tipo_val_sconto == "tar_sett") $n_sconto = (float) $costo_tariffa - ((float) $n_sconto * (float) $lunghezza_periodo);
if ($tipo_val_sconto == "perc_tot" or $tipo_val_sconto == "perc_tar") {
if ($tipo_val_sconto == "perc_tot") $n_sconto = ($costo_totale_provvisorio * (float) $n_sconto) / 100;
if ($tipo_val_sconto == "perc_tar") $n_sconto = ($costo_tariffa * (float) $n_sconto) / 100;
$n_sconto = $n_sconto / (float) $arrotond_predef;
...
echo mex("Lo <div style=\"display: inline; color: red;\">sconto</div> inserito",$pag)." (<b>$n_sconto</b>) ".mex("supera il costo totale senza sconto",$pag)." (<b>".punti_in_num($costo_totale_provvisorio)."</b>)$per_la_prenotazione.<br>";
$inserire = "NO";
...
$n_prezzo_costo_agg[$numca] = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$inizioperiodo,$fineperiodo,$n_settimane_costo[$numca],$n_moltiplica_costo[$numca],$costo_tariffa,$tariffesettimanali,$costo_totale_provvisorio,$caparra,$numpersone,"","",$costo_escludi_perc);
...
$num_reg = ($dati_cap['num'] + $numca - 1);
$costo_totale_provvisorio = $n_costo_tot + $n_costo_agg_tot - $d_costo_agg_tot;
$n_prezzo_costo_agg[$num_reg] = (float) calcola_prezzo_totale_costo($dati_ca,$num_costo,$inizioperiodo,$fineperiodo,$n_settimane_costo[$num_reg],$n_moltiplica_costo[$num_reg],$costo_tariffa,$tariffesettimanali,$costo_totale_provvisorio,$caparra,$numpersone,"","",$costo_escludi_perc);
$mess_costi .= comunica_aggiunta_costo($dati_ca,$num_costo,$n_prezzo_costo_agg[$num_reg],$stile_soldi,$pag,$Euro,$associasett_ca_ricorda[$num_reg],$n_moltiplica_costo[$num_reg],$settimane_costo_ricorda[$num_reg],$per_la_prenotazione,"1",fixset(${"catpers_ca$numca"}));
$n_costo_agg_tot = $n_costo_agg_tot + $n_prezzo_costo_agg[$num_reg];
if ($dati_ca[$num_costo]['escludi_tot_perc'] == "s") $costo_escludi_perc = $costo_escludi_perc + $n_prezzo_costo_agg[$num_reg];
...
$n_sconto = $n_sconto + (float) $passo;
$sconto = $n_sconto;
$n_costo_tot = $n_costo_tot - (float) $passo;
...
$commissioni_arrotond = fixset($dati_tariffe[$tipotariffa]['commissioni_arrotond']['def']);
...
if ($tipo_val_commissioni == "perc_ts") $costo_base = $costo_base - (float) $sconto;
if ($tipo_val_commissioni == "perc_tsc") $costo_base = $costo_base - (float) $sconto + (float) $n_costo_agg_tot;
$n_commissioni = ($costo_base * (float) $n_commissioni) / 100;
...
if (strcmp((string) $valuta_caparra,"") and (float) $n_pagato <= (float) $caparra and $diff_pagato > 0) $valuta_aggiorna_pagato = $valuta_caparra;
...
if ($valuta_caparra == $valuta_aggiorna_pagato and (float) $n_pagato <= (float) $caparra and $diff_pagato > 0) {
$cambio_aggiorna_pagato = $cambio_caparra;
$arrotond_aggiorna_pagato = $arrotond_caparra;
} # fine if ($valuta_caparra == $valuta_aggiorna_pagato and (float) $n_pagato <= (float) $caparra and $diff_pagato > 0)
...
if ($altre_valute[$num1]['nome'] == $valuta_aggiorna_pagato and (float) $altre_valute[$num1]['cambio'] == (float) $cambio_aggiorna_pagato and (float) $altre_valute[$num1]['arrotond'] == (float) $arrotond_aggiorna_pagato) {
$sel = " selected";
$valuta_trovata = 1;
} # fine if ($altre_valute[$num1]['nome'] == $valuta_aggiorna_pagato and (float) $altre_valute[$num1]['cambio'] == (float) $cambio_aggiorna_pagato and...
else $sel = "";
...
$diff_costo_tot = (float) $n_costo_tot - (float) $d_costo_tot;
...
if (strcmp((string) $valuta_tariffa,"")) {
$n_costo_valuta = (float) $n_costo_tot * (float) $cambio_tariffa;
$n_costo_valuta = $n_costo_valuta / (float) $arrotond_tariffa;
...
$dati_extra = array('prenota_interconn' => $prenota_interconn);
if ($prenota_interconn) {
$dati_extra['lista_interconn'] = $lista_interconn;
$dati_extra['lista_utenti_mess_interconn'] = $lista_utenti_mess_interconn;
} # fine ($prenota_interconn)
...
# cat_persone può crescere troppo per entrare nel database, per questo viene compresso
if (function_exists('gzcompress')) $cat_persone = aggslashdb(base64_encode(gzcompress(serialize($cat_persone_idpr))));
else $cat_persone = aggslashdb(serialize($cat_persone_idpr));
...
$n_pagato = "";
$n_nometipotariffa = "";
unset($_REQUEST['n_pagato']);
unset($_REQUEST['n_confermato']);
} # fine if ($inserire != "NO")
...
} # fine else if ((!empty($cancella) and $priv_canc_prenotazioni != "n" or (!empty($ripristina) and...
...
${"cognome2_".$num_ripeti} = risul_query($dati_cliente,0,'cognome2');
...
for ($num1 = 0 ; $num1 < 4 ; $num1++) $cod_prenota .= substr($valori,random_int(0,22),1);
} # fine if (function_exists('random_int')) 
else {
list($usec, $sec) = explode(' ', microtime());
mt_srand($sec + (int) ((float) $usec * 1000000.0));
for ($num1 = 0 ; $num1 < 4 ; $num1++) $cod_prenota .= substr($valori,mt_rand(0,22),1);
...
$tasse_tot = (float) 0;
$mostra_conferma = "";
if ($modifica_clienti == "SI" and $vedi_clienti == "SI") $mostra_bott_mod_osp = 2;
else {
if ($modifica_clienti != "NO" and $vedi_clienti != "NO") $mostra_bott_mod_osp = 1;
else $mostra_bott_mod_osp = 0;
} # fine else if ($modifica_clienti != "NO" and $vedi_clienti != "NO")
...
$d_id_data_inizio = risul_query($dati_prenota_modifica,0,'iddatainizio');
$d_data_inizio = esegui_query("select * from $tableperiodi where idperiodi = '$d_id_data_inizio'");
if (numlin_query($d_data_inizio)) {
$d_data_inizio = risul_query($d_data_inizio,0,'datainizio');
$d_data_inizio_f = formatta_data($d_data_inizio,$stile_data);
} # fine if (numlin_query($d_data_inizio))
else $d_id_data_inizio = 0;
...
if (numlin_query($d_data_fine)) $d_data_fine = risul_query($d_data_fine,0,'datafine');
else $d_data_fine = "";
...
$d_costo_tariffa_vett[$id_prenota] = (float) fixset($d_tariffa[1]);
$d_costo_tariffa = $d_costo_tariffa_vett[$id_prenota];
$d_sconto_vett[$id_prenota] = (float) risul_query($dati_prenota_modifica,0,'sconto');
$d_sconto = $d_sconto_vett[$id_prenota];
$d_tasse_perc = (float) risul_query($dati_prenota_modifica,0,'tasseperc');
...
$id_prenota = $id_prenota_idpr[$num_idpr];
$dati_prenota_modifica = esegui_query("select * from $tableprenota where idprenota = '$id_prenota' and idclienti != '0'");
if ($id_clienti != risul_query($dati_prenota_modifica,0,'idclienti')) {
$id_clienti = "~~~~";
if ($mostra_bott_mod_osp == 1) {
$dati_cliente = esegui_query("select utente_inserimento from $tableclienti where idclienti = '".risul_query($dati_prenota_modifica,0,'idclienti')."' ");
$utente_inserimento = risul_query($dati_cliente,0,'utente_inserimento');
if (($vedi_clienti == "PROPRI" or $modifica_clienti == "PROPRI") and $utente_inserimento != $id_utente) $mostra_bott_mod_osp = 0;
if (($vedi_clienti == "GRUPPI" or $modifica_clienti == "GRUPPI") and empty($utenti_gruppi[$utente_inserimento])) $mostra_bott_mod_osp = 0;
} # fine if ($mostra_bott_mod_osp == 1)
} # fine if ($id_clienti != risul_query($dati_prenota_modifica,0,'idclienti'))
${"dati_cap".$id_prenota} = dati_costi_agg_prenota($tablecostiprenota,$id_prenota,$dati_cat_pers);
$dati_cap = ${"dati_cap".$id_prenota};
...
$d_costo_tariffa_vett[$id_prenota] = (float) fixset($d_tariffa[1]);
if ($d_costo_tariffa != $d_costo_tariffa_vett[$id_prenota]) $d_costo_tariffa = "~~~~";
$d_sconto_vett[$id_prenota] = (float) risul_query($dati_prenota_modifica,0,'sconto');
...
if (fixset($d_cat_persone['excat'][$cat_pers]['m']) != $d_cat_persone_vett[$id_prenota][$num1]['molt']) $d_cat_persone['excat'][$cat_pers]['m'] = "~~";
if (!strstr(fixstr($d_cat_persone['excat'][$cat_pers]['n']).", ",$d_cat_persone_vett[$id_prenota][$num1]['n_plur'].", ")) {
if (!empty($d_cat_persone['excat'][$cat_pers]['n'])) $d_cat_persone['excat'][$cat_pers]['n'] .= ", ";
...
$mostra_cliente = "NO";
if ($mostra_bott_mod_osp == 1) {
$dati_cliente = esegui_query("select utente_inserimento from $tableclienti where idclienti = '$id_clienti' ");
$utente_inserimento = risul_query($dati_cliente,0,'utente_inserimento');
} # fine if ($mostra_bott_mod_osp == 1)
} # fine if ($id_clienti == "~~~~")
else {
...
if ($modifica_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento])) $mostra = "NO";
...
else $dati_ospite = esegui_query("select cognome,cognome2,nome,sesso,datanascita,utente_inserimento from $tableclienti where idclienti = '$id_clienti_osp' ");
$utente_ospite = risul_query($dati_ospite,0,'utente_inserimento');
if (($modifica_clienti == "PROPRI" and $utente_ospite != $id_utente) or ($modifica_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_ospite]))) $dati_osp .= ($num1 + 1).". ";
else $dati_osp .=  "<a href=\"$url_mod_cli\">".($num1 + 1).".<\/a> ";
if (($vedi_clienti == "PROPRI" and $utente_ospite != $id_utente) or ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_ospite]))) $dati_osp .= mex("Cliente",$pag)." <em>$id_clienti_osp</em>";
else {
$dati_osp .= "<em>".addslashes(risul_query($dati_ospite,0,'cognome'))."<\/em> ";
$cccognome2 = addslashes((string) risul_query($dati_ospite,0,'cognome2'));
...
$O = "o";
if ($ccsesso == "f") $O = "a";
if ($cccognome2) $dati_osp .=  "<em>$cccognome2<\/em> ";
...
if ($priv_mod_utente_ins == "s" or ($priv_mod_utente_ins == "g" and !empty($utenti_gruppi[$idutenti]))) {
...
} # fine if ($priv_mod_utente_ins == "s" or ($priv_mod_utente_ins == "g" and !empty($utenti_gruppi[$idutenti])))
...
if ($priv_mod_date == "s" and !$id_prenota_prec) {
...
echo "</span> ";
} # fine if ($priv_mod_date == "s" and !$id_prenota_prec)
...
if ($id_prenota_prec) $standalone = 1;
else $standalone = "";
mostra_menu_date(C_DATI_PATH."/selperiodimenu$anno.$id_utente.php","n_fineperiodo",$n_fineperiodo,1,$blank_selected,$id_utente,$tema,$standalone,$d_data_fine);
echo "</span> ";
} # fine if ($priv_mod_date == "s")
...
echo "</td><td>";
if ($priv_mod_checkin == "s" and !$id_prenota_prec) {
echo mex("cambia in",$pag)." ";
if ($d_data_inizio != "~~~~") {
echo "<select name=\"n_data_stima_checkin\">";
$formato_vedi = "d-m-Y";
if ($stile_data == "usa") $formato_vedi = "m-d-Y";
$anno_ini = (int) substr($d_data_inizio,0,4);
$mese_ini = (int) substr($d_data_inizio,5,2);
$giorno_ini = (int) substr($d_data_inizio,8,2);
...
echo "</select>";
} # fine if ($priv_mod_checkin == "s" and !$id_prenota_prec)
echo "</td></tr>";
...
if (strcmp((string) $valuta_tar,"") and ((float) $altre_valute[$valuta_tar]['cambio'] != (float) $d_cambio_tariffa or (float) $altre_valute[$valuta_tar]['arrotond'] != (float) $d_arrotond_tariffa)) {
$tariffa_valuta_attuale = converti_valuta($d_costo_tariffa,$altre_valute[$valuta_tar]['cambio'],$altre_valute[$valuta_tar]['arrotond']);
if ($tariffa_valuta_attuale and (float) $tariffa_valuta_attuale != (float) $tariffa_valuta) {
...
} # fine if ($tariffa_valuta_attuale and (float) $tariffa_valuta_attuale != (float) $tariffa_valuta)
} # fine if (strcmp((string) $valuta_tar,"") and ((float) $altre_valute[$valuta_tar]['cambio'] != (float) $d_cambio_tariffa or...
...
if (isset(${"cambia_con_regola2".$suff_idpr}) and ${"cambia_con_regola2".$suff_idpr} == "S") $checked = " checked";
else $checked = "";
...
if (strcmp((string) $valuta_sco,"") and ((float) $altre_valute[$valuta_sco]['cambio'] != (float) $d_cambio_tariffa or (float) $altre_valute[$valuta_sco]['arrotond'] != (float) $d_arrotond_tariffa)) {
$sconto_valuta_attuale = converti_valuta($d_sconto,$altre_valute[$valuta_sco]['cambio'],$altre_valute[$valuta_sco]['arrotond']);
if ($sconto_valuta_attuale and (float) $sconto_valuta_attuale != (float) $sconto_valuta) {
...
} # fine if ($sconto_valuta_attuale and (float) $sconto_valuta_attuale != (float) $sconto_valuta)
} # fine if (strcmp((string) $valuta_sco,"") and ((float) $altre_valute[$valuta_sco]['cambio'] != (float) $d_cambio_tariffa or...
...
if (strcmp((string) $valuta_cap,"") and ((float) $altre_valute[$valuta_cap]['cambio'] != (float) $d_cambio_caparra or (float) $altre_valute[$valuta_cap]['arrotond'] != (float) $d_arrotond_caparra)) {
$caparra_valuta_attuale = converti_valuta($d_caparra,$altre_valute[$valuta_cap]['cambio'],$altre_valute[$valuta_cap]['arrotond']);
if ($caparra_valuta_attuale and (float) $caparra_valuta_attuale != (float) $caparra_valuta) {
...
} # fine if ($caparra_valuta_attuale and (float) $caparra_valuta_attuale != (float) $caparra_valuta)
} # fine if (strcmp((string) $valuta_cap,"") and ((float) $altre_valute[$valuta_cap]['cambio'] != (float) $d_cambio_caparra or...
...
$tasse_ca = (float) $tasse_ca / $dati_tariffe['tasse_arrotond'];
$tasse_ca = round($tasse_ca);
$tasse_ca = $tasse_ca * (float) $dati_tariffe['tasse_arrotond'];
} # fine else if ($dati_cap[$numca]['tasseperc'] == -1)
$tasse_tot = (float) $tasse_tot + $tasse_ca;
...
if (isset($$numcostoaggiuntivo) and $$numcostoaggiuntivo == "SI") $checked = " checked";
else $checked = "";
...
if (!isset($$molt_costoaggiuntivo) or $$molt_costoaggiuntivo != "1" or $dati_cap[$numca]['associasett'] == "s") $frase_cat_pers .= $dati_cap[$numca]['cat_pers'][0]['n_plur'];
...
if (!isset($$molt_costoaggiuntivo) or $$molt_costoaggiuntivo != "1" or $dati_cap[$numca]['associasett'] == "s") $frase_cat_pers .= " <em>".$dati_cap[$numca]['cat_pers'][0]['n_plur']."</em>";
...
echo " ".mex("per",$pag)." ";
if ($priv_mod_costi_agg == "s" and $dati_cap[$numca]['numsett'] == "c") echo "<input type=\"text\" name=\"$sett_costoaggiuntivo\" size=\"3\" maxlength=\"3\" value=\"".htmlspecialchars(fixstr($$sett_costoaggiuntivo))."\">";
...
if ($priv_mod_costi_agg == "s" and $dati_cap[$numca]['moltiplica'] == "c") echo "<input type=\"text\" name=\"$molt_costoaggiuntivo\" size=\"3\" maxlength=\"12\" value=\"".htmlspecialchars(fixstr($$molt_costoaggiuntivo))."\">";
...
if (($priv_mod_costi_agg == "s" and $dati_cap[$numca]['moltiplica'] == "c") or (isset($dati_cap[$numca]['moltiplica_costo_sett'][$num1]) and $dati_cap[$numca]['moltiplica_costo_sett'][$num1] > 1)) echo " x ";
...
elseif (isset($dati_cap[$numca]['moltiplica_costo_sett'][$num1]) and $dati_cap[$numca]['moltiplica_costo_sett'][$num1] > 1) echo $dati_cap[$numca]['moltiplica_costo_sett'][$num1];
echo "</small></td>";
...
if ((float) $costo_tot_valuta != (float) $costo_tot_valuta_corr) {
...
} # fine if ((float) $costo_tot_valuta != (float) $costo_tot_valuta_corr)
...
$resto_tarsco_valuta = round(((float) $d_costo_tariffa - (float) $d_sconto  - (float) $d_pagato),2);
...
$dapagare_cosagg_valuta = round(((float) $costo_agg_tot + (float) $resto_tarsco_valuta),2);
...
if ($da_pagare_valuta_corr and (float) $da_pagare_valuta != (float) $da_pagare_valuta_corr) echo " <small>(".mex("Al cambio attuale",$pag).": <em>".virgola_in_num($da_pagare_valuta_corr,$stile_soldi)." $d_valu_tariffa)</small></em>";
...
<input type=\"text\" name=\"$numsettimane\" size=\"3\" maxlength=\"3\" value=\"".htmlspecialchars(fixstr($$numsettimane))."\"
...
if ($pcanc) $frase_ripristina_prenota = mex("Ripristina la prenotazione",$pag)." $id_prenota";
...
if ($pcanc) $frase_ripristina_prenota = mex("Ripristina le prenotazioni",$pag);
$frase_copia_prenota = mex("Inserisci prenotazioni uguali",$pag);
$cr_class = "cress";
} # fine else if ($num_id_prenota == 1)

$dcognome = str_replace("\"","&quot;",fixstr($dcognome));
$dcognome2 = str_replace("\"","&quot;",fixstr($dcognome2));
$dcitta = str_replace("\"","&quot;",fixstr($dcitta));
$dnazione = str_replace("\"","&quot;",fixstr($dnazione));
$dnome = str_replace("\"","&quot;",fixstr($dnome));
$dsoprannome = str_replace("\"","&quot;",fixstr($dsoprannome));
$dvia = str_replace("\"","&quot;",fixstr($dvia));
$select_costo_stampa = "";
...
<button class=\"ires\" type=\"submit\"><div>$frase_ripristina_prenota</div></button>
</div></form></td>
...
<input type=\"hidden\" name=\"cognome2_1\" value=\"$dcognome2\">
<input type=\"hidden\" name=\"nome_1\" value=\"$dnome\">
<input type=\"hidden\" name=\"soprannome_1\" value=\"$dsoprannome\">
<input type=\"hidden\" name=\"titolo_1\" value=\"".htmlspecialchars(fixstr($dtitolo_cli))."\">
<input type=\"hidden\" name=\"sesso_1\" value=\"".htmlspecialchars(fixstr($dsesso))."\">
<input type=\"hidden\" name=\"data_nascita_1\" value=\"".htmlspecialchars(fixstr($ddatanascita))."\">
<input type=\"hidden\" name=\"documento_1\" value=\"".htmlspecialchars(fixstr($ddocumento))."\">
<input type=\"hidden\" name=\"tipo_documento_1\" value=\"".htmlspecialchars(fixstr($dtipodoc))."\">
<input type=\"hidden\" name=\"citta_documento_1\" value=\"".htmlspecialchars(fixstr($dcittadoc))."\">
<input type=\"hidden\" name=\"regione_documento_1\" value=\"".htmlspecialchars(fixstr($dregionedoc))."\">
<input type=\"hidden\" name=\"nazione_documento_1\" value=\"".htmlspecialchars(fixstr($dnazionedoc))."\">
<input type=\"hidden\" name=\"scadenza_documento_1\" value=\"".htmlspecialchars(fixstr($dscadenzadoc))."\">
<input type=\"hidden\" name=\"citta_nascita_1\" value=\"".htmlspecialchars(fixstr($dcittanascita))."\">
<input type=\"hidden\" name=\"regione_nascita_1\" value=\"".htmlspecialchars(fixstr($dregionenascita))."\">
<input type=\"hidden\" name=\"nazione_nascita_1\" value=\"".htmlspecialchars(fixstr($dnazionenascita))."\">
<input type=\"hidden\" name=\"cittadinanza_1\" value=\"".htmlspecialchars(fixstr($dnazionalita))."\">
<input type=\"hidden\" name=\"codice_lingua_1\" value=\"".htmlspecialchars(fixstr($dlingua_cli))."\">
<input type=\"hidden\" name=\"nazione_1\" value=\"$dnazione\">
<input type=\"hidden\" name=\"regione_1\" value=\"".htmlspecialchars(fixstr($dregione))."\">
<input type=\"hidden\" name=\"citta_1\" value=\"$dcitta\">
<input type=\"hidden\" name=\"via_1\" value=\"$dvia\">
<input type=\"hidden\" name=\"numcivico_1\" value=\"".htmlspecialchars(fixstr($dnumcivico))."\">
<input type=\"hidden\" name=\"telefono_1\" value=\"".htmlspecialchars(fixstr($dtelefono))."\">
<input type=\"hidden\" name=\"telefono2_1\" value=\"".htmlspecialchars(fixstr($dtelefono2))."\">
<input type=\"hidden\" name=\"telefono3_1\" value=\"".htmlspecialchars(fixstr($dtelefono3))."\">
<input type=\"hidden\" name=\"codice_fiscale_1\" value=\"".htmlspecialchars(fixstr($dcod_fiscale))."\">
<input type=\"hidden\" name=\"partita_iva_1\" value=\"".htmlspecialchars(fixstr($dpartita_iva))."\">
<input type=\"hidden\" name=\"fax_1\" value=\"".htmlspecialchars(fixstr($dfax))."\">
<input type=\"hidden\" name=\"cap_1\" value=\"".htmlspecialchars(fixstr($dcap))."\">
<input type=\"hidden\" name=\"email_1\" value=\"".htmlspecialchars(fixstr($demail))."\">
<input type=\"hidden\" name=\"email2_1\" value=\"".htmlspecialchars(fixstr($demail2))."\">
<input type=\"hidden\" name=\"email_certificata_1\" value=\"".htmlspecialchars(fixstr($demail_cert))."\">
...
$contr_escludi = array();
$escludi_pag = esegui_query("select numero from $tablecontratti where tipo = 'pag_esc' and testo $LIKE '%,mod_prenota,%' ");
