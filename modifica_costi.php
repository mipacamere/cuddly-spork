...
$var_pag[30] = 'mantenere_comb';
$var_pag[31] = 'mantenere_beniinv';
...
$var_pag[63] = 'modo_catpers_molt';
$var_pag[64] = 'mantenere_percentuale';
$var_pag[65] = 'mag_bie_sel';
$var_pag[66] = 'assegna_da_giorno_ini';
$var_pag[67] = 'assegna_prenota_contemp';
$var_pag[68] = 'combina_ca';
$var_pag[69] = 'n_catpers_passa';
$var_pag[70] = 'nc_catpers_passa';
$n_var_pag = 71;
$num2 = 0;
...
$num2 = 0;
if (isset($_POST['num_beni_inv_elimina_sel'])) $num2 = (int) $_POST['num_beni_inv_elimina_sel'];
elseif (isset($_GET['num_beni_inv_elimina_sel'])) $num2 = (int) $_GET['num_beni_inv_elimina_sel'];
for ($num1 = 1 ; $num1 <= $num2 ; $num1++) {
$var_pag[$n_var_pag++] = "bene_inv_sel$num1";
$var_pag[$n_var_pag++] = "molt_bene_inv_sel$num1";
} # fine for $num1
...
if (isset($_REQUEST['nc_catpers_passa'])) $num2 = (int) $_REQUEST['nc_catpers_passa'];
else $num2 = 0;
for ($num1 = 0 ; $num1 < $num2 ; $num1++) $var_pag[$n_var_pag++] = "c_catpers$num1";
...
if (!isset($idntariffe)) $idntariffe = "";
$num_costo = fixstr($dati_ca['id'][$idntariffe]);
if ($attiva_costi_agg_consentiti != "n" and (!isset($costi_agg_consentiti_vett[$idntariffe]) or $costi_agg_consentiti_vett[$idntariffe] != "SI")) $num_costo = "";
if ($num_costo != "") {
...
if ($n_nome != "") $nomecostoagg = htmlspecialchars(fixstr($n_nome),ENT_COMPAT);
...
if (($tasseperc_ca and controlla_soldi($tasseperc_ca) == "NO") or $tasseperc_ca > 100 or (float) $tasseperc_ca < 0) $errore = "SI";
...
if (!empty($bene_gia_sel[${"bene_inv_sel".$num1}])) $errore = "SI";
...
if ($beni_inv_elimina != "nessuno" and $tipo_bie == "m" and empty($mag_bie_sel)) $errore = "SI";
...
if (str_replace("\\\"".fixset(${"pp_dal".$num1})."\\\">","",$file_date_int) == $file_date_int) { $errore = "SI"; $conflitti .= "13;"; }
if (str_replace("\\\"".fixset(${"pp_al".$num1})."\\\">","",$file_date_int) == $file_date_int) { $errore = "SI"; $conflitti .= "13;"; }
$nuova_ultima_data = str_replace("-","",fixstr(${"pp_dal".$num1}));
if ($nuova_ultima_data <= $ultima_data) { $errore = "SI"; $conflitti .= "13;"; }
else $ultima_data = $nuova_ultima_data;
$nuova_ultima_data = str_replace("-","",fixstr(${"pp_al".$num1}));
if ($nuova_ultima_data <= $ultima_data) { $errore = "SI"; $conflitti .= "13;"; }
else $ultima_data = $nuova_ultima_data;
} # fine for $num1
} # fine else if (controlla_num_pos($num_periodi_permessi_sel) == "NO" or...
} # fine if ($periodi_permessi == "sel")
if (!isset($tipo_pp) or ($tipo_pp != "t" and $tipo_pp != "u" and $tipo_pp != "p")) $errore = "SI";
elseif ($tipo_ca != "s" and $tipo_pp == "p") $errore = "SI";
...
$arrotonda_ca = formatta_soldi($arrotonda_ca);
if ($tipo_valore_ca != "f" and $arrotonda_ca != fixset($dati_ca[$num_costo]['arrotonda'])) {
esegui_query("update $tablenometariffe set arrotonda_ca = '$arrotonda_ca' where idntariffe = '$idntariffe'");
$modificato = "SI";
} # fine if ($tipo_valore_ca != "f" and $arrotonda_ca != fixset($dati_ca[$num_costo]['arrotonda']))
...
$dati_ca[$num_costo]['cp_molt']['num'] = (int) fixset($dati_ca[$num_costo]['cp_molt']['num']);
...
echo ".<br><br>
<input type=\"hidden\" name=\"n_catpers_passa\" value=\"$fine_for_cp\">
<input type=\"hidden\" name=\"nc_catpers_passa\" value=\"".$dati_ca[$num_costo]['cp_molt']['num']."\">";
...
if ($opt_mag) {
$mag_bie_sel = fixset($dati_ca[$num_costo]['mag_beniinv']);
