...
$var_pag[118] = 'canc_clienti_pcanc';
$n_var_pag = 119;
$num2 = 0;
...
if (!isset($opz_cerc_pren) or ($opz_cerc_pren != "arr" and $opz_cerc_pren != "part" and $opz_cerc_pren != "piucanc" and $opz_cerc_pren != "canc")) $opz_cerc_pren = "";
...
if ($priv_mod_prenotazioni == "g" and empty($utenti_gruppi[$utente_inserimento])) $continua = "NO";
...
$cognome = esegui_query("select cognome,cognome2,utente_inserimento from $tableclienti where idclienti = '$id_clienti' ");
$mostra_cliente = "SI";
...
if ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento])) $mostra_cliente = "NO";
} # fine if ($vedi_clienti == "PROPRI" or...
if ($mostra_cliente == "NO") $cognome = mex("cliente",$pag)." $id_clienti";
else {
$cognome2 = (string) risul_query($cognome,0,'cognome');
...
if ($differenza != 0) {
if ($altre_valute['num'] or (strcmp((string) $valuta_caparra,"") and (float) $pagato_dopo <= (float) $caparra and $differenza > 0)) {
...
$diff_pagato_valuta = (float) $differenza * (float) $altre_valute[$num2]['cambio'];
$diff_pagato_valuta = $diff_pagato_valuta / (float) $altre_valute[$num2]['arrotond'];
...
if ($altre_valute[$num2]['nome'] == $valuta_caparra and (float) $altre_valute[$num2]['cambio'] == (float) $cambio_caparra and (float) $altre_valute[$num2]['arrotond'] == (float) $arrotond_caparra and $caparra > $pagato) {
$sel = " selected";
$valuta_trovata = 1;
} # fine if ($altre_valute[$num2]['nome'] == $valuta_caparra and (float) $altre_valute[$num2]['cambio'] == (float) $cambio_caparra and...
else $sel = "";
echo "<option value=\"".htmlspecialchars($altre_valute[$num2]['nome'])."\"$sel>$diff_pagato_valuta_p ".$altre_valute[$num2]['nome']."</option>";
} # fine for $num2
if (!$valuta_trovata and strcmp((string) $valuta_caparra,"") and (float) $pagato_dopo <= (float) $caparra and $differenza > 0) {
$diff_pagato_valuta = (float) $differenza * (float) $cambio_caparra;
$diff_pagato_valuta = $diff_pagato_valuta / (float) $arrotond_caparra;
$diff_pagato_valuta = round($diff_pagato_valuta);
$diff_pagato_valuta = $diff_pagato_valuta * (float) $arrotond_caparra;
$diff_pagato_valuta_p = punti_in_num($diff_pagato_valuta,$stile_soldi);
if (substr($diff_pagato_valuta_p,0,1) != "-") $diff_pagato_valuta_p = "+".$diff_pagato_valuta_p;
...
if ($priv_vedi_tab_prenotazioni == "g" and empty($utenti_gruppi[$utente_inserimento])) $modifica = "NO";
if ($priv_mod_prenotazioni == "n") $modifica = "NO";
if ($priv_mod_prenotazioni == "p" and $utente_inserimento != $id_utente) $modifica = "NO";
if ($priv_mod_prenotazioni == "g" and empty($utenti_gruppi[$utente_inserimento])) $modifica = "NO";
$id_inizio_prenota = risul_query($prenotazione,0,'iddatainizio');
...
if (${"valuta_pagamento".$numero} != $d_valuta_caparra[0] or (float) $d_pagato_dopo > (float) $caparra or $d_pagato_dopo < $pagato) $inserire = "NO";
else {
...
$confermato = (string) risul_query($prenotazione,0,'conferma');
...
$saldo_valuta = (float) $saldo * (float) $cambio_pagamento[$numero];
$saldo_valuta = $saldo_valuta / (float) $arrotond_pagamento[$numero];
...
${"metodo_pagamento".$numero} = htmlspecialchars(fixstr(${"metodo_pagamento".$numero}),ENT_COMPAT);
if (isset($met_paga_perm[${"metodo_pagamento".$numero}]) and $met_paga_perm[${"metodo_pagamento".$numero}] == "SI") $metodo_pagamento = aggslashdb(${"metodo_pagamento".$numero});
...
if (!empty(${"note_pagamento".$numero})) esegui_query("update $tablesoldi set note = '".aggslashdb(htmlspecialchars(${"note_pagamento".$numero},ENT_COMPAT))."' where idsoldi = '$idsoldi' ");
...
$costo0 = (float) $costo0 + (float) $saldo;
...
$cognome_saldo = esegui_query("select cognome,cognome2 from $tableclienti where idclienti = '$idcliente_saldo' ");
$cognome2_saldo = (string) risul_query($cognome_saldo,0,'cognome2');
$cognome_saldo = risul_query($cognome_saldo,0,'cognome');
if (strcmp($cognome2_saldo,"")) $cognome_saldo .= " $cognome2_saldo";
...
if (!empty(${"note_pagamento".$numero})) $nome_costo .= " - ".htmlspecialchars(${"note_pagamento".$numero},ENT_COMPAT);
...
if ($id_utente != 1 and empty($utenti_gruppi[$id_utente_vedi])) $id_utente_vedi = $id_utente;
...
if ($num_periodi_date > 1 or fixset($d_increment[0]) > 1) {
...
} # fine if ($num_periodi_date > 1 or fixset($d_increment[0]) > 1)
else {
$cerca_ini = $oggi_ini;
...
<option value=\"piucanc\">".mex("Anche cancellate",$pag)."</option>
<option value=\"canc\">".mex("Solo cancellate",$pag)."</option>
</select> ".mex("dal",$pag)." ";
$oggi = date("Y-m-d",(time() + (C_DIFF_ORE * 3600)));
...
$tabelle_lock = "";
if ((!empty($subtotale_selezionate) or $opz_cerc_pren == "piucanc") and !$pcanc) $altre_tab_lock = array($tableprenota,$tableprenotacanc,$tableperiodi,$tableappartamenti,$tablecontratti,$tablepersonalizza,$tableutenti);
else $altre_tab_lock = array($tableprenota,$tableperiodi,$tableappartamenti,$tablecontratti,$tablepersonalizza,$tableutenti);
$tabelle_lock = lock_tabelle($tabelle_lock,$altre_tab_lock);
...
if (isset($colonna_selezionata[3]) and $colonna_selezionata[3] == "nec") $colonne_necessarie['ca'][$num_ca_tab] = 1;
$num_ca_tab++;
...
$separatore_gr_tab[$num_gr_tab] = fixset($colonna_selezionata[2]);
...
if ($colonna_selezionata[(count($colonna_selezionata) - 1)] == "nec") $colonne_necessarie['gr'][$num_gr_tab] = 1;
$num_gr_tab++;
...
if ($opz_cerc_pren == "piucanc") {
$prenotazioni = esegui_query("(select * from $tableprenota where $iddatainizio < $c_idfineperiodo_q and $iddatafine >= $c_idinizioperiodo_q and idclienti != '0' $condizione_prenota_proprie)
 union
 (select * from $tableprenotacanc where $iddatainizio < $c_idfineperiodo_q and $iddatafine >= $c_idinizioperiodo_q and idclienti != '0' $condizione_prenota_proprie)
 order by $ordine_prenota");
} # fine if ($opz_cerc_pren == "piucanc")
else $prenotazioni = esegui_query("select * from $tableprenota where $iddatainizio < $c_idfineperiodo_q and $iddatafine >= $c_idinizioperiodo_q and idclienti != '0' $condizione_prenota_proprie order by $ordine_prenota");
...
$lista_date1 = explode("<option value=\"",fixstr($lista_date0[1]));
if (substr(fixstr($lista_date1[2]),0,10) == $cerca_fine) $num_fine = 2;
else $num_fine = 1;
...
if (substr(fixstr($lista_date1[$num_fine]),0,10) == $cerca_fine) {
...
else $n_cerca_fine = substr(fixstr($lista_date1[1]),0,10);
...
} # fine if (substr(fixstr($lista_date1[$num_fine]),0,10) == $cerca_fine)
...
if ($pcanc) $prenotazioni = esegui_query("select * from $tableprenota where idclienti != '0' $numeri_prenota_sel_query $condizione_prenota_proprie order by $ordine_prenota");
else {
$prenotazioni = esegui_query("(select * from $tableprenota where idclienti != '0' $numeri_prenota_sel_query $condizione_prenota_proprie)
 union
 (select * from $tableprenotacanc where idclienti != '0' $numeri_prenota_sel_query $condizione_prenota_proprie)
 order by $ordine_prenota");
} # fine else if ($pcanc)
$num_prenotazioni = numlin_query($prenotazioni);
/*if (!$num_prenotazioni and !$pcanc) {
$prenotazioni = esegui_query("select * from $tableprenotacanc where idclienti != '0' $numeri_prenota_sel_query $condizione_prenota_proprie order by $ordine_prenota");
$num_prenotazioni = numlin_query($prenotazioni);
if ($num_prenotazioni) {
$pcanc = "&amp;pcanc=1";
$tableprenota = $tableprenotacanc;
...
} # fine if (!$num_prenotazioni and !$pcanc)*/
...
if ($opz_cerc_pren == "piucanc") $frase_tab .=  " (".strtolower(mex("Anche cancellate",$pag)).")";
...
if (numlin_query($data_fine)) {
$data_fine = risul_query($data_fine,0,'datafine');
$data_fine_f = formatta_data($data_fine,$stile_data);
} # fine if (numlin_query($data_fine))
else {
$data_fine = esegui_query("select max(datafine) from $tableperiodi ");
$data_fine = risul_query($data_fine,0,0);
$data_fine_f = ">".formatta_data($data_fine,$stile_data);
} # fine if (numlin_query($data_fine))
...
} # fine if ($id_data_inizio_prec)
else $numero_settimane = ">".($id_data_fine_prec + 1);
$num1_orig = $num1;
$num1 = 0;
...
$numero_settimane = $id_data_fine - $id_data_inizio + 1;
...
$nome_tariffa = (string) risul_query($prenotazioni,$num1,'tariffa');
$nome_tariffa = explode("#@&",$nome_tariffa);
$costo_tariffa = (float) fixset($nome_tariffa[1]);
...
$sconto = (float) risul_query($prenotazioni,$num1,'sconto');
$confermato = (string) risul_query($prenotazioni,$num1,'conferma');
$confermato = substr($confermato,0,1);
$colore = "";
if ($pagato < $costo_tot) {
...
$costo_agg_parziale = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$id_data_inizio,$id_data_fine,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$costo_tariffa,$tariffesettimanali,($costo_tariffa + $costo_agg_tot - $sconto),$caparra,$numpersone,fixset($dati_cap[$numca]['cat_pers']),0,$costo_escludi_perc);
$costo_agg_tot = (float) $costo_agg_tot + $costo_agg_parziale;
if ($dati_cap[$numca]['escludi_tot_perc'] == "s") $costo_escludi_perc = (float) $costo_escludi_perc + $costo_agg_parziale;
if ($dati_cap[$numca]['tasseperc']) {
if ($dati_cap[$numca]['tasseperc'] == -1) $tasse = (float) $costo_agg_parziale;
else {
$tasse = ($costo_agg_parziale / (float) ($dati_cap[$numca]['tasseperc'] + 100)) * (float) $dati_cap[$numca]['tasseperc'];
$tasse = $tasse / $dati_tariffe['tasse_arrotond'];
$tasse = round($tasse);
$tasse = $tasse * (float) $dati_tariffe['tasse_arrotond'];
} # fine else if ($dati_cap[$numca]['tasseperc'] == -1)
$tasse_TOT = (float) $tasse_TOT + (float) $tasse;
...
$cat_pers_ex = $val;
reset($cat_pers_ex);
foreach ($cat_pers_ex as $cat_ex => $val2) {
if (!isset($cat_persone_TOT['ex'][$cat_ex])) $cat_persone_TOT['ex'][$cat_ex] = 0;
$cat_persone_TOT['ex'][$cat_ex] += $val2;
} # fine foreach ($cat_pers_ex as $cat_ex => $val2)
} # fine if ((string) $cat_corr == "ex")
else {
if (!isset($cat_persone_TOT[$cat_corr])) $cat_persone_TOT[$cat_corr] = 0;
$cat_persone_TOT[$cat_corr] += $val;
...
$pcanc_corr = $pcanc;
$classcanc_corr = $classcanc;
$stilebarrato_corr = $stilebarrato;
$commento = (string) risul_query($prenotazioni,$num1,'commento');
if ((!empty($subtotale_selezionate) or $opz_cerc_pren == "piucanc") and strstr($commento,">hd_del_res<")) {
$pcanc_corr = "&amp;pcanc=1";
$classcanc_corr = " class=\"bcoldeleted\"";
$stilebarrato_corr = " style=\"text-decoration: line-through\"";
} # fine if ((!empty($subtotale_selezionate) or $opz_cerc_pren == "piucanc") and...

$link_modifica = "SI";
$utente_inserimento = risul_query($prenotazioni,$num1,'utente_inserimento');
$nome_utente_inserimento = fixset($nome_utente_prenota[$utente_inserimento]);
...
if ($link_modifica == "SI") {
$link_modifica_inizio = "<a$stilebarrato_corr href=\"modifica_prenota.php?id_prenota=$numero$pcanc_corr&amp;anno=$anno&amp;id_sessione=$id_sessione&amp;origine=visualizza_tabelle.php\">";
...
if ($id_data_inizio == 0 and isset($num1_orig)) {
$num1 = $num1_orig;
$prenotazioni = $prenotazioni_orig;
} # fine ($id_data_inizio == 0 and isset($num1_orig))
...
$cognome2 = (string) risul_query($cognome,0,'cognome2');
$cognome = risul_query($cognome,0,'cognome');
if (strcmp($cognome2,"")) $cognome .= " $cognome2";
} # fine else if ($mostra_cliente == "NO")
...
$cognome_ospite = esegui_query("select cognome,cognome2,utente_inserimento from $tableclienti where idclienti = '$id_ospite' ");
$mostra_ospite = "SI";
...
if ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento])) $mostra_ospite = "NO";
} # fine if ($vedi_clienti == "PROPRI" or...
if ($mostra_ospite != "NO") {
$cognome2_ospite = (string) risul_query($cognome_ospite,0,'cognome2');
$cognome_ospite = risul_query($cognome_ospite,0,'cognome');
if (strcmp($cognome2_ospite,"")) $cognome_ospite .= " $cognome2_ospite";
...
if (!empty($colonne_necessarie)) $mostra_riga = "NO";
else $mostra_riga = "SI";
$colonne_costi = "";
...
if ($mostra_riga == "NO" and isset($colonne_necessarie['ca'][$num2])) $mostra_riga = "SI";
...
if (isset($col_gr_piano[$num2]) and $col_gr_piano[$num2] == "SI") { $colonne_costi .= $spazio.fixset($piano_app[$appartamento]); $spazio = $separatore; }
if (isset($col_gr_casa[$num2]) and $col_gr_casa[$num2] == "SI") { $colonne_costi .= $spazio.fixset($casa_app[$appartamento]); $spazio = $separatore; }
...
if (strcmp(fixstr($commento_pers[$nome_cp_col_gr[$num2][$num3]]),"")) {
$colonne_costi .= $spazio.$commento_pers[$nome_cp_col_gr[$num2][$num3]];
$spazio = $separatore;
} # fine if (strcmp(fixstr($commento_pers[$nome_cp_col_gr[$num2][$num3]]),""))
} # fine for $num3
...
elseif ($mostra_riga == "NO" and isset($colonne_necessarie['gr'][$num2])) $mostra_riga = "SI";
$colonne_costi .= "</td>";
} # fine for $num2
...
echo "<tr$classcanc_corr>";
if ($colonna_numero) echo "<td$stile_checkin$stilebarrato_corr>$link_modifica_inizio$numero$link_modifica_fine</td>";
$num_cambia++;
...
if ($colonna_tariffacompleta) echo "<td$stilebarrato_corr>$costo_tot_p</td>";
if ($colonna_caparra) echo "<td$stilebarrato_corr>$caparra_p</td>";
...
if ($colonna_piano) echo "<td>".fixset($piano_app[$appartamento])."</td>";
if ($colonna_casa) echo "<td>".fixset($casa_app[$appartamento])."</td>";
if ($colonna_persone) {
echo "<td>$num_persone";
...
if (!empty($costo_tot_vauta_TOT[$Euro])) $costo_tot_TOT_vedi .= $costo_tot_vauta_TOT[$Euro]."&nbsp;".$Euro." + ";
...
$contr_escludi = array();
$escludi_pag = esegui_query("select numero from $tablecontratti where tipo = 'pag_esc' and testo $LIKE '%,tab_prenota,%' ");
for ($num1 = 0 ; $num1 < numlin_query($escludi_pag) ; $num1++) $contr_escludi[risul_query($escludi_pag,$num1,'numero')] = 1;
$option_num_contr = "";
for ($num_contratto = 1 ; $num_contratto <= $max_contr ; $num_contratto++) {
if (empty($contr_escludi[$num_contratto])) {
...
} # fine if (empty($contr_escludi[$num_contratto]))
} # fine for $num_contratto
if ($option_num_contr and (!isset($show_bar) or $show_bar != "NO")) {
if ((!empty($subtotale_selezionate) or $opz_cerc_pren == "piucanc") and !$pcanc) $pcanc_corr = "+";
else $pcanc_corr = $pcanc;
echo "<br><div style=\"text-align: center;\">
<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_contratto.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"pcanc\" value=\"$pcanc_corr\">
<input type=\"hidden\" name=\"origine\" value=\"visualizza_tabelle.php?tipo_tabella=prenotazioni&amp;id_utente_vedi=$id_utente_vedi&amp;senza_colori=$senza_colori&amp;mos_tut_dat=$mos_tut_dat&amp;pag_pren_corr=$pag_pren_corr&amp;pag_pren_prec=$pag_pren_prec&amp;pag_pren_succ=$pag_pren_succ&amp;ordine_prenota=$ordine_prenota$pcanc\">
...
$n_nome_cassa = htmlspecialchars($n_nome_cassa,ENT_COMPAT);
...
if ($priv_vedi_tab_costi == "g" and empty($utenti_gruppi[$utente_inserimento])) $continua = "NO";
...
$arrotond_predef = (float) risul_query($arrotond_predef,0,'valpersonalizza');
...
$val_costo = (float) risul_query($costi,$num1,'val_costo');
$costo_valuta = (float) risul_query($costi,$num1,'costo_valuta');
$cambio_corr = $altre_valute[$valuta_corr]['cambio'];
if ($valuta[1] != $cambio_corr) {
$ultimo_costo_valuta[$valuta[0]] = $idcosti;
$n_val_costo = (float) $costo_valuta / (float) $cambio_corr;
...
$costo = esegui_query("select * from $tablecosti where idcosti = '$idcosti' ");
$tipo_costo = (float) risul_query($costo,0,'tipo_costo');
$val_costo = (float) risul_query($costo,0,'val_costo');
...
if ($attiva_casse_consentite == "n" or (isset($casse_consentite_vett[$id_cassa]) and $casse_consentite_vett[$id_cassa] == "SI")) {
...
} # fine if ($attiva_casse_consentite == "n" or (isset($casse_consentite_vett[$id_cassa]) and...
} # fine for $num1
...
if (empty($cassa_presente[$nome_cassa])) {
$num_cassa_corr++;
$casse_mostra[$num_cassa_corr] = $nome_cassa;
} # fine if (empty($cassa_presente[$nome_cassa]))
} # fine for $num1
...
$max_contr = risul_query($max_contr,0,0);
$contr_escludi = array();
$escludi_pag = esegui_query("select numero from $tablecontratti where tipo = 'pag_esc' and testo $LIKE '%,casse,%' ");
for ($num1 = 0 ; $num1 < numlin_query($escludi_pag) ; $num1++) $contr_escludi[risul_query($escludi_pag,$num1,'numero')] = 1;
$option_num_contr = "";
for ($num_contratto = 1 ; $num_contratto <= $max_contr ; $num_contratto++) {
if (empty($contr_escludi[$num_contratto])) {
...
$num_costi_in_tab++;
$val_costo = (float) risul_query($costi,$num2,'val_costo');
...
$valuta_convertibile = 1;
$altre_valute_TOT .= "<label><input type=\"checkbox\" name=\"conv_valuta".$altre_valute['id'][$valuta_corr]."\" value=\"1\" style=\"width: 14px; height: 12px;\" checked>".((float) $saldo_corr * -1)."&nbsp;$valuta_corr</label> + ";
} # fine if ($priv_ins_spese == "s" and strcmp((string) $altre_valute['id'][$valuta_corr],""))
else $altre_valute_TOT .= ((float) $saldo_corr * -1)."&nbsp;$valuta_corr + ";
...
if (!empty($entrate_valuta_TOT[$Euro])) $saldo_cassa = (float) $entrate_valuta_TOT[$Euro];
else $saldo_cassa = 0;
if (!empty($spese_valuta_TOT[$Euro])) $saldo_cassa = $saldo_cassa - (float) $spese_valuta_TOT[$Euro];
...
if (isset($mese_fine) and controlla_num_pos($mese_fine) == "SI" and $mese_fine <= 48 and $mese_fine >= $mese_cont) {
...
#idregole1_importa = esegui_query("select idregole from $tableregole_imp where app_agenzia != '' or tariffa_chiusa != '' ");
$idregole1_importa = esegui_query("select idregole from $tableregole_imp where (app_agenzia != '' and (motivazione2 is NULL or motivazione2 != 'x')) or tariffa_chiusa != '' order by iddatainizio ");
$num_idregole1_importa = numlin_query($idregole1_importa);
$num_regole1_importa = $num_idregole1_importa;
...
if ($num1 < $num_idregole1_importa) $idregola = risul_query($idregole1_importa,$num1,'idregole');
else $idregola = $id_regole_agg[$num1];
$regola1_importa = esegui_query("select * from $tableregole_imp where idregole = '$idregola' ");
$idini = risul_query($regola1_importa,$num1,'iddatainizio');
$idfine = risul_query($regola1_importa,$num1,'iddatafine');
...
$app_agenzia = (string) risul_query($regola1_importa,$num1,'app_agenzia');
$tariffa_chiusa = (string) risul_query($regola1_importa,$num1,'tariffa_chiusa');
$motivazione = (string) risul_query($regola1_importa,$num1,'motivazione');
$motivazione2 = (string) risul_query($regola1_importa,$num1,'motivazione2');
$motivazione3 = (string) risul_query($regola1_importa,$num1,'motivazione3');
...
$idreg_esist = risul_query($reg_esist,0,'idregole');
esegui_query("update $tableregole set iddatafine = '".aggslashdb($idfine)."' where idregole = '$idreg_esist' ");
if ($idreg_esist == $idregola and $tableregole_imp == $tableregole) $num_regole1_importa--;
...
if ($tableregole_imp == $tableregole) {
$id_regole_agg[$num_regole1_importa] = $idregole;
$num_regole1_importa++;
} # fine if ($tableregole_imp == $tableregole)
...
$locale_mex = substr(strstr($lingua[$id_utente_cambia],"-"),1);
$lingua_mex = str_replace("-$locale_mex","",$lingua[$id_utente_cambia]);
...
$numgiorno = $numgiorno + $aggiungi_giorni;
if ($tipo_periodi == "g") {
if ($nome_giorno == "Sun") $nome_giorno = mex(" Do","giorni_mesi.php");
if ($nome_giorno == "Mon") $nome_giorno = mex(" Lu","giorni_mesi.php");
if ($nome_giorno == "Tue") $nome_giorno = mex(" Ma","giorni_mesi.php");
if ($nome_giorno == "Wed") $nome_giorno = mex(" Me","giorni_mesi.php");
if ($nome_giorno == "Thu") $nome_giorno = mex(" Gi","giorni_mesi.php");
if ($nome_giorno == "Fri") $nome_giorno = mex(" Ve","giorni_mesi.php");
if ($nome_giorno == "Sat") $nome_giorno = mex(" Sa","giorni_mesi.php");
} # fine if ($tipo_periodi == "g")
else $nome_giorno = "";
$nome_mese = substr($datainizio,5,2);
if ($nome_mese == "01") $nome_mese = mex("Gen","giorni_mesi.php");
if ($nome_mese == "02") $nome_mese = mex("Feb","giorni_mesi.php");
if ($nome_mese == "03") $nome_mese = mex("Mar","giorni_mesi.php");
if ($nome_mese == "04") $nome_mese = mex("Apr","giorni_mesi.php");
if ($nome_mese == "05") $nome_mese = mex("Mag","giorni_mesi.php");
if ($nome_mese == "06") $nome_mese = mex("Giu","giorni_mesi.php");
if ($nome_mese == "07") $nome_mese = mex("Lug","giorni_mesi.php");
if ($nome_mese == "08") $nome_mese = mex("Ago","giorni_mesi.php");
if ($nome_mese == "09") $nome_mese = mex("Set","giorni_mesi.php");
if ($nome_mese == "10") $nome_mese = mex("Ott","giorni_mesi.php");
if ($nome_mese == "11") $nome_mese = mex("Nov","giorni_mesi.php");
if ($nome_mese == "12") $nome_mese = mex("Dic","giorni_mesi.php");
...
\$d_names = \"\\\"".mex(" Do","giorni_mesi.php")."\\\",\\\"".mex(" Lu","giorni_mesi.php")."\\\",\\\"".mex(" Ma","giorni_mesi.php")."\\\",\\\"".mex(" Me","giorni_mesi.php")."\\\",\\\"".mex(" Gi","giorni_mesi.php")."\\\",\\\"".mex(" Ve","giorni_mesi.php")."\\\",\\\"".mex(" Sa","giorni_mesi.php")."\\\"\";
...
if ($tipo_periodi == "g") {
$date_option_agg_ut = str_replace("[Sun]",mex(" Do","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[Mon]",mex(" Lu","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[Tue]",mex(" Ma","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[Wed]",mex(" Me","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[Thu]",mex(" Gi","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[Fri]",mex(" Ve","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[Sat]",mex(" Sa","giorni_mesi.php"),$date_option_agg_ut);
} # fine if ($tipo_periodi == "g")
$date_option_agg_ut = str_replace("[01]",mex("Gen","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[02]",mex("Feb","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[03]",mex("Mar","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[04]",mex("Apr","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[05]",mex("Mag","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[06]",mex("Giu","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[07]",mex("Lug","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[08]",mex("Ago","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[09]",mex("Set","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[10]",mex("Ott","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[11]",mex("Nov","giorni_mesi.php"),$date_option_agg_ut);
$date_option_agg_ut = str_replace("[12]",mex("Dic","giorni_mesi.php"),$date_option_agg_ut);
unset($y_ini_menu);
unset($m_ini_menu);
unset($d_ini_menu);
unset($n_dates_menu);
...
");
} # fine else if (!empty($importa_prezzi))
...
$locale_mex = $locale_mex2;
$lingua_mex = $lingua_mex2;

} # fine if (isset($mese_fine) and controlla_num_pos($mese_fine) == "SI" and $mese_fine <= 48 and $mese_fine >= $mese_cont)

$datafine_f = formatta_data(fixset($datafine),$stile_data);
if (!empty($origine)) $azione = controlla_pag_origine($origine);
else $azione = "visualizza_tabelle.php";
...
if (empty($permanenza_minima[$tariffa][$num1]) or $p_min > $permanenza_minima[$tariffa][$num1]) $permanenza_minima[$tariffa][$num1] = $p_min;
} # fine for $num1
...
$resto_int = $prezzo - (float) $prezzo_int;
$prezzo_sett = floor($prezzo_int / (float) $num_sett);
...
$resto_int = $prezzop - (float) $prezzo_int;
$prezzo_sett = floor($prezzo_int / (float) $num_sett);
for ($num3 = 1 ; $num3 <= $num_sett ; $num3++) $prezzop_vett[$num3] = $prezzo_sett;
$resto = $prezzo_int - ($prezzo_sett * $num_sett);
if ($resto >= 1) {
$prezzop_vett[1]++;
$resto--;
} # fine if ($resto >= 1)
for ($num3 = $num_sett ; $num3 > ($num_sett - $resto) ; $num3--) $prezzop_vett[$num3]++;
$prezzop_vett[1] += $resto_int;
for ($num3 = $iniperiodo ; $num3 <= $fineperiodo ; $num3++) {
for ($num4 = 1 ; $num4 <= $num_sett ; $num4++) {
if ($num4 != 1) $num3++;
if ($num3 <= $fineperiodo) {
esegui_query("update $tableperiodi set tariffa$num2"."p = '".$prezzop_vett[$num4]."' where idperiodi = '$num3' ");
#aggiorna_tariffe_esporta($dati_tariffe,"tariffa$num2",$num3,"NO",$prezzop_vett[$num4],$tableperiodi,$agg_vett_tar_esp,$num_agg_tar_esp);
} # fine if ($num3 <= $fineperiodo)
} # fine for $num4
} # fine for $num3
} # fine else if ($lista_sett_in_per[$num1] == 1)
} # fine if (strcmp((string) $prezzop,""))
else esegui_query("update $tableperiodi set tariffa$num2"."p = NULL where idperiodi >= '$iniperiodo' and idperiodi <= '$fineperiodo' ");
} # fine if ((string) $prezzop != (string) risul_query($prezzo_esist,0,"tariffa".$num2."p") or...
} # fine if ($dati_tariffe['tariffa'.$num2]['moltiplica'] == "p")

if ($aggiorna_esporta) {
if ($lista_sett_in_per[$num1] == 1) {
aggiorna_tariffe_esporta($dati_tariffe,"tariffa$num2","$iniperiodo-$fineperiodo",$prezzo,$prezzop,$tableperiodi,$agg_vett_tar_esp,$num_agg_tar_esp);
} # fine if ($lista_sett_in_per[$num1] == 1)
else {
$num_sett = $lista_sett_in_per[$num1];
for ($num3 = $iniperiodo ; $num3 <= $fineperiodo ; $num3++) {
...
if ($num4 != 1) $num3++;
if ($num3 <= $fineperiodo) {
if (!isset($prezzo_vett[$num4])) $prezzo_vett[$num4] = "";
if (!isset($prezzop_vett[$num4])) $prezzop_vett[$num4] = "";
aggiorna_tariffe_esporta($dati_tariffe,"tariffa$num2",$num3,$prezzo_vett[$num4],$prezzop_vett[$num4],$tableperiodi,$agg_vett_tar_esp,$num_agg_tar_esp);
} # fine if ($num3 <= $fineperiodo)
} # fine for $num4
} # fine for $num3
} # fine else if ($lista_sett_in_per[$num1] == 1)
} # fine if ($aggiorna_esporta)
...
for ($num3 = $iniperiodo ; $num3 <= $fineperiodo ; $num3++) {
$p_min = fixset(${"per$iniperiodo"."tar$num2"."pmin"});
...
elseif (strcmp((string) $$tariffa,"")) $$tariffa += (float) $tar_corr;
...
elseif (strcmp((string) $$tariffap,"")) $$tariffap += (float) $tarp_corr;
} # fine for $numtariffa
...
$tariffa = ${"tariffa".$numtariffa};
$tariffap = ${"tariffa".$numtariffa."p"};
if (($attiva_tariffe_consentite_ut == "n" or isset($tariffe_consentite_vett_ut[$numtariffa])) and !$tar_importata[$numtariffa]) $cella_prezzi = "<td id=\"pertar$numtariffa\"".fixset($tar_chiusa[$numtariffa]).">";
...
if ($priv_vedi_tab_periodi != "p" or $attiva_costi_agg_consentiti == "n" or (isset($costi_agg_consentiti_vett[$dati_ca[$num1]['id']]) and $costi_agg_consentiti_vett[$dati_ca[$num1]['id']] == "SI")) {
$tariffe_abbinate = "";
$tariffe_incomp = "";
for ($numtariffa = 1 ; $numtariffa <= $dati_tariffe['num'] ; $numtariffa = $numtariffa + 1) {
if ($priv_vedi_tab_periodi != "p" or $attiva_tariffe_consentite == "n" or isset($tariffe_consentite_vett[$numtariffa])) {
...
echo "<td>";
if ($attiva_costi_agg_consentiti_ut == "n" or (isset($costi_agg_consentiti_vett_ut[$dati_ca[$num1]['id']]) and $costi_agg_consentiti_vett_ut[$dati_ca[$num1]['id']] == "SI")) {
...
$tabelle_lock[$num_lock_w] = $PHPR_TAB_PRE."prenotacanc".$anni[$num1];
$num_lock_w++;
} # fine else if (empty($continua))
} # fine for $num1
for ($num1 = 0 ; $num1 < $num_anni ; $num1++) {
...
$tabelle_lock[$num_lock_w] = $PHPR_TAB_PRE."rclientiprenota".$anni[$num1];
$num_lock_w++;
} # fine else if (empty($continua))
} # fine for $num1
if (empty($continua)) {
...
$num_lock++;
$altre_tab_lock[$num_lock] = $tablerelclienti;
$num_lock++;
} # fine if (empty($continua))
else {
$tabelle_lock[$num_lock_w] = $tableclienti;
$num_lock_w++;
$tabelle_lock[$num_lock_w] = $tablerelclienti;
$num_lock_w++;
} # fine else if (empty($continua))
for ($num1 = 0 ; $num1 < $num_anni ; $num1++) {
...
if ($anno_mostra < $anno) for ($num2 = 0 ; $num2 < $num_prenotacanc ; $num2++) $cliente_attivo_pcanc[risul_query($prenotacanc,$num2,'idclienti')] = 1;
else for ($num2 = 0 ; $num2 < $num_prenotacanc ; $num2++) $cliente_attivo[risul_query($prenotacanc,$num2,'idclienti')] = 1;
$tablerclientiprenota_mostra = $PHPR_TAB_PRE."rclientiprenota".$anno_mostra;
if ($anno_mostra < $anno) {
$ospiti = esegui_query("select distinct $tablerclientiprenota_mostra.idclienti from $tablerclientiprenota_mostra inner join $tableprenotacanc_mostra on $tablerclientiprenota_mostra.idprenota = $tableprenotacanc_mostra.idprenota ");
$num_ospiti = numlin_query($ospiti);
for ($num2 = 0 ; $num2 < $num_ospiti ; $num2++) $cliente_attivo_pcanc[risul_query($ospiti,$num2,'idclienti')] = 1;
$ospiti = esegui_query("select distinct $tablerclientiprenota_mostra.idclienti from $tablerclientiprenota_mostra inner join $tableprenota_mostra on $tablerclientiprenota_mostra.idprenota = $tableprenota_mostra.idprenota ");
} # fine if ($anno_mostra < $anno)
else $ospiti = esegui_query("select distinct idclienti from $tablerclientiprenota_mostra ");
$num_ospiti = numlin_query($ospiti);
...
if (!empty($continua) and !empty($canc_clienti_pcanc)) {
if (empty($cliente_attivo[$idclienti])) $cancella_cliente_id[$idclienti] = 1;
} # fine if (!empty($continua) and !empty($canc_clienti_pcanc))
elseif (empty($cliente_attivo[$idclienti]) and empty($cliente_attivo_pcanc[$idclienti])) $cancella_cliente_id[$idclienti] = 1;
if (!empty($cliente_attivo[$idclienti]) and !empty($cliente_attivo_pcanc[$idclienti])) unset($cliente_attivo_pcanc[$idclienti]); 
} # fine for $num1
if (empty($continua)) {
unlock_tabelle($tabelle_lock);
echo "
...
if (!empty($cliente_attivo_pcanc)) {
echo " (<label><input type=\"checkbox\" name=\"canc_clienti_pcanc\" value=\"1\">".mex("Cancellare anche",$pag)."
 <b>".count($cliente_attivo_pcanc)."</b> ".mex("clienti con solo prenotazioni cancellate negli anni passati, cancellando le loro prenotazioni in modo permanente",$pag)."</label>)
</div></form></td></tr><tr><td>&nbsp;</td></tr><tr><td>";
} # fine if (!empty($cliente_attivo_pcanc))
else echo "</div></form></td><td>";
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_tabelle.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">
...
$anno_mostra = $anni[$num2];
if ($anno_mostra < $anno) {
$tableprenotacanc_mostra = $PHPR_TAB_PRE."prenotacanc".$anno_mostra;
esegui_query("delete from $tableprenotacanc_mostra where idclienti = '$idclienti' ");
$tablerclientiprenota_mostra = $PHPR_TAB_PRE."rclientiprenota".$anno_mostra;
esegui_query("delete from $tablerclientiprenota_mostra where idclienti = '$idclienti' ");
} # fine if ($anno_mostra < $anno)
} # fine for $num2
...
else $clienti = esegui_query("select * from $tableclienti where (cognome $ILIKE '%$cognome_cerca%' or cognome2 $ILIKE '%$cognome_cerca%') $condizione_utente order by cognome ");
} # fine if (!numlin_query($campi_cerca_clienti))
else {
$query_cerca_clienti = "select * from $tableclienti where (cognome $ILIKE '%$cognome_cerca%' or cognome2 $ILIKE '%$cognome_cerca%' or email $ILIKE '%$cognome_cerca%' or email2 $ILIKE '%$cognome_cerca%' or email3 $ILIKE '%$cognome_cerca%'";
...
$cognome2 = (string) risul_query($clienti,$num1,'cognome2');
if (strcmp($cognome2,"")) $cognome .= " $cognome2";
...
if ($num2 >= $num_prenotazioni) $num_r = $num2 - $num_prenotazioni;
else $num_r = $num2;
$utente_inserimento_prenota = risul_query($prenotazioni,$num_r,'utente_inserimento',$tableprenota);
if ($priv_vedi_tab_prenotazioni == "s" or ($priv_vedi_tab_prenotazioni == "p" and $utente_inserimento_prenota == $id_utente) or ($priv_vedi_tab_prenotazioni == "g" and !empty($utenti_gruppi[$utente_inserimento_prenota]))) {
$numero = risul_query($prenotazioni,$num_r,'idprenota',$tableprenota);
$id_data_inizio = risul_query($prenotazioni,$num_r,'iddatainizio',$tableprenota);
$link_modifica = "SI";
if ($priv_mod_prenotazioni == "n") $link_modifica = "NO";
if ($priv_mod_prenotazioni == "p" and $utente_inserimento_prenota != $id_utente) $link_modifica = "NO";
if ($priv_mod_prenotazioni == "g" and empty($utenti_gruppi[$utente_inserimento_prenota])) $link_modifica = "NO";
if ($priv_mod_prenota_iniziate != "s" and $id_periodo_corrente >= $id_data_inizio) $link_modifica = "NO";
if ($priv_mod_prenota_ore != "000") {
$adesso = date("YmdHis",(time() + (C_DIFF_ORE * 3600)));
$data_ins = risul_query($prenotazioni,$num_r,'datainserimento',$tableprenota);
$limite = date("YmdHis",mktime((substr($data_ins,11,2) + (int) $priv_mod_prenota_ore),substr($data_ins,14,2),substr($data_ins,17,2),substr($data_ins,5,2),substr($data_ins,8,2),substr($data_ins,0,4)));
if ($adesso > $limite) $link_modifica = "NO";
} # fine if ($priv_mod_prenota_ore != "000")
...
if ($modifica_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento])) $mostra_link = "NO";
...
$max_contr = risul_query($max_contr,0,0);
$contr_escludi = array();
$escludi_pag = esegui_query("select numero from $tablecontratti where tipo = 'pag_esc' and testo $LIKE '%,tab_clienti,%' ");
for ($num1 = 0 ; $num1 < numlin_query($escludi_pag) ; $num1++) $contr_escludi[risul_query($escludi_pag,$num1,'numero')] = 1;
$option_num_contr = "";
for ($num_contratto = 1 ; $num_contratto <= $max_contr ; $num_contratto++) {
if (empty($contr_escludi[$num_contratto])) {
...
if (strcmp((string) $app_agenzia,"")) {
if (!isset($appartamenti_consentiti[$app_agenzia]) or $appartamenti_consentiti[$app_agenzia] != "SI") $link_cancella = 0;
...
$n_app = htmlspecialchars($n_app,ENT_COMPAT);
...
$inv = "&nbsp;";
$stile_inv = "";
if ($priv_vedi_inv_app == "s" or !isset($app_consentiti_inv[$idappartamenti]) or $app_consentiti_inv[$idappartamenti] != "NO") {
...
if ($num_appartamenti < 70 or !empty($mostra_griglia_app)) {

if (!empty($modifica_app_vicini) and $id_utente == 1) {
$tabelle_lock = array($tableappartamenti);
$tabelle_lock = lock_tabelle($tabelle_lock);
$app_vicini = array('vett' => 'SI');
for ($num1 = 0 ; $num1 < $num_appartamenti ; $num1++) ${"idappartamenti".$num1} = htmlspecialchars(${"idappartamenti".$num1},ENT_COMPAT);
for ($num1 = 0 ; $num1 < $num_appartamenti ; $num1++) {
$idappartamenti = risul_query($appartamenti,$num1,'idappartamenti');
...
echo "<br><br>";
} # fine if ($num_appartamenti < 70 or !empty($mostra_griglia_app))
...
$n_nome_bene = htmlspecialchars($n_nome_bene,ENT_COMPAT);
$n_codice_bene = htmlspecialchars($n_codice_bene,ENT_COMPAT);
$n_descrizione_bene = htmlspecialchars($n_descrizione_bene,ENT_COMPAT);
$tabelle_lock = array($tablebeniinventario);
$altre_tab_lock = array($tableutenti);
$tabelle_lock = lock_tabelle($tabelle_lock,$altre_tab_lock);
if ($id_utente != 1 or empty($n_utente_ins_bene) or controlla_num_pos($n_utente_ins_bene) != "SI") $n_utente_ins_bene = $id_utente;
...
$n_nome_mag = htmlspecialchars($n_nome_mag,ENT_COMPAT);
$n_descrizione_mag = htmlspecialchars($n_descrizione_mag,ENT_COMPAT);
...
${"nome_bene".$idinv} = htmlspecialchars(${"nome_bene".$idinv},ENT_COMPAT);
${"codice_bene".$idinv} = htmlspecialchars(${"codice_bene".$idinv},ENT_COMPAT);
${"descrizione_bene".$idinv} = htmlspecialchars(${"descrizione_bene".$idinv},ENT_COMPAT);
...
$altri_contr_np = fixset($nomi_contratti['num_prog'][$dir_salva."/".$nome_contr]);
...
if ($dir_salva and ($priv_vedi_tab_doc == "s" or $attiva_contratti_consentiti == "n" or (isset($contratti_consentiti_vett[$num_contr]) and $contratti_consentiti_vett[$num_contr] == "SI"))) {
...
if ($attiva_contratti_consentiti == "n" or (isset($contratti_consentiti_vett[$num_contr]) and $contratti_consentiti_vett[$num_contr] == "SI")) $nome_file_contr = "<a href=\"./visualizza_contratto.php?id_sessione=$id_sessione&amp;anno=$anno&amp;id_transazione=tabdoc&amp;numero_contratto=$num_contr&amp;contr_corr=".urlencode($contr_corr_orig)."\">$nome_file_contr</a>";
...
if ($tab_contr) {
if (($num_pagine_prenota == 1 or $pagina_prenota == $num_pagine_prenota) and ($attiva_contratti_consentiti == "n" or (isset($contratti_consentiti_vett[$num_contr]) and $contratti_consentiti_vett[$num_contr] == "SI")) and $ultimo_n_contr == $max_contr_esist and $priv_cancella_contratti != "n") {
...
if ($priv_mod_doc == "s" and ($attiva_contratti_consentiti == "n" or (isset($contratti_consentiti_vett[$num_contr]) and $contratti_consentiti_vett[$num_contr] == "SI"))) {
...
if ($attiva_contratti_consentiti == "n" or (isset($contratti_consentiti_vett[$num_contr]) and $contratti_consentiti_vett[$num_contr] == "SI")) {
$anno_doc = $anno;
$max_contr_esist = trova_max_contr_salv($num_contr,$anno_doc,$dir_salva,$nome_max_contr,$suff_file,$nomi_contratti,$tablecontratti);
$nuovo_n_contr = $max_contr_esist + 1;
...
$cifra_vedi = (float) round($cifra_vedi,1);
if (strlen($cifra_vedi) > 3) {
$cifra_vedi = (float) floor($cifra_vedi);
if (strlen($cifra_vedi) > 3) {
$cifra_vedi = (float) $cifra_vedi / 1000;
...
$anno_inizio = (int) substr($datainizio[$idper],0,4);
$mese_inizio = (int) substr($datainizio[$idper],5,2);
...
$tariffa[1] = (float) fixset($tariffa[1]);
$sconto = risul_query($tutte_prenota,$num1,'sconto',$tableprenota_stat);
if ((float) $sconto > $tariffa[1]) $sconto = $tariffa[1];
$prezzo_tariffa = $tariffa[1];
...
$commissioni = (float) risul_query($tutte_prenota,$num1,'commissioni',$tableprenota_stat);
if ($commissioni > 0) {
$tariffa_tot = (float) $tariffa_tot - (float) $commissioni;
$prezzo_tariffa = (float) $prezzo_tariffa - (float) $commissioni;
$commissioni_sett = round(((float) $commissioni / ($id_data_fine - $id_data_inizio + 1)),2);
for ($num2 = 0 ; $num2 < count($tariffe_settimanali) ; $num2++) $tariffe_settimanali[$num2] = (float) $tariffe_settimanali[$num2] - (float) $commissioni_sett;
$resto_arr = (float) $commissioni - ((float) $commissioni_sett * ($id_data_fine - $id_data_inizio + 1));
$tariffe_settimanali[0] = (float) $tariffe_settimanali[0] - (float) $resto_arr;
} # fine if ($commissioni > 0)
} # fine if (!$prezzi['commissioni'])
if (!$prezzi['tasse']) {
$tasse_tot = 0;
$tasse_perc = (float) risul_query($tutte_prenota,$num1,'tasseperc',$tableprenota_stat);
$caparra = (float) risul_query($tutte_prenota,$num1,'caparra',$tableprenota_stat);
$numpersone = risul_query($tutte_prenota,$num1,'num_persone',$tableprenota_stat);
if ($tasse_perc) {
$tasse = (($tariffa[1] - (float) $sconto) / ($tasse_perc + 100)) * $tasse_perc;
$tasse = $tasse / (float) $prezzi['tasse_arrotond'];
$tasse = round($tasse);
$tasse = $tasse * (float) $prezzi['tasse_arrotond'];
$tasse_tot = (float) $tasse_tot + $tasse;
} # fine if ($tasse_perc)
$prezzo_tariffa = (float) $prezzo_tariffa - (float) $tasse_tot;
$tasse_sett = round(((float) $tasse_tot / ($id_data_fine - $id_data_inizio + 1)),2);
for ($num2 = 0 ; $num2 < count($tariffe_settimanali) ; $num2++) $tariffe_settimanali[$num2] = (float) $tariffe_settimanali[$num2] - (float) $tasse_sett;
$resto_arr = (float) $tasse_tot - ((float) $tasse_sett * ($id_data_fine - $id_data_inizio + 1));
$tariffe_settimanali[0] = (float) $tariffe_settimanali[0] - (float) $resto_arr;
$dati_cap = dati_costi_agg_prenota($tablecostiprenota_stat,$id_prenota);
$costo_agg_tot = 0;
$costo_escludi_perc = 0;
for ($numca = 0 ; $numca < $dati_cap['num'] ; $numca++) {
$costo_agg_parziale = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$id_data_inizio,$id_data_fine,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$tariffa[1],$tariffe_settimanali_int,((float) $tariffa[1] + $costo_agg_tot - (float) $sconto),$caparra,$numpersone,fixset($dati_cap[$numca]['cat_pers']),0,$costo_escludi_perc);
$costo_agg_tot = (float) $costo_agg_tot + $costo_agg_parziale;
if ($dati_cap[$numca]['escludi_tot_perc'] == "s") $costo_escludi_perc = (float) $costo_escludi_perc + $costo_agg_parziale;
if ($dati_cap[$numca]['tasseperc']) {
if ($dati_cap[$numca]['tasseperc'] == -1) $tasse_ca = (float) $costo_agg_parziale;
else {
$tasse_ca = ($costo_agg_parziale / (float) ($dati_cap[$numca]['tasseperc'] + 100)) * (float) $dati_cap[$numca]['tasseperc'];
$tasse_ca = $tasse_ca / $prezzi['tasse_arrotond'];
$tasse_ca = round($tasse_ca);
$tasse_ca = $tasse_ca * (float) $prezzi['tasse_arrotond'];
} # fine else if ($dati_cap[$numca]['tasseperc'] == -1)
$tasse_tot = (float) $tasse_tot + (float) $tasse_ca;
} # fine if ($dati_cap[$numca]['tasseperc'])
} # fine for $numca
$tariffa_tot = (float) $tariffa_tot - (float) $tasse_tot;
} # fine if (!$prezzi['tasse'])

$val_medio_costi_agg_e_sconto_per_sett = round((((float) $tariffa_tot - (float) $prezzo_tariffa) / (float) $num_sett),2);
$val_medio_sconto_per_sett = round(((float) $sconto  / (float) $num_sett),2);
...
$anno_ins = (int) substr($data_inserimento,0,4);
$mese_ins = (int) substr($data_inserimento,5,2);
if (substr(fixstr($datainizio[$id_data_inizio]),0,4) == $anno_stat) {
if ($anno_ins < $anno_stat) {
$anno_ins = (int) $anno_stat;
$mese_ins = (int) 0;
} # fine if ($anno_ins < $anno_stat)
$entrate_ins_mese[$anno_ins][$mese_ins] = (float) fixset($entrate_ins_mese[$anno_ins][$mese_ins]) + (float) $tariffa_tot;
...
$anno_inizio = (int) substr(fixstr($datainizio[$num2]),0,4);
$mese_inizio = (int) substr(fixstr($datainizio[$num2]),5,2);
if (!isset($app_occupati[$anno_inizio][$mese_inizio])) $app_occupati[$anno_inizio][$mese_inizio] = 0;
$app_occupati[$anno_inizio][$mese_inizio]++;
$entrate_mese[$anno_inizio][$mese_inizio] = (float) fixset($entrate_mese[$anno_inizio][$mese_inizio]) + (float) fixset($tariffe_settimanali[($num2 - $id_data_inizio)]) + (float) $val_medio_costi_agg_e_sconto_per_sett;
$entrate_mese_no_ca[$anno_inizio][$mese_inizio] = (float) fixset($entrate_mese_no_ca[$anno_inizio][$mese_inizio]) + (float) fixset($tariffe_settimanali[($num2 - $id_data_inizio)]) - (float) $val_medio_sconto_per_sett;
...
$anno_inizio = (int) substr(fixstr($datainizio[$id_data_inizio]),0,4);
$mese_inizio = (int) substr(fixstr($datainizio[$id_data_inizio]),5,2);
...
$entrate_origine[$anno_inizio][$origine] = (float) fixset($entrate_origine[$anno_inizio][$origine]) + (float) $tariffa_tot;
} # fine if ($origine != "")
if ($nazionalita != "") {
...
$entrate_nazionalita[$anno_inizio][$nazionalita] = (float) fixset($entrate_nazionalita[$anno_inizio][$nazionalita]) + (float) $tariffa_tot;
} # fine if ($nazionalita != "")
...
if ($metodo_pagamento) $pagamenti[$anno_inizio][$metodo_pagamento] = (float) fixset($pagamenti[$anno_inizio][$metodo_pagamento]) + (float) $saldo_prenota;
} # fine for $num2
...
$entrate_prog_ins_mese[$anno_stat][0] = (float) fixset($entrate_ins_mese[$anno_stat][0]);
$num_prenota_prog_ins_mese[$anno_stat][0] = fixset($num_prenota_ins_mese[$anno_stat][0]);
for ($num1 = 1 ; $num1 <= 12 ; $num1++) {
$entrate_prog_ins_mese[$anno_stat][$num1] = (float) $entrate_prog_ins_mese[$anno_stat][($num1 - 1)] + (float) fixset($entrate_ins_mese[$anno_stat][$num1]);
...
if (!empty($mesi[$num1])) {
$percent_occupazione[$anno_stat]['TOT'] = (float) $percent_occupazione[$anno_stat]['TOT'] + (float) $percent_occupazione[$anno_stat][$num1];
$num_mesi_media++;
if ($num_appartamenti) {
$revpar[$anno_stat][$num1] = round(((float) fixset($entrate_mese_no_ca[$anno_stat][$num1]) / ((float) $num_appartamenti * (float) $num_giorni_mese[$anno_stat][$num1])),2);
$revpar_ca[$anno_stat][$num1] = round(((float) fixset($entrate_mese[$anno_stat][$num1]) / ((float) $num_appartamenti * (float) $num_giorni_mese[$anno_stat][$num1])),2);
} # fine if ($num_appartamenti)
$entrate_mese[$anno_stat]['TOT'] = (float) $entrate_mese[$anno_stat]['TOT'] + (float) $entrate_mese[$anno_stat][$num1];
$entrate_mese_no_ca[$anno_stat]['TOT'] = (float) $entrate_mese_no_ca[$anno_stat]['TOT'] + (float) $entrate_mese_no_ca[$anno_stat][$num1];
$app_occupati[$anno_stat]['TOT'] = (float) $app_occupati[$anno_stat]['TOT'] + (float) fixset($app_occupati[$anno_stat][$num1]);
...
if ($app_occupati[$anno_stat][$num1]) $prezzo_medio_prenota[$anno_stat][$num1] = (float) fixset($entrate_mese[$anno_stat][$num1]) / (float) fixset($app_occupati[$anno_stat][$num1]);
else $prezzo_medio_prenota[$anno_stat][$num1] = (float) 0;
} # fine if (!empty($mesi[$num1]))
} # fine for $num1
if ($num_mesi_media) $percent_occupazione[$anno_stat]['TOT'] = round(((float) $percent_occupazione[$anno_stat]['TOT'] / (float) $num_mesi_media),1);
if ($num_giorni_revpar and $num_appartamenti) $revpar[$anno_stat]['TOT'] = round(((float) $entrate_mese_no_ca[$anno_stat]['TOT'] / ((float) $num_appartamenti * (float) $num_giorni_revpar)),2);
if ($num_giorni_revpar_ca and $num_appartamenti) $revpar_ca[$anno_stat]['TOT'] = round(((float) $entrate_mese[$anno_stat]['TOT'] / ((float) $num_appartamenti * (float) $num_giorni_revpar_ca)),2);
if ($app_occupati[$anno_stat]['TOT']) $prezzo_medio_prenota[$anno_stat]['TOT'] = (float) $entrate_mese[$anno_stat]['TOT'] / (float) $app_occupati[$anno_stat]['TOT'];

if (isset($num_origine) and is_array($num_origine)) {
$num_origine_anno = fixset($num_origine[$anno_stat]);
...
else $percent_num_origine[$anno_stat][$orig] = round(((float) $val / (float) $num_prenota_mese[$anno_stat]['TOT'] * 100),1);
if (!$entrate_origine[$anno_stat][$orig] or !$entrate_mese[$anno_stat]['TOT']) $percent_entrate_origine[$anno_stat][$orig] = 0;
else $percent_entrate_origine[$anno_stat][$orig] = round(((float) $entrate_origine[$anno_stat][$orig] / (float) $entrate_mese[$anno_stat]['TOT'] * 100),1);
} # fine foreach ($num_origine_anno as $orig => $val)
} # fine if (is_array($num_origine_anno))
} # fine if (isset($num_origine) and is_array($num_origine))

if (isset($num_nazionalita) and is_array($num_nazionalita)) {
$num_nazionalita_anno = fixset($num_nazionalita[$anno_stat]);
...
else $percent_num_nazionalita[$anno_stat][$nazion] = round(((float) $val / (float) $num_prenota_mese[$anno_stat]['TOT'] * 100),1);
if (!$entrate_nazionalita[$anno_stat][$nazion] or !$entrate_mese[$anno_stat]['TOT']) $percent_entrate_nazionalita[$anno_stat][$nazion] = 0;
else $percent_entrate_nazionalita[$anno_stat][$nazion] = round(((float) $entrate_nazionalita[$anno_stat][$nazion] / (float) $entrate_mese[$anno_stat]['TOT'] * 100),1);
} # fine foreach ($num_nazionalita_anno as $nazion => $val)
} # fine if (is_array($num_nazionalita_anno))
} # fine if (isset($num_nazionalita) and is_array($num_nazionalita))
...
$nome_mese = array("",mex("Gen","giorni_mesi.php"),mex("Feb","giorni_mesi.php"),mex("Mar","giorni_mesi.php"),mex("Apr","giorni_mesi.php"),mex("Mag","giorni_mesi.php"),mex("Giu","giorni_mesi.php"),mex("Lug","giorni_mesi.php"),mex("Ago","giorni_mesi.php"),mex("Set","giorni_mesi.php"),mex("Ott","giorni_mesi.php"),mex("Nov","giorni_mesi.php"),mex("Dic","giorni_mesi.php"));
...
if (!empty($prezzo_comm_sel)) $prezzi['commissioni'] = 1;
else $prezzi['commissioni'] = 0;
if (!empty($prezzo_tasse_sel)) $prezzi['tasse'] = 1;
else $prezzi['tasse'] = 0;
if (!$prezzi['tasse']) {
$arrotond_tasse = esegui_query("select * from $tablepersonalizza where idpersonalizza = 'arrotond_tasse' and idutente = '$id_utente'");
$prezzi['tasse_arrotond'] = (float) risul_query($arrotond_tasse,0,'valpersonalizza');
include("./includes/funzioni_costi_agg.php");
} # fine if (!$prezzi['tasse'])
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">";
for ($num1 = 1 ; $num1 <= 12 ; $num1++) {
if (!empty(${"mese_sel".$num1})) echo "<input type=\"hidden\" name=\"mese_sel$num1\" value=\"SI\">";
} # fine for $num1
...
<input type=\"hidden\" name=\"prezzo_comm_sel\" value=\"".htmlspecialchars(fixstr($prezzo_comm_sel))."\">
<input type=\"hidden\" name=\"prezzo_tasse_sel\" value=\"".htmlspecialchars(fixstr($prezzo_tasse_sel))."\">
...
<input type=\"hidden\" name=\"mese_ini_ins\" value=\"".htmlspecialchars(fixstr($mese_ini_ins))."\">
<input type=\"hidden\" name=\"mese_fine_ins\" value=\"".htmlspecialchars(fixstr($mese_fine_ins))."\">
<input type=\"hidden\" name=\"gio_ini_ins\" value=\"".htmlspecialchars(fixstr($gio_ini_ins))."\">
<input type=\"hidden\" name=\"gio_fine_ins\" value=\"".htmlspecialchars(fixstr($gio_fine_ins))."\">
<input type=\"hidden\" name=\"prezzo_comm_sel\" value=\"".htmlspecialchars(fixstr($prezzo_comm_sel))."\">
...
".mex("Mesi",$pag).": ";
for ($num1 = 1 ; $num1 <= 12 ; $num1++) {
if (empty(${"mese_sel".$num1})) { $checked = ""; $b = ""; $slash_b = ""; }
else { $checked = " checked"; $b = "<b>"; $slash_b = "</b>"; }
...
if (!empty(${"mese_sel".$num1}) or (!$num1 and !empty($mese_sel1))) {
$x[$num_x] = $num1;
#$nome_x[$num_x] = $num1."<br><small>(".$num_giorni_mese[$anno_stat][$num1].mex("g",$pag).")</small>";
$num_x++;
...
if (!empty(${"anno_sel".$anni[$num1]})) {
$anno_stat = $anni[$num1];
${"y".$num_y} = fixset(${"num_origine".$anno_stat}[$anno_stat]);
...
if (!empty(${"anno_sel".$anni[$num1]})) {
$anno_stat = $anni[$num1];
${"y".$num_y} = fixset(${"entrate_origine".$anno_stat}[$anno_stat]);
for ($num2 = 0 ; $num2 < $num_x ; $num2++) {
if (empty(${"y".$num_y}[$x[$num2]])) ${"y".$num_y}[$x[$num2]] = 0;
...
$pagamenti_anno = fixset(${"pagamenti".$anno_stat}[$anno_stat]);
...
if (!empty(${"anno_sel".$anni[$num1]})) {
$anno_stat = $anni[$num1];
${"y".$num_y} = fixset(${"pagamenti".$anno_stat}[$anno_stat]);
$titolo_serie[$num_y] = $anno_stat;
$num_y++;
if ($num_y > 8) break;
} # fine (!empty(${"anno_sel".$anni[$num1]}))
...
if (!empty(${"anno_sel".$anni[$num1]})) {
$anno_stat = $anni[$num1];
if (isset(${"num_nazionalita".$anno_stat}) and is_array(${"num_nazionalita".$anno_stat})) {
$num_nazionalita = fixset(${"num_nazionalita".$anno_stat}[$anno_stat]);
...
} # fine if (isset(${"num_nazionalita".$anno_stat}) and is_array(${"num_nazionalita".$anno_stat}))
} # fine if (!empty(${"anno_sel".$anni[$num1]}))
} # fine for $num1
...
if (!empty(${"anno_sel".$anni[$num1]})) {
$anno_stat = $anni[$num1];
${"y".$num_y} = fixset(${"num_nazionalita".$anno_stat}[$anno_stat]);
for ($num2 = 0 ; $num2 < $num_x ; $num2++) {
if (empty(${"y".$num_y}[$x[$num2]])) ${"y".$num_y}[$x[$num2]] = 0;
...
if (!empty(${"anno_sel".$anni[$num1]})) {
$anno_stat = $anni[$num1];
${"y".$num_y} = fixset(${"entrate_nazionalita".$anno_stat}[$anno_stat]);
for ($num2 = 0 ; $num2 < $num_x ; $num2++) {
if (empty(${"y".$num_y}[$x[$num2]])) ${"y".$num_y}[$x[$num2]] = 0;
