...
if ($ut_ins_succ != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$ut_ins_succ]))) $ln1 = 0;
...
$cognome = esegui_query("select cognome,cognome2 from $tableclienti where idclienti = '$idclienti' ");
if (numlin_query($cognome)) {
$cognome2 = str_replace("&gt;","",str_replace("&lt;","",(string) risul_query($cognome,0,'cognome2')));
$cognome = str_replace("&gt;","",str_replace("&lt;","",risul_query($cognome,0,'cognome')));
if (strcmp($cognome2,"")) $cognome .= " $cognome2";
} # fine if (numlin_query($cognome))
else $cognome = "";
...
if ($priv_mod_prenotazioni == "g" and empty($utenti_gruppi[$utente_inserimento])) $link_modifica = 0;
...
if ($ut_ins_prec != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$ut_ins_prec]))) {
...
if (numlin_query($pren_succ)) {
$ut_ins_succ = risul_query($pren_succ,0,'utente_inserimento');
if ($ut_ins_succ != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$ut_ins_succ]))) continue;
} # fine if (numlin_query($pren_succ))
...
if (($vedi_clienti == "PROPRI" and $utente_inserimento != $id_utente) or ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento]))) $dati_xml .= mex("Cliente",$pag)." $id_cli ";
else {
...
$costo_agg_tot = round(((float) $tariffa_tot - (float) $costo_tariffa + (float) $sconto),2);
if ($costo_agg_tot) {
...
$resto_tarsco_valuta = round(((float) $costo_tariffa - (float) $sconto - (float) $pagato),2);
...
if (controlla_num_pos(fixset($mese)) != "SI") $mese = date('n');
...
if ($priv_vedi_tab_mesi != "p" or $num1 >= $num_appartamenti or $appartamenti_consentiti[$id_appartamento] != "NO") {
...
#if ($priv_vedi_tab_mesi == "p" and (($utente_inserimento != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$utente_inserimento]))) or ($periodo_consentito_app[$id_appartamento][$id_settimana] == "NO" and $esiste != 1))) {
if ($priv_vedi_tab_mesi == "p" and (($utente_inserimento != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$utente_inserimento]))))) {
$freccia_sx = "";
$freccia_dx = "";
...
if (numlin_query($pren_prec)) {
$ut_ins_prec = risul_query($pren_prec,0,'utente_inserimento');
if ($ut_ins_prec != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$ut_ins_prec]))) $freccia_sx = "&lt;- ";
} # fine if (numlin_query($pren_prec))
...
if (numlin_query($pren_succ)) {
$ut_ins_succ = risul_query($pren_succ,0,'utente_inserimento');
if ($ut_ins_succ != $id_utente and ($priv_prenota_gruppi != "SI" or empty($utenti_gruppi[$ut_ins_succ]))) $freccia_dx = " -&gt;";
} # fine if (numlin_query($pren_succ))
...
if ($vedi_clienti == "GRUPPI" and empty($utenti_gruppi[$utente_inserimento_cli])) $mostra_cliente = "NO";
} # fine if ($vedi_clienti == "PROPRI" or $vedi_clienti == "GRUPPI")
if ($mostra_cliente == "NO") $cognome = $id_prenota;
else {
$cognome = risul_query($prenotazione_presente,$prenota_corr,'cognome',$tableclienti);
$cognome2 = (string) risul_query($prenotazione_presente,$prenota_corr,'cognome2',$tableclienti);
if (strcmp($cognome2,"")) $cognome .= " $cognome2";
...
} # fine if ($priv_vedi_tab_mesi != "p" or $num1 >= $num_appartamenti or $appartamenti_consentiti[$id_appartamento] != "NO")
} # fine for $num1
...
$contr_escludi = array();
$escludi_pag = esegui_query("select numero from $tablecontratti where tipo = 'pag_esc' and testo $LIKE '%,tab_mese,%' ");
