...
} # fine if (numlin_query($prenota_succ) == 1)
...
function trova_prenota_interconn (&$lista_interconn,&$lista_utenti_mess_interconn,$num_id_prenota,$id_prenota_idpr,$anno,$tableinterconnessioni,$inizioperiodo="",$fineperiodo="") {
$prenota_interconn = "";
$lista_interconn = "";
$lista_utenti_mess_interconn = "";
$file_interconnessioni = C_DATI_PATH."/dati_interconnessioni.php";
if (@is_file($file_interconnessioni)) include($file_interconnessioni);
else $ic_present = array();
if (!empty($ic_present)) {
unset($id_per_corr_reale);
for ($num_idpr = 0 ; $num_idpr < $num_id_prenota ; $num_idpr++) {
$id_prenota = $id_prenota_idpr[$num_idpr];
$interconn_esist = esegui_query("select nome_ic from $tableinterconnessioni where idlocale = '$id_prenota' and tipoid = 'prenota' and anno = '$anno' ");
if (numlin_query($interconn_esist)) {
$nome_ic = risul_query($interconn_esist,0,'nome_ic');
if (is_array($ic_present) and isset($ic_present[$nome_ic]) and $ic_present[$nome_ic] == "SI") {
if ((!$inizioperiodo or $inizioperiodo >= $id_prenota_idpr['iddi'][$id_prenota]) and (!$fineperiodo or $fineperiodo <= $id_prenota_idpr['iddf'][$id_prenota])) {
if (!isset($id_per_corr_reale)) $id_per_corr_reale = calcola_id_periodo_corrente($anno,"NO");
#$id_per_corr_reale = 0;
if ($id_prenota_idpr['iddf'][$id_prenota] > $id_per_corr_reale) {
$interconn_dir = opendir("./includes/interconnect/");
while ($mod_ext = readdir($interconn_dir)) {
if ($mod_ext != "." and $mod_ext != ".." and @is_dir("./includes/interconnect/$mod_ext") and @is_file("./includes/interconnect/$mod_ext/name.php")) {
include("./includes/interconnect/$mod_ext/name.php");
if ($interconnection_name == $nome_ic) {
$prenota_interconn .= "$id_prenota,";
if (!strstr(",".$lista_interconn,",$nome_ic,")) {
$lista_interconn .= "$nome_ic,";
if (isset(${"ic_mess_user_".$interconnection_name}) and $lista_utenti_mess_interconn != "all") {
unset(${"ic_mess_user_".$interconnection_name});
include($file_interconnessioni);
if (isset(${"ic_mess_user_".$interconnection_name})) {
if (strtolower(${"ic_mess_user_".$interconnection_name}) == "all") $lista_utenti_mess_interconn = "all";
else $lista_utenti_mess_interconn .= ${"ic_mess_user_".$interconnection_name}.",";
} # fine if (isset(${"ic_mess_user_".$interconnection_name}))
} # fine if (isset(${"ic_mess_user_".$interconnection_name}))
} # fine if (!strstr(",".$lista_interconn,",$nome_ic,"))
break;
} # fine if ($interconnection_name == $nome_ic)
} # fine if ($mod_ext != "." and $mod_ext != ".." and @is_dir("./includes/interconnect/$mod_ext") and...
} # fine while ($mod_ext = readdir($interconn_dir))
closedir($interconn_dir);
} # fine if ($id_prenota_idpr['iddf'][$id_prenota] > $id_per_corr_reale)
} # fine if ((!$inizioperiodo or $inizioperiodo >= $id_prenota_idpr['iddi'][$id_prenota]) and...
} # fine if (is_array($ic_present) and isset($ic_present[$nome_ic]) and $ic_present[$nome_ic] == "SI")
} # fine if (numlin_query($interconn_esist))
} # fine for $num_idpr
$prenota_interconn = substr($prenota_interconn,0,-1);
} # fine if (!empty($ic_present))
return $prenota_interconn;
} # fine function trova_prenota_interconn




function spedisci_mess_interconn ($testo,$prenota_interconn,$lista_interconn,$lista_utenti_mess_interconn,$anno,$tableutenti,$tablerelgruppi,$tablemessaggi) {
$lista_utenti = ",";
if (strtolower($lista_utenti_mess_interconn) == "all" or stristr(",".$lista_utenti_mess_interconn,",g")) {
if (strtolower($lista_utenti_mess_interconn) == "all") {
$tutti_utenti = esegui_query("select * from $tableutenti order by idutenti");
$cmp_idutente = "idutenti";
} # fine if (strtolower($lista_utenti_mess_interconn) == "all")
else {
$cond_gruppi = "";
$lista_utenti_vett = explode(",",$lista_utenti_mess_interconn);
for ($num1 = 0 ; $num1 < (count($lista_utenti_vett) - 1) ; $num1++) {
if (strtolower(substr($lista_utenti_vett[$num1],0,1)) == "g") $cond_gruppi .= " or idgruppo = '".aggslashdb(substr($lista_utenti_vett[$num1],1))."'";
else $lista_utenti .= $lista_utenti_vett[$num1].",";
} # fine for $num1
if (!$cond_gruppi) $cond_gruppi = "idgruppo = '-1'";
else $cond_gruppi = substr($cond_gruppi,3);
$tutti_utenti = esegui_query("select * from $tablerelgruppi where $cond_gruppi order by idutente");
$cmp_idutente = "idutente";
} # fine else if (substr($lista_utenti_mess_interconn,0,1) == "g")
$num_tutti_utenti = numlin_query($tutti_utenti);
for ($num1 = 0 ; $num1 < $num_tutti_utenti ; $num1++) {
$idutenti = risul_query($tutti_utenti,$num1,$cmp_idutente);
$lista_utenti .= $idutenti.",";
} # fine for $num1
if (!$num_tutti_utenti and $lista_utenti == ",") $lista_utenti .= "1,";
} # fine if (strtolower($lista_utenti_mess_interconn) == "all" or substr($lista_utenti_mess_interconn,0,1) == "g")
else $lista_utenti = ",".aggslashdb($lista_utenti_mess_interconn).",";
$lista_utenti_vett = explode(",",substr($lista_utenti,1,-1));
$utente_visto = array();
$lista_utenti = ",";
for ($num1 = 0 ; $num1 < count($lista_utenti_vett) ; $num1++) {
if (!isset($utente_visto[$lista_utenti_vett[$num1]])) {
$utente_visto[$lista_utenti_vett[$num1]] = 1;
$lista_utenti .= $lista_utenti_vett[$num1].",";
} # fine if (!isset($utente_visto[$lista_utenti_vett[$num1]]))
} # fine for $num1
...
esegui_query("insert into $tablemessaggi (idmessaggi,tipo_messaggio,stato,idutenti,idutenti_visto,datavisione,mittente,testo,dati_messaggio1,dati_messaggio2,datainserimento) values ('$max_mess','intercon','ins','$lista_utenti','$lista_utenti','$datainserimento','".str_replace("_ic","",str_replace(",",", ",substr($lista_interconn,0,-1)))."','".aggslashdb(substr($testo,0,65000))."','$prenota_interconn','$anno','$datainserimento') ");
#esegui_query("insert into $tableinterconnessioni (idlocale,idremoto1,idremoto2,tipoid,nome_ic,anno,datainserimento,hostinserimento,utente_inserimento) values ('$max_mess','".aggslashdb($idremoto1)."','".aggslashdb($status)."','mess','$interconnection_name','$anno','$datainserimento','$HOSTNAME','$id_utente') ");
if (defined('C_NUM_HOSTING') and C_NUM_HOSTING != "" and defined('C_CARTELLA_VAR')) {
$cartella_var = C_CARTELLA_VAR;
if (substr($cartella_var,-1) != "/") $cartella_var .= "/";
if (@is_dir($cartella_var.C_NUM_HOSTING."/log")) {
$file_log = $cartella_var.C_NUM_HOSTING."/log/log".date("Ymd-His")."_mes_$max_mess.txt";
@scrivi_file($testo,$file_log);
} # fine if (@is_dir($cartella_var.C_NUM_HOSTING."/log"))
} # fine if (defined('C_NUM_HOSTING') and C_NUM_HOSTING != "" and...
} # fine function spedisci_mess_interconn
...
if (!empty($modo['ripristina'])) {
if (empty($modo['gia_canc'])) $modo['gia_canc'] = 1;
$modo['permanente'] = 0;
} # fine if (!empty($modo['ripristina']))
if (!empty($modo['gia_canc'])) {
$pcanc = "canc";
if (empty($modo['ripristina'])) $modo['permanente'] = 1;
...
$tableregole = $PHPR_TAB_PRE."regole".($anno - 1);
...
if (!empty($modo['ripristina'])) {
if (!empty($modo['controlla_app_chiusi'])) $altre_tab_lock = array($tableanni,$tableperiodi,$tableappartamenti,$tableclienti,$tableregole,$tablepersonalizza);
else $altre_tab_lock = array($tableanni,$tableperiodi,$tableappartamenti,$tableclienti,$tablepersonalizza);
} # fine if (!empty($modo['ripristina']))
else $altre_tab_lock = array($tableanni,$tableperiodi,$tableclienti,$tablepersonalizza);
$stile_data = stile_data($id_utente);
} # fine if ($dettagli_prenota or strcmp($cassa_pagamenti,""))
else {
if (!empty($modo['ripristina'])) {
if (!empty($modo['controlla_app_chiusi'])) $altre_tab_lock = array($tableanni,$tableperiodi,$tableappartamenti,$tableregole,$tablepersonalizza);
else $altre_tab_lock = array($tableanni,$tableperiodi,$tableappartamenti,$tablepersonalizza);
} # fine if (!empty($modo['ripristina']))
else $altre_tab_lock = array($tableanni,$tableperiodi,$tablepersonalizza);
} # fine else if ($dettagli_prenota or strcmp($cassa_pagamenti,""))
...
} # fine if ($num_dati_prenota)
elseif ($continua_canc != "NO") {
$testo .= mex("La prenotazione",$pag)." $id_prenota ".mex("era già stata cancellata",$pag)."!<br>";
$continua_canc = "NO";
} # fine elseif ($continua_canc != "NO")

if (!$modo['permanente'] and $continua_canc != "NO") {
if (empty($modo['ripristina'])) $pren_esist = esegui_query("select idprenota from $tableprenotacanc where idprenota = '$id_prenota' ");
else $pren_esist = esegui_query("select idprenota from $tableprenota where idprenota = '$id_prenota' ");
if (numlin_query($pren_esist)) {
if (empty($modo['ripristina'])) $modo['permanente'] = 1;
else $continua_canc = "NO";
} # fine if (numlin_query($pren_esist))
} # fine if (!$modo['permanente'] and $continua_canc != "NO")

if (!empty($modo['ripristina']) and $continua_canc != "NO") {
$d_appartamento = (string) risul_query($dati_prenota,0,'idappartamenti');
$app_esistente = esegui_query("select idappartamenti from $tableappartamenti where idappartamenti = '".aggslashdb($d_appartamento)."' ");
if (!numlin_query($app_esistente)) $continua_canc = "NO";
else {
$pren_presenti = esegui_query("select * from $tableprenota where idappartamenti = '".aggslashdb($d_appartamento)."' and iddatainizio <= '$d_idfineprenota' and iddatafine >= '$d_idinizioprenota' ");
if (numlin_query($pren_presenti)) $continua_canc = "NO";
if ($id_prenota_succ and $continua_canc != "NO") {
$dati_prenota_succ = esegui_query("select * from $tableprenota_succ where idprenota = '$id_prenota_succ' and idclienti != '0'");
$d_idinizioprenota_succ = risul_query($dati_prenota_succ,0,'iddatainizio');
$d_idfineprenota_succ = risul_query($dati_prenota_succ,0,'iddatafine');
$pren_presenti = esegui_query("select * from $tableprenota_succ where idappartamenti = '".aggslashdb($d_appartamento)."' and iddatainizio <= '$d_idfineprenota_succ' and iddatafine >= '$d_idinizioprenota_succ' ");
if (numlin_query($pren_presenti)) $continua_canc = "NO";
} # fine if ($id_prenota_succ and $continua_canc != "NO")
} # fine else if (!numlin_query($app_esistente))
if ($continua_canc == "NO") {
$testo .= "".mex("Le date della prenotazione",$pag)." <b>$id_prenota</b> ".mex("nell'appartamento",'unit.php')." <b class=\"colred\">$d_appartamento</b> ".mex("non sono attualmente disponibili",$pag)."!";
if ($dettagli_prenota) $testo .= " ".mex("Le date vanno liberate manualmente prima di poter ripristinare la prenotazione",$pag).".";
$testo .= "<br>";
} # fine if ($continua_canc == "NO")
elseif (!empty($modo['controlla_app_chiusi'])) {
$chiusure_app = esegui_query("select idregole from $tableregole where app_agenzia = '".aggslashdb($d_appartamento)."' and motivazione2 = 'x' and iddatainizio <= '$d_idfineprenota' and iddatafine >= '$d_idinizioprenota' ");
if (numlin_query($chiusure_app)) {
$continua_canc = "NO";
$testo .= "".mex("Le date della prenotazione",$pag)." <b>$id_prenota</b> ".mex("nell'appartamento",'unit.php')." <b class=\"colred\">$d_appartamento</b> ".mex("sono attualmente chiuse",$pag)."!";
if ($dettagli_prenota) $testo .= " ".mex("Le date vanno liberate manualmente prima di poter ripristinare la prenotazione",$pag).".";
$testo .= "<br>";
} # fine if (numlin_query($chiusure_app))
} # fine elseif (!empty($modo['controlla_app_chiusi']))
} # fine if (!empty($modo['ripristina']) and $continua_canc != "NO")
...
$metodo_pagamento = htmlspecialchars($metodo_pagamento,ENT_COMPAT);
...
if (!empty($id_pagamento)) esegui_query("update $tablesoldi set id_pagamento = '".aggslashdb(htmlspecialchars($id_pagamento,ENT_COMPAT))."' where idsoldi = '$idsoldi' ");
if (!empty($note_pagamento)) esegui_query("update $tablesoldi set note = '".aggslashdb(htmlspecialchars($note_pagamento,ENT_COMPAT))."' where idsoldi = '$idsoldi' ");
$stato_canc['idsoldi_pagato_canc'] = $idsoldi;
$stato_canc['pagato_canc'] = $d_pagato;
...
if (!empty($note_pagamento)) $nome_costo .= " - ".htmlspecialchars($note_pagamento,ENT_COMPAT);
...
$stato_canc['idcosti_pagato_canc'] = $idcosti;
} # fine if (strcmp($cassa_pagamenti,""))

} # fine if ($cancella_pagato and $priv_mod_pagato == "s")
else {
$soldi_prenota_cancellate = esegui_query("select soldi_prima from $tablesoldi where idsoldi = 1");
$soldi_prenota_cancellate = (float) risul_query($soldi_prenota_cancellate,0,'soldi_prima');
...
$d_prenota_comp = (string) risul_query($dati_prenota,0,'idprenota_compagna');
if ($d_prenota_comp) {
...
if (!empty($modo['ripristina'])) {
esegui_query("insert into $tableprenota select * from $tableprenotacanc where idprenota = '$id_prenota' ");
global $HOSTNAME;
$datainserimento = date("Y-m-d H:i:s",(time() + (C_DIFF_ORE * 3600)));
esegui_query("update $tableprenota set utente_inserimento = '".aggslashdb($id_utente)."', datainserimento = '$datainserimento', data_modifica = NULL, hostinserimento = '".aggslashdb(htmlspecialchars(fixstr($HOSTNAME),ENT_COMPAT))."' where idprenota = '$id_prenota' ");
$dati_canc = strstr($d_commento,">hd_del_res<");
$n_commento = str_replace($dati_canc,"",$d_commento);
if ($n_commento != $d_commento) esegui_query("update $tableprenota set commento = '".aggslashdb($n_commento)."' where idprenota = '$id_prenota' ");
$dati_canc = explode("|",$dati_canc);
if (!empty($dati_canc[2])) $testo .= "".mex("Le prenotazioni vicine alla prenotazione",$pag)." $id_prenota (".$dati_canc[2].") ".mex("vanno ripristinate manualemnte",$pag).".<br>";
if (!empty($dati_canc[3]) and !empty($dati_canc[5])) {
$nome_ic = $dati_canc[5];
$ic_esist = 0;
if (isset($modo['ic_ripristina'])) {
if ($modo['ic_ripristina'] == $nome_ic) $ic_esist = 1;
} # fine if (isset($modo['ic_ripristina']))
else {
$file_interconnessioni = C_DATI_PATH."/dati_interconnessioni.php";
if (@is_file($file_interconnessioni)) {
include($file_interconnessioni);
if (isset($ic_present) and is_array($ic_present) and isset($ic_present[$nome_ic]) and $ic_present[$nome_ic] == "SI") $ic_esist = 1;
} # fine if (@is_file($file_interconnessioni))
} # fine else if (isset($modo['ic_ripristina']))
if ($ic_esist) esegui_query("insert into $tableinterconnessioni (idlocale,idremoto1,idremoto2,tipoid,nome_ic,anno,datainserimento,hostinserimento,utente_inserimento) values ('$id_prenota','".aggslashdb($dati_canc[3])."','".aggslashdb($dati_canc[4])."','prenota','$nome_ic','$anno','$datainserimento','$HOSTNAME','$id_utente') ");
} # fine if (!empty($dati_canc[3]) and !empty($dati_canc[5]))
if ($d_pagato != 0) {
$soldi_prenota_cancellate = esegui_query("select soldi_prima from $tablesoldi where idsoldi = 1");
$soldi_prenota_cancellate = (float) risul_query($soldi_prenota_cancellate,0,'soldi_prima');
$soldi_prenota_cancellate = $soldi_prenota_cancellate - $d_pagato;
...
$datacanc = date("Y-m-d H:i:s",(time() + (C_DIFF_ORE * 3600)));
$tipo_commento = "hd_del_res";
$n_commento = "$nome_ut_canc|".date("Y-m-d H:i",(time() + (C_DIFF_ORE * 3600)))."|$d_prenota_comp";
$dati_interc = esegui_query("select * from $tableinterconnessioni where idlocale = '$id_prenota' and tipoid = 'prenota' and anno = '$anno' ");
if (numlin_query($dati_interc)) $n_commento .= "|".risul_query($dati_interc,0,'idremoto1')."|".risul_query($dati_interc,0,'idremoto2')."|".risul_query($dati_interc,0,'nome_ic');
else $n_commento .= "|||";
if (strstr($d_commento,">")) $n_commento = $d_commento.">".$tipo_commento."<".$n_commento;
else $n_commento = $d_commento.">>>".$tipo_commento."<".$n_commento;
esegui_query("update $tableprenotacanc set commento = '".aggslashdb($n_commento)."', data_modifica = '$datacanc' where idprenota = '$id_prenota' ");
} # fine else if (!empty($modo['ripristina']))
...
if (!empty($modo['ripristina'])) esegui_query("insert into $tableprenota_succ select * from $tableprenotacanc_succ where idprenota = '$id_prenota_succ' ");
else esegui_query("insert into $tableprenotacanc_succ select * from $tableprenota_succ where idprenota = '$id_prenota_succ' ");
} # fine else if ($modo['permanente'])
if (!empty($modo['gia_canc'])) esegui_query("delete from $tableprenotacanc_succ where idprenota = '$id_prenota'");
...
if (!empty($modo['ripristina'])) $parola_cancellata = mex("ripristinata",$pag);
else $parola_cancellata = mex("cancellata",$pag);
$testo_prec = "";
if ($dettagli_prenota) {
...
$data_inizio = esegui_query("select datainizio from $tableperiodi where idperiodi = '$d_idinizioprenota' ");
$data_fine = esegui_query("select datafine from $tableperiodi where idperiodi = '$d_idfineprenota' ");
...
$testo_prec = $testo;
$testo = mex("Prenotazione",$pag)." $id_prenota ".mex("a nome di",$pag)." <b>$cognome_cliente</b> ".mex("dal",$pag)." $data_inizio ".mex("al",$pag)." $data_fine <em>$parola_cancellata</em>";
} # fine if ($dettagli_prenota)
else $testo .= mex("Prenotazione",$pag)." $id_prenota $parola_cancellata";
if ($modo['permanente']) $testo .= " ".mex("in modo permanente",$pag);
$testo .= "!<br>$testo_prec";

} # fine if ($continua_canc != "NO")
$stato_canc['canc'] = $continua_canc;
...
function controlla_id_prenota(&$id_prenota,&$id_prenota_idpr,&$num_id_prenota,&$id_prenota_int,&$priv_mod_prenotazioni,$anno,$PHPR_TAB_PRE,$pcanc="",$iddf=0) {
...
$id_prenota = aggslashdb(htmlspecialchars((string) $id_prenota));
...
if ($iddf) $id_prenota_idpr['iddf'] = array();
...
if ($priv_mod_prenotazioni == "g" and empty($utenti_gruppi[$utente_inserimento])) $priv_mod_prenotazioni = "n";
} # fine if ($priv_mod_prenotazioni == "p" or $priv_mod_prenotazioni == "g")
$d_prenota = esegui_query("select iddatainizio,iddatafine,datainserimento from $tableprenota where idprenota = '$id_prenota'");
if (!numlin_query($d_prenota) or controlla_num($id_prenota) != "SI") $priv_mod_prenotazioni = "n";
else {
if ($iddf) $id_prenota_idpr['iddf'][$id_prenota] = risul_query($d_prenota,0,'iddatafine');
...
$tableprenota_orig = $PHPR_TAB_PRE."prenota".$pcanc.($anno + 1);
...
$tableprenota_prec = $PHPR_TAB_PRE."prenota".$pcanc.($anno - 1);
...
$d_costo_tariffa = (float) fixset($d_tariffa[1]);
...
$d_sconto = (float) risul_query($dati_prenota_modifica,0,'sconto');
...
$d_prezzo_costo_agg[$numca] = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$d_id_data_inizio,$d_id_data_fine,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$d_costo_tariffa,$d_tariffesettimanali,($d_costo_tariffa + $d_costo_agg_tot - $d_sconto),$d_caparra,$d_num_persone,fixset($dati_cap[$numca]['cat_pers']),0,$costo_escludi_perc);
$d_costo_agg_tot = (float) $d_costo_agg_tot + $d_prezzo_costo_agg[$numca];
if ($dati_cap[$numca]['escludi_tot_perc'] == "s") $costo_escludi_perc = (float) $costo_escludi_perc + $d_prezzo_costo_agg[$numca];
} # fine for $numca
...
for ($num_idpr = 0 ; $num_idpr < $num_id_prenota ; $num_idpr++) {
$id_prenota = $id_prenota_idpr[$num_idpr];
if ($tipo_sposta == "fisso" and strcmp((string) $n_appartamento_idpr[$id_prenota],"")) {
$n_assegnazioneapp_idpr[$id_prenota] = "k";
$n_lista_app_idpr[$id_prenota] = $n_appartamento_idpr[$id_prenota];
} # fine if ($tipo_sposta == "fisso" and strcmp((string) $n_appartamento_idpr[$id_prenota],"")) 
$n_sconto = explode("#>&",fixstr($n_sconto_caparra_idpr[$id_prenota]));
if (strcmp(fixstr($n_sconto[1]),"")) $n_caparra = (float) $n_sconto[1];
else $n_caparra = "";
if (strcmp(fixstr($n_sconto[2]),"")) $n_commissioni = (float) $n_sconto[2];
else $n_commissioni = "";
$n_valuta_tariffa = fixstr($n_sconto[3]);
if (strcmp((string) $n_sconto[0],"")) $n_sconto = (float) $n_sconto[0];
else $n_sconto = "";
...
if (function_exists('gzcompress')) $cat_persone_idpr = unserialize(gzuncompress(base64_decode(risul_query($dati_transazione,0,'dati_transazione21'))));
else $cat_persone_idpr = unserialize(risul_query($dati_transazione,0,'dati_transazione21'));
$dati_extra = unserialize(risul_query($dati_transazione,0,'dati_transazione22'));
...
$n_costo_tot = fixset($n_costo_tot_idpr[$id_prenota]);
...
$n_nometipotariffa = fixset($n_nometipotariffa_idpr[$id_prenota]);
$n_costo_tariffa = fixset($n_costo_tariffa_idpr[$id_prenota]);
$costi_aggiuntivi_modificati_int = fixset($costi_aggiuntivi_modificati_int_idpr[$id_prenota]);
$costi_aggiuntivi_da_inserire_int = fixset($costi_aggiuntivi_da_inserire_int_idpr[$id_prenota]);
$n_sconto = $n_sconto_idpr[$id_prenota];
$n_caparra = $n_caparra_idpr[$id_prenota];
$n_commissioni = $n_commissioni_idpr[$id_prenota];
$n_tariffesettimanali = fixset($n_tariffesettimanali_idpr[$id_prenota]);
$prenota_comp = $prenota_comp_idpr[$id_prenota];
$n_pagato = fixset($n_pagato_idpr[$id_prenota]);
$n_confermato = fixset($n_confermato_idpr[$id_prenota]);
...
} # fine if ($tipo_n_app == "aggiungi" and strcmp((string) $n_appartamento,"" and $assegnazioneapp == "k")
...
if ($trovata_tariffa == "NO") $inserire = "NO";
else {
if (($attiva_tariffe_consentite == "s" and (!isset($tariffe_consentite_vett[substr($n_tipotariffa,7)]) or $tariffe_consentite_vett[substr($n_tipotariffa,7)] != "SI") and $n_nometipotariffa != $d_nome_tariffa_idpr[$id_prenota])) $inserire = "NO";
...
if ($cat_persone[$num2]['n_plur'] != $d_cat_persone[$ncp]['n_plur'] or $cat_persone[$num2]['perc'] != $d_cat_persone[$ncp]['perc'] or fixset($d_cat_persone[$cat_pers]['esist']) == ($ncp + 1)) $inserire = "NO";
...
if ($cat_persone['int']) $cat_persone['int'] = fixset($cat_persone['arrotond']).$cat_persone['int'];
...
else $caparra = fixset($d_caparra_idpr[$id_prenota]);
...
if (isset($dati_ca[$num_costo]['id']) and $dati_ca[$num_costo]['id'] == $dati_cap[$numca]['idntariffe'] and $dati_cap[$numca]['nome'] == $dati_ca[$num_costo]['nome'] and $dati_ca[$num_costo]['numlimite']) $num_limite = (string) $dati_ca[$num_costo]['numlimite'];
...
$valuta_trovata = 0;
if (${"valuta_pagamento".$id_prenota} == $d_valuta_caparra_idpr[$id_prenota] and (float) $d_pagato_idpr[$id_prenota] < (float) $caparra and $n_pagato > $d_pagato_idpr[$id_prenota]) {
...
#${"metodo_pagamento".$id_prenota} = htmlspecialchars(${"metodo_pagamento".$id_prenota},ENT_COMPAT);
...
$n_origine_prenota = htmlspecialchars($n_origine_prenota,ENT_COMPAT);
...
$n_commento = htmlspecialchars($n_commento,ENT_COMPAT);
} # fine if ($n_commento != "")
...
if (!empty($inizio_prenota_id[$idprenota_cambiata])) {
$applicare_cambio[$num1] = "SI";
if (empty($app_assegnabili_id[$idprenota_cambiata])) $applicare_cambio[$num1] = "NO";
if (!empty($app_assegnabili_id[$idprenota_cambiata]) and $app_assegnabili_id[$idprenota_cambiata] != "v") {
...
} # fine if (!empty($app_assegnabili_id[$idprenota_cambiata]) and...
...
} # fine if (!empty($inizio_prenota_id[$idprenota_cambiata]))
$num1++;
} # fine for $num1
for ($num1 = 0 ; $num1 < $num_spostamenti ; $num1++) {
$idprenota_cambiata = $spostamenti[$num1];
...
if ($n_nometipotariffa != "") {
$moltiplica_tar = substr($n_costo_tariffa,-1);
if ($moltiplica_tar == "s") $n_costo_tariffa = substr($n_costo_tariffa,0,-1);
if ($moltiplica_tar == "p") $n_costo_tariffa = substr($n_costo_tariffa,0,-1)."#@&p";
...
esegui_query("delete from $tablecostiprenota_orig where idprenota = '$id_prenota_orig'");
if ($prenota_in_anno_succ) $id_prenota_prec_corr = $prenota_in_anno_succ;
else $id_prenota_prec_corr = $id_prenota_prec;
$costiprenota_prec = esegui_query("select * from $tablecostiprenota_prec where idprenota = '$id_prenota_prec_corr'");
...
if ($prenota_in_anno_succ) esegui_query("update $tableprenota_orig set caparra = '$n_caparra' where idprenota = '$id_prenota_orig' ");
} # fine if (strcmp((string) $n_caparra,""))
...
if (strcmp(fixstr($d_valuta_tariffa_idpr[$id_prenota]),"")) $valuta_prenota = $d_valuta_tariffa_idpr[$id_prenota]."<".$d_cambio_tariffa_idpr[$id_prenota]."<".$d_arrotond_tariffa_idpr[$id_prenota].">";
else $valuta_prenota = ">";
...
elseif (strcmp(fixstr($d_valuta_caparra_idpr[$id_prenota]),"")) $valuta_prenota .= $d_valuta_caparra_idpr[$id_prenota]."<".$d_cambio_caparra_idpr[$id_prenota]."<".$d_arrotond_caparra_idpr[$id_prenota];

if ($aggiorna_valuta) {
if ($valuta_prenota == ">") {
esegui_query("update $tableprenota set valuta = NULL where idprenota = '$id_prenota' ");
if ($prenota_in_anno_succ) esegui_query("update $tableprenota_orig set valuta = NULL where idprenota = '$id_prenota_orig' ");
} # fine if ($valuta_prenota == ">")
else {
esegui_query("update $tableprenota set valuta = '".aggslashdb($valuta_prenota)."' where idprenota = '$id_prenota' ");
if ($prenota_in_anno_succ) esegui_query("update $tableprenota_orig set valuta = '".aggslashdb($valuta_prenota)."' where idprenota = '$id_prenota_orig' ");
} # fine else if ($valuta_prenota == ">")
...
if ($prenota_in_anno_succ) esegui_query("update $tableprenota_orig set commissioni = '$n_commissioni' where idprenota = '$id_prenota_orig' ");
...
if ($prenota_in_anno_succ) esegui_query("update $tableprenota_orig set conferma = '$n_confermato' where idprenota = '$id_prenota_orig' ");
...
# non aggiorno la data di modifica se la prenotazione è cancellata (campo usato per data di cancellazione)
if (strstr($tableprenota,"canc")) esegui_query("update $tableprenota set tariffa_tot = '$n_costo_tot' where idprenota = '$id_prenota' ");
else esegui_query("update $tableprenota set tariffa_tot = '$n_costo_tot', data_modifica = '$adesso' where idprenota = '$id_prenota' ");
if ($prenota_in_anno_succ) {
if (strstr($tableprenota,"canc")) esegui_query("update $tableprenota_orig set tariffa_tot = '$n_costo_tot' where idprenota = '$id_prenota_orig' ");
else esegui_query("update $tableprenota_orig set tariffa_tot = '$n_costo_tot', data_modifica = '$adesso' where idprenota = '$id_prenota_orig' ");
} # fine if ($prenota_in_anno_succ)
...
if ($valuta) esegui_query("update $tablesoldi set valuta = '".aggslashdb($valuta)."', saldo_valuta = '".aggslashdb($saldo_valuta)."' where idsoldi = '$idsoldi' ");
if (!empty(${"id_pagamento".$id_prenota})) esegui_query("update $tablesoldi set id_pagamento = '".aggslashdb(htmlspecialchars(${"id_pagamento".$id_prenota},ENT_COMPAT))."' where idsoldi = '$idsoldi' ");
if (!empty(${"note_pagamento".$id_prenota})) esegui_query("update $tablesoldi set note = '".aggslashdb(htmlspecialchars(${"note_pagamento".$id_prenota},ENT_COMPAT))."' where idsoldi = '$idsoldi' ");

if ($pcanc) {
$soldi_prenota_cancellate = esegui_query("select soldi_prima from $tablesoldi where idsoldi = 1");
$soldi_prenota_cancellate = (float) risul_query($soldi_prenota_cancellate,0,'soldi_prima');
$soldi_prenota_cancellate = $soldi_prenota_cancellate + (float) $saldo;
esegui_query("update $tablesoldi set soldi_prima = '$soldi_prenota_cancellate' where idsoldi = 1");
} # fine if ($pcanc)
unlock_tabelle($tabelle_lock);
...
$cognome_saldo = risul_query($cognome_saldo,0,'cognome');
$nome_costo = "".mex("Prenotazione",$pag)." $id_prenota ".mex("dal",$pag)." $data_inizio_costo ".mex("al",$pag)." $data_fine_costo ".mex("per",$pag)." $cognome_saldo";
if (!empty(${"note_pagamento".$id_prenota})) $nome_costo .= " - ".htmlspecialchars(${"note_pagamento".$id_prenota},ENT_COMPAT);
$idcosti = esegui_query("select max(idcosti) from $tablecosti");
$idcosti = risul_query($idcosti,0,0) + 1;
if ($cassa_pagamenti == "#1#") $nome_cassa = "";
else $nome_cassa = $cassa_pagamenti;
...
if ($valuta) esegui_query("update $tablecosti set valuta = '".aggslashdb($valuta)."', costo_valuta = '".aggslashdb($saldo_valuta)."' where idcosti = '$idcosti' ");
if (!empty(${"id_pagamento".$id_prenota})) esegui_query("update $tablecosti set id_pagamento = '".aggslashdb(htmlspecialchars(${"id_pagamento".$id_prenota},ENT_COMPAT))."' where idcosti = '$idcosti' ");
unlock_tabelle($tabelle_lock);
} # fine if (strcmp($cassa_pagamenti,""))
...
if ($dati_extra['prenota_interconn']) {
$tablerelgruppi = $PHPR_TAB_PRE."relgruppi";
$tablemessaggi = $PHPR_TAB_PRE."messaggi";
$testo = "<br><span class=\"colwarn\">".mex("Attenzione",$pag)."</span>: ".mex("rimpicciolire manualmente una prenotazione proveniente dalle interconnessioni",$pag)." (<em>".$dati_extra['prenota_interconn']."</em>)".", ".mex("senza aspettare che venga modificata dall'interconnessione",$pag).",  <span class=\"colred\">".mex("può creare overbooking",$pag)."</span>.<br><br>";
spedisci_mess_interconn($testo,$dati_extra['prenota_interconn'],$dati_extra['lista_interconn'],$dati_extra['lista_utenti_mess_interconn'],$anno,$tableutenti,$tablerelgruppi,$tablemessaggi);
} # fine if ($dati_extra['prenota_interconn'])
