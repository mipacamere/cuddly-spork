...
$link_pgsql = $numconnessione;
pg_exec($numconnessione,"set datestyle to 'iso'");
...
$riga_corr = str_replace("<riga>","<riga@%&@>",$riga_corr);
$scritto = fwrite($file,"<riga>$riga_corr</riga>
");
if (!$scritto) { echo "Write ERROR<br>"; break; }
fflush($file);
} # fine for $num1
...
$scritto = fwrite($file,$linee_file[$num1]);
if (!$scritto) { echo "Write ERROR<br>"; break; }
} # fine for $num1
...
if ($tipo_db == "postgresql") $risul = pg_exec($numconnessione,$query);
if ($tipo_db == "mysql") $risul = mysql_query($query);
if ($tipo_db == "mysqli") $risul = mysqli_query($numconnessione,$query);
if ($tipo_db == "sqlite") {
...
global $numconnessione;
if ($tipo_db == "postgresql") pg_exec($numconnessione,"create index $nome on $tabella ($colonne)");
if ($tipo_db == "mysql") mysql_query("alter table $tabella add index $nome ($colonne)");
if ($tipo_db == "mysqli") $risul = mysqli_query($numconnessione,"alter table $tabella add index $nome ($colonne)");
if ($tipo_db == "sqlite") {
...
if (function_exists('pg_escape_string')) $stringa = pg_escape_string($numconnessione,$stringa);
...
if ($tipo_db == "sqlite") $stringa = $numconnessione->escapeString($stringa);
...
if (((defined('C_CREA_NUOVI_APP') and C_CREA_NUOVI_APP == "NO") or (defined('C_MASSIMO_NUM_APP') and C_MASSIMO_NUM_APP != 0)) and $nome_tabella == "appartamenti" and $linee_inserite_in_tabella[$nome_tabella] >= $num_appartamenti_prec) { $file_coerente = "NO"; $num_err .= "#6"; echo "<br>".mex("Il file di backup contiene più di",$pag)." $num_appartamenti_prec ".mex("appartamenti",'unit.php').".<br><br>"; }
...
if ($linea == "<righetabella>" and $leggendo_colonne == "NO") {
$leggendo_righe = "SI";
$linee_inserite_in_tabella[$nome_tabella] = 0;
} # fine if ($linea == "<righetabella>" and $leggendo_colonne == "NO")
...
if ($linea != "" and $linea != "<?php" and $linea != "define('C_VERSIONE_ATTUALE',".C_PHPR_VERSIONE_NUM.");" and preg_replace("/define\('C_DIFF_ORE',-?[0-9]{1,2}\);/","",$linea) != "" and preg_replace("|define\('C_DIFF_TZ','[A-Z][A-Za-z_/]*'\);|","",$linea) != "" and preg_replace("/define\('C_MIN_SESSIONE',-?[0-9]{1,999}\);/","",$linea) != "" and preg_replace("/define\('C_USA_COOKIES',-?[0-9]{1,1}\);/","",$linea) != "" and $linea != "?>") { $file_coerente = "NO"; $num_err .= "#34"; }
...
if ($nome_file == "./dati/lingua.php" and $linea != "" and $linea != "<?php" and $linea != "?>" and preg_replace("/\\\$lingua\\[[0-9]{1,8}\\] = \"[a-z-]{1,7}\";/","",$linea) != "") { $file_coerente = "NO"; $num_err .= "#42"; }
...
if (preg_replace("/if +\\(defined\\('C_[A-Z][A-Z_]*[A-Z]_IC'\\)\\) +\\\$ic_(org|password|user)[a-z_]+[0-9a-z_]*_ic += +C_[A-Z][A-Z_]*[A-Z]_IC;/","",$linea) == "") $linea_trovata = "SI";
if (preg_replace("/else +\\\$ic_(org|password|user)[a-z_]+[0-9a-z_]*_ic += +\"\";/","",$linea) == "") $linea_trovata = "SI";
...
if (strcmp($N_PHPR_LOAD_EXT,"") and $N_PHPR_LOAD_EXT != "SI") $N_PHPR_LOAD_EXT = "NO";
if (strcmp($N_PHPR_DB_TYPE,"") and $N_PHPR_DB_TYPE != "mysqli" and $N_PHPR_DB_TYPE != "postgresql" and $N_PHPR_DB_TYPE != "sqlite") $N_PHPR_DB_TYPE = "mysql";
if ($N_PHPR_DB_TYPE == "mysql" and @function_exists('mysqli_connect')) $N_PHPR_DB_TYPE = "mysqli";
$N_PHPR_DB_HOST = substr(preg_replace("/[^a-z0-9-.]/i","",$N_PHPR_DB_HOST),0,253);
if ($N_PHPR_DB_PORT and (controlla_num_pos($N_PHPR_DB_PORT) != "SI" or $N_PHPR_DB_PORT > "65535")) $N_PHPR_DB_PORT = "";
$N_PHPR_DB_USER = substr(trim(str_replace(" ","",$N_PHPR_DB_USER)),0,128);
$N_PHPR_DB_PASS = substr(trim(str_replace(" ","",$N_PHPR_DB_PASS)),0,128);
$N_PHPR_TAB_PRE = substr(preg_replace("/[^a-z0-9_]/i","",$N_PHPR_TAB_PRE),0,12);
...
if (!$mantieni_anni or !empty($campo_anno_esistente[$anno_cancella])) {
...
} # fine if (!$mantieni_anni or !empty($campo_anno_esistente[$anno_cancella]))
elseif ($mantieni_anni) $anno_mantieni[$anno_cancella] = 1;
} # fine for $num1
if ($mantieni_anni and isset($anno_mantieni)) {
...
} # fine if ($mantieni_anni and isset($anno_mantieni))
...
$num_col =  count($riga);
for ($num2 = 0 ; $num2 < $num_col ; $num2++) {
if ($nome_tabella == "clienti" and $nome_colonna[($num2+1)] == "cognome" and (string) $riga[$num2] == "") $riga[$num2] = 'surname';
aggslashdb2($riga[$num2],$N_PHPR_DB_TYPE);
if ((string) $riga[$num2] != "") $query .= $nome_colonna[($num2+1)].",";
} # fine for $num2
...
if (!empty($anno_mantieni[$anno_vecchio])) {
...
} # fine if (!empty($anno_mantieni[$anno_vecchio]))
} # fine for $num1
...
if (!empty($anno_mantieni[$anno_priv])) {
...
} # fine if (!empty($anno_mantieni[str_replace("giorno_vedi_ini_sett","",$idpersonalizza)]))
} # fine for $num1
...
esegui_query2("delete from ".$N_PHPR_TAB_PRE."personalizza where idpersonalizza = 'priv_key_cc' or idpersonalizza = 'cert_cc' or idpersonalizza = 'gest_cvc' ",$N_PHPR_DB_TYPE);
esegui_query2("delete from ".$N_PHPR_TAB_PRE."relclienti where tipo = 'cc' ",$N_PHPR_DB_TYPE);
...
if ($riga[$num_colonna['tipo']] == "img_inln") $contr_agg_iim .= $riga[$num_colonna['testo']];
...
$max_iim = esegui_query("select max(numero) from ".$PHPR_TAB_PRE."contratti where tipo = 'file_iim'");
if (numlin_query($max_iim) != 0) $max_iim = risul_query($max_iim,0,0);
else $max_iim = 0;
if (!$max_iim) $max_iim = 0;
...
else $min_var_vecchio = fixset($var_vecchie_ord[fixstr($min_var_vecchio[$contr_agg_var])]);
...
if (substr($tipo_contr,0,5) == "contr" or $tipo_contr == "opzeml" or $tipo_contr == "oggetto" or $tipo_contr == "allegato" or $tipo_contr == "img_inln" or $tipo_contr == "pag_esc" or $tipo_contr == "headhtm" or $tipo_contr == "foothtm" or $tipo_contr == "datdownl" or $tipo_contr == "dir" or substr($tipo_contr,0,4) == "mln_" or $tipo_contr == "impor_vc" or $tipo_contr == "num_prog" or $tipo_contr == "nomefile" or $tipo_contr == "autosalv" or $tipo_contr == "compress" or $tipo_contr == "incr_np" or $tipo_contr == "api" or $tipo_contr == "formati") {
...
if ($tipo_contr == "img_inln") {
$vecchia_iim = $riga[$num_colonna['testo']];
if (strcmp((string) $vecchia_iim,"")) {
$iimg_esist = explode(",",$vecchia_iim);
$d_num_iimg = (count($iimg_esist) - 1);
for ($num2 = 1 ; $num2 < $d_num_iimg ; $num2++) {
$num_iim = str_replace(strstr($iimg_esist[$num2],":"),"",$iimg_esist[$num2]);
if (empty($nuovo_num_iim[$num_iim])) $nuovo_num_iim[$num_iim] = $max_iim + $num_iim;
$vecchia_iim = str_replace(",$num_iim:",",".$nuovo_num_iim[$num_iim].":",$vecchia_iim);
} # fine for $num2
$riga[$num_colonna['testo']] = $vecchia_iim;
} # fine if (strcmp((string) $vecchia_iim,""))
} # fine if ($tipo_contr == "img_inln")
...
$azione_vecchia[1] = fixset($nuovo_num_var[$azione_vecchia[1]]);
...
if ($azione_vecchia[0] == "trunc") $azione_vecchia[1] = fixset($nuovo_num_var[$azione_vecchia[1]]);
...
if ($azione_vecchia[0] == "unset") $azione_vecchia[1] = fixset($nuovo_num_var[$azione_vecchia[1]]);
if ($azione_vecchia[0] == "array") {
$azione_vecchia[1] = fixset($nuovo_num_var[$azione_vecchia[1]]);
if ($azione_vecchia[2] == "cop") $azione_vecchia[3] = fixset($nuovo_num_var[$azione_vecchia[3]]);
...
$all_esist = esegui_query("select numero from ".$PHPR_TAB_PRE."contratti where tipo = 'file_all' and testo = '".aggslashdb($riga[$num_colonna['testo']])."'");
if (numlin_query($all_esist)) {
$inserire_riga = "NO";
$aggiorna_all[$nuovo_num_all[$vecchio_all]] = risul_query($all_esist,0,'numero');
} # fine if (numlin_query($all_esist))
else $riga[$num_colonna['numero']] = $nuovo_num_all[$vecchio_all];
} # fine if (!$contr_agg or strstr($contr_agg_all,",$vecchio_all,") or...
else $inserire_riga = "NO";
} # fine if ($tipo_contr == "file_all")
if ($tipo_contr == "file_iim") {
$vecchia_iim = $riga[$num_colonna['numero']];
if (!$contr_agg or strstr($contr_agg_iim,",$vecchia_iim:")) {
if (empty($nuovo_num_iim[$vecchia_iim])) $nuovo_num_iim[$vecchia_iim] = $max_iim + $vecchia_iim;
$fiim_esist = esegui_query("select numero from ".$PHPR_TAB_PRE."contratti where tipo = 'file_iim' and testo = '".aggslashdb($riga[$num_colonna['testo']])."'");
if (numlin_query($fiim_esist)) {
$inserire_riga = "NO";
$aggiorna_iim[$nuovo_num_iim[$vecchia_iim]] = risul_query($fiim_esist,0,'numero');
} # fine if (numlin_query($fiim_esist))
else $riga[$num_colonna['numero']] = $nuovo_num_iim[$vecchia_iim];
} # fine if (!$contr_agg or strstr($contr_agg_iim,",$vecchia_iim:"))
else $inserire_riga = "NO";
} # fine if ($tipo_contr == "file_iim")
...
reset($aggiorna_all);
foreach ($aggiorna_all as $num_nuovo => $num_esistente) {
$all_agg = esegui_query("select numero,testo from $PHPR_TAB_PRE"."contratti where tipo = 'allegato' and (testo $LIKE '%,".aggslashdb($num_nuovo).",%' or testo $LIKE '%:".aggslashdb($num_nuovo).",%' or testo $LIKE '%:".aggslashdb($num_nuovo).">%') ");
$nl_all_agg = numlin_query($all_agg);
for ($num1 = 0 ; $num1 < $nl_all_agg ; $num1++) {
$num = risul_query($all_agg,$num1,'numero');
$txt = (string) risul_query($all_agg,$num1,'testo');
$txt = str_replace(",$num_nuovo,",",$num_esistente,",$txt);
$txt = str_replace(":$num_nuovo,",":$num_esistente,",$txt);
$txt = str_replace(":$num_nuovo>",":$num_esistente>",$txt);
esegui_query("update $PHPR_TAB_PRE"."contratti set testo = '".aggslashdb($txt)."' where numero = '$num' and tipo = 'allegato' ");
} # fine for $num1
} # fine foreach ($aggiorna_all as $num_nuovo => $num_esistente)
reset($aggiorna_iim);
foreach ($aggiorna_iim as $num_nuovo => $num_esistente) {
$iimg_agg = esegui_query("select numero,testo from $PHPR_TAB_PRE"."contratti where tipo = 'img_inln' and testo $LIKE '%,".aggslashdb($num_nuovo).":%' ");
$nl_iimg_agg = numlin_query($iimg_agg);
for ($num1 = 0 ; $num1 < $nl_iimg_agg ; $num1++) {
$num = risul_query($iimg_agg,$num1,'numero');
$txt = (string) risul_query($iimg_agg,$num1,'testo');
esegui_query("update $PHPR_TAB_PRE"."contratti set testo = '".aggslashdb(str_replace(",$num_nuovo:",",$num_esistente:",$txt))."' where numero = '$num' and tipo = 'img_inln' ");
} # fine for $num1
} # fine foreach ($aggiorna_iim as $num_nuovo => $num_esistente)
unlock_tabelle($tabelle_lock);
...
if ((!$contr_agg or $nome_contr[0] == $contr_agg) and isset($nuovo_num_contr[$nome_contr[0]])) $n_nomi_contratti .= "#@&".$nuovo_num_contr[$nome_contr[0]]."#?&".$nome_contr[1];
} # fine for $num1
