...
if ($aggiungi_tipologie != "SI" or strlen((string) $num_tipologie) > 3) $num_tipologie = 1;

$costi_elimina_promo = array();
$costi_aggiungi_promo = array();
if ($num_codici_promo and strcmp(fixstr($codpromo),"")) {
...
} # fine if ($num_codici_promo and strcmp(fixstr($codpromo),""))
...
$dati_app = prendi_dati_app($tableappartamenti);
$numappartamenti = $dati_app['totapp'];
$dati_app_pos = null;
...
for ($n_t = 1 ; $n_t <= $num_tipologie ; $n_t++) if (empty(${"numcostiagg".$n_t}) or controlla_num_pos(${"numcostiagg".$n_t}) == "NO" or strlen((string) ${"numcostiagg".$n_t}) > 3) ${"numcostiagg".$n_t} = 0;
reset($costi_aggiungi_promo);
...
if ($chiedi_num_persone == "SI" and isset(${"idcostoagg".$numca."_".$n_t}) and fixset($dati_ca[fixset($dati_ca['id'][${"idcostoagg".$numca."_".$n_t}])]['mostra_numpers']) == "s" and !empty(${"nummoltiplica_ca".$numca."_".$n_t})) ${"costoagg".$numca."_".$n_t} = "SI";
...
$numpersone_nr = array();
$posti_mancanti = array();
$cat_persone = array();
$cat_persone_nr = array();
$cat_numpers_orig = array();
$app_richiesti_nr = array();
...
$num_app_tipo_richiesti = (int) ${"num_app_tipo_richiesti".$n_t};
$num_app_tipo_richiesti_orig[$n_t] = $num_app_tipo_richiesti;
if (controlla_num_pos(${"num_app_tipo_richiesti".$n_t}) == "NO" or !$num_app_tipo_richiesti or strlen((string) $num_app_tipo_richiesti) > 3 or ($max_num_app_tipologia != 0 and $num_app_tipo_richiesti > $max_num_app_tipologia)) {
$verificare = "NO";
echo "$fr_Il_numero_di $fr_appartamenti $fr_richiesto_e_sbagliato";
if ($num_tipologie > 1) echo " ($n_t)";
echo ".<br>";
} # fine if (controlla_num_pos(${"num_app_tipo_richiesti".$n_t}) == "NO" or...
else $num_app_richiesti += $num_app_tipo_richiesti;

unset(${"app_richiesti".$n_t});
$numpersone_nr[$n_t] = array();
$cat_persone_nr[$n_t] = array();
$app_richiesti_nr[$n_t] = array();
$posti_mancanti[$n_t] = 0;
...
$numpersone = ceil((float) $numpersone / (float) $dati_r2['napp'][$tipotariffa]);
$diff_persone = ((int) $numpersone * (int) $dati_r2['napp'][$tipotariffa]) - (int) $numpersone_orig_nt[$n_t];

# Se ci sono appartamenti tra quelli multipli che non possono ospitare il numero di persone medio metto il numero di persone ospitabile da ogni appartamento in $numpersone_nr
$napp_posti = 0;
for ($num1 = 0 ; $num1 < $num_app ; $num1++) {
if (isset($dati_app['maxocc'][$vett_app[$num1]])) {
if ($dati_app['maxocc'][$vett_app[$num1]]) $maxoccupanti = $dati_app['maxocc'][$vett_app[$num1]];
else $maxoccupanti = $numpersone_orig_nt[$n_t];
if ($maxoccupanti >= $numpersone) $napp_posti++;
} # fine if (isset($dati_app['maxocc'][$vett_app[$num1]]))
} # fine if for $num1
if ($napp_posti < $dati_r2['napp'][$tipotariffa]) {
$numpersone_nr[$n_t] = trova_numpersone_per_app($lista_app_reg[$n_t],$posti_mancanti[$n_t],$cat_pers_letto,$cat_persone[$n_t],$dati_app_pos,$dati_app,$dati_r2['napp'][$tipotariffa],$numpersone_orig_nt[$n_t],$dati_cat_pers);
$vett_app = explode(",",$lista_app_reg[$n_t]);
$num_app = count($vett_app);
$posti_mancanti['cat_pers_letto'][$n_t] = $cat_pers_letto;
if ($cat_pers_letto >= 0) {
$cat_numpers_orig[$n_t][$cat_pers_letto] = ${"cat$cat_pers_letto"."_numpers".$n_t};
${"cat$cat_pers_letto"."_numpers$n_t"} = $cat_persone[$n_t][$cat_persone[$n_t]['ord'][$cat_pers_letto]]['molt'];
} # fine if ($cat_pers_letto >= 0)
else $cat_pers_letto = "";
} # fine if ($napp_posti < $dati_r2['napp'][$tipotariffa])

} # fine if ($numpersone)
$cat_persone_copia = $cat_persone[$n_t];
for ($num1 = 0 ; $num1 < $dati_r2['napp'][$tipotariffa] ; $num1++) {
if ($diff_persone and ($num1 + $diff_persone) > $dati_r2['napp'][$tipotariffa] and !isset($numpersone_nr[$n_t][$num1])) $numpersone_nr[$n_t][$num1] = $numpersone - 1;
if (isset($numpersone_nr[$n_t][$num1])) $numpersone_cp = $numpersone_nr[$n_t][$num1];
...
$posti_mancanti[$n_t] = ($posti_mancanti[$n_t] * $num_app_tipo_richiesti);
...
if (!empty($dati_r2['napp']['v'][$tipotariffa])) $interrompi_vicine_ogni[$n_t] = $dati_r2['napp'][$tipotariffa];
} # fine if ($num_app_tipo_richiesti > 1)
else $num_app_tipo_richiesti = $dati_r2['napp'][$tipotariffa];
if (!empty($dati_r2['napp']['v'][$tipotariffa])) {
$prenota_vicine_nt[$n_t] = 1;
$app_richiesti[',vicini,'] = "SI";
} # fine if (!empty($dati_r2['napp']['v'][$tipotariffa]))
...
$num_cal = fixset($dati_ca['id'][$costo_aggiungi_letti]);
if ($num_cal and $dati_ca[$num_cal]['letto'] == "s" and $dati_ca[$num_cal]['numsett'] != "c" and $dati_ca[$num_cal]['combina'] != "s") {
$max_capacita = 0;
for ($num1 = 0 ; $num1 < $numappartamenti ; $num1++) {
$idapp = $dati_app['posizione'][$num1];
if (((empty(${"app_richiesti".$n_t}) or (isset(${"app_richiesti".$n_t}[$idapp]) and ${"app_richiesti".$n_t}[$idapp] == "SI")) and !$app_regola2_predef) or (str_replace(",$idapp,","",",$app_regola2_predef,") != ",$app_regola2_predef," and $app_regola2_predef)) {
$maxoccupanti = $dati_app['maxocc'][$idapp];
if (!$maxoccupanti) $max_capacita = $numpersone;
elseif ($maxoccupanti > $max_capacita) $max_capacita = $maxoccupanti;
} # fine if (((empty(${"app_richiesti".$n_t}) or (isset(${"app_richiesti".$n_t}[$idapp]) and ${"app_richiesti".$n_t}[$idapp] == "SI")) and !$app_regola2_predef) or...
} # fine for $num1
if ($dati_ca[$num_cal]['moltiplica'] != "c") $max_num_aggiungi_letti = 1;
if (($numpersone > $max_capacita and ((int) $numpersone - (int) $max_capacita) <= $max_num_aggiungi_letti) or ($posti_mancanti[$n_t] and $posti_mancanti[$n_t] <= ($max_num_aggiungi_letti * $dati_r2['napp'][$tipotariffa]) and $posti_mancanti['cat_pers_letto'][$n_t] >= -1)) {
...
$num_aggiungi_letti = ((int) $numpersone - (int) $max_capacita);
if ($dati_cat_pers['num'] and !$posti_mancanti[$n_t]) {
$cat_pers_letto = "";
$perc_cat_pers_letto = "";
...
} # fine if ($dati_cat_pers['num'] and !$posti_mancanti[$n_t])
if (((int) $numpersone - (int) $max_capacita) <= $num_aggiungi_letti) {
${"aggiungi_costo_letto".$n_t} = "SI";
if ($posti_mancanti[$n_t]) {
$num_aggiungi_letti = ceil((float) $posti_mancanti[$n_t] / (float) $num_app_tipo_richiesti);
if ($max_num_aggiungi_letti < $num_aggiungi_letti) $num_aggiungi_letti = $max_num_aggiungi_letti;
} # fine if ($posti_mancanti[$n_t])
${"num_aggiungi_letti".$n_t} = (int) $num_aggiungi_letti;
$numpersone = (int) $numpersone - (int) $num_aggiungi_letti;
${"numpersone".$n_t} = $numpersone;
if (!empty($numpersone_orig_nt[$n_t])) {
if ($num_aggiungi_letti == 1) $num_app_reali_costo[$n_t][$costo_aggiungi_letti] = ($numpersone_orig_nt[$n_t] - ($numpersone * $dati_r2['napp'][$tipotariffa])) * $num_app_tipo_richiesti_orig[$n_t];
else $diff_aggiungi_letti[$n_t][$costo_aggiungi_letti] = ($numpersone_orig_nt[$n_t] - ($numpersone * $dati_r2['napp'][$tipotariffa])) * $num_app_tipo_richiesti_orig[$n_t];
...
$cat_numpers_orig[$n_t][$cat_pers_letto] = ${"cat$cat_pers_letto"."_numpers".$n_t};
if (!empty($numpersone_orig_nt[$n_t])) {
if ($num_aggiungi_letti == 1) $catpers_cp = $num_app_reali_costo[$n_t][$costo_aggiungi_letti];
else $catpers_cp = (($num_aggiungi_letti - 1) * $dati_r2['napp'][$tipotariffa]) + $diff_aggiungi_letti[$n_t][$costo_aggiungi_letti];
...
if (!empty($cat_persone_nr[$n_t][$num2][fixset($cat_persone_nr[$n_t][$num2]['ord'][$cat_pers_letto])]['molt']) and $catpers_cp) {
...
} # fine if (!empty($cat_persone_nr[$n_t][$num2][fixset($cat_persone_nr[$n_t][$num2]['ord'][$cat_pers_letto])]['molt']) and $catpers_cp)
...
} # fine if (((int) $numpersone - (int) $max_capacita) <= $num_aggiungi_letti)
} # fine if (($numpersone > $max_capacita and ((int) $numpersone - (int) $max_capacita) <= $max_num_aggiungi_letti) or...
} # fine if ($num_cal and $dati_ca[$num_cal]['letto'] == "s" and $dati_ca[$num_cal]['numsett'] != "c" and...
} # fine if (strcmp((string) $costo_aggiungi_letti,""))

if ($posti_mancanti[$n_t] and ${"aggiungi_costo_letto".$n_t} != "SI") {
$verificare = "NO";
echo "$fr_Il_numero_di $fr_persone $fr_supera_la_capienza_massima_della_tipologia_richiesta";
if ($num_tipologie > 1) echo " ($n_t)";
} # fine if ($posti_mancanti[$n_t] and ${"aggiungi_costo_letto".$n_t} != "SI")
} # fine if ($numpersone)


# se vi sono costi (anche da aggiungi_costo_letto) con appartamenti incompatibili
if ($posti_mancanti[$n_t]) $app_richiesti_con_incomp = fixset(${"app_richiesti".$n_t});
...
if (${"aggiungi_costo_letto".$n_t} == "SI" and $dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['appincompatibili']) {
$app_incomp_costi[$n_t] .= ",".$dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['appincompatibili'];
$app_incomp_cal[$n_t] = ",".$dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['appincompatibili'].",";
} # fine if (${"aggiungi_costo_letto".$n_t} == "SI" and $dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['appincompatibili'])
if ($app_incomp_costi[$n_t]) {
$app_incomp_costi[$n_t] .= ",";
$app_richiesti_copia = fixset(${"app_richiesti".$n_t});
if (empty($lista_app_reg[$n_t])) {
$lista_app_reg[$n_t] = "";
for ($num1 = 0 ; $num1 < $numappartamenti ; $num1++) {
$idapp = $dati_app['posizione'][$num1];
...
} # fine if (empty($lista_app_reg[$n_t]))
...
if ($idcostoagg == $costo_aggiungi_letti) {
if (!$app_incomp_cal[$n_t]) $app_incomp_cal[$n_t] = ",$idapp,";
elseif (!strstr($app_incomp_cal[$n_t],",$idapp,")) $app_incomp_cal[$n_t] .= "$idapp,";
} # fine if ($idcostoagg == $costo_aggiungi_letti)
} # fine if ($risul != "SI")
else {
${"app_richiesti".$n_t}[$idapp] = "SI";
$app_trovato = "SI";
...
if ($posti_mancanti[$n_t]) $app_richiesti_copia = $app_richiesti_con_incomp;
else $app_richiesti_copia = fixset(${"app_richiesti".$n_t});
$app_trovato = "NO";
$posto_usato = array();
...
${"app_richiesti".$n_t}[$idapp] = "SI";
$app_trovato = "SI";
if ($posti_mancanti[$n_t] and strstr($app_incomp_cal[$n_t],",$idapp,")) $posti_mancanti['app_incomp_cal'][$n_t][$idapp] = 1;
} # fine else if ($maxoccupanti and $maxoccupanti < $numpersone)
if (!empty($numpersone_nr[$n_t])) {
$num_posto_max = -1;
$numpers_posto_max = 0;
for ($num2 = 0 ; $num2 < $num_app_tipo_richiesti ; $num2++) {
if (strcmp(fixstr($numpersone_nr[$n_t][$num2]),"")) {
if ($num1 == 0) $app_richiesti_nr[$n_t][$num2] = array();
if (!$maxoccupanti or $maxoccupanti >= $numpersone_nr[$n_t][$num2]) {
$app_richiesti_nr[$n_t][$num2][$idapp] = "SI";
if (${"app_richiesti".$n_t}[$idapp] == "NO" and !isset($posto_usato[$num2]) and $numpers_posto_max < $numpersone_nr[$n_t][$num2]) {
$num_posto_max = $num2;
$numpers_posto_max = $numpersone_nr[$n_t][$num2];
} # fine if (${"app_richiesti".$n_t}[$idapp] == "NO" and !isset($posto_usato[$num2]) and...
} # fine if (!$maxoccupanti or $maxoccupanti >= $numpersone_nr[$n_t][$num2])
} # fine if (strcmp(fixstr($numpersone_nr[$n_t][$num2]),""))
} # fine for $num2
if ($num_posto_max >= 0) {
$posto_usato[$num_posto_max] = 1;
#$posti++;
$app_trovato = "SI";
} # fine if ($num_posto_max >= 0)
} # fine if (!empty($numpersone_nr[$n_t]))
} # fine if (empty($app_richiesti_copia) or (isset($app_richiesti_copia[$idapp]) and $app_richiesti_copia[$idapp] == "SI"))
if ($app_regola2_predef) {
...
if ($posti_mancanti[$n_t] and $app_incomp_cal[$n_t]) {
if ($posti_mancanti[$n_t] <= ($num_app_tipo_richiesti * $num_aggiungi_letti)) {
# Elimino gli appartamenti incompatibili per avere più possibilità di liberare gli appartamenti giusti con liberasettimane
$num_elim = 0;
$app_usati = array();
for ($num1 = 0 ; $num1 < $num_app_tipo_richiesti ; $num1++) {
if (($num_elim * $num_aggiungi_letti) < $posti_mancanti[$n_t]) {
$app_richiesti_orig = $app_richiesti_nr[$n_t][$num1];
$app_trovato_nr = 0;
foreach ($app_richiesti_orig as $idapp => $val) {
if (strstr($app_incomp_cal[$n_t],",$idapp,")) $app_richiesti_nr[$n_t][$num1][$idapp] = "";
elseif (!$app_trovato_nr and !isset($app_usati[$idapp])) {
$app_usati[$idapp] = 1;
$app_trovato_nr = 1;
} # fine elseif (!$app_trovato_nr and !isset($app_usati[$idapp])
} # fine foreach ($app_richiesti_orig as $idapp => $val)
if ($app_trovato_nr) $num_elim++;
else $app_richiesti_nr[$n_t][$num1] = $app_richiesti_orig;
} # fine if (($num1 * $num_aggiungi_letti) < $posti_mancanti[$n_t])
} # fine for $num1
} # fine if ($posti_mancanti[$n_t] <= ($num_app_tipo_richiesti * $num_aggiungi_letti))
else $num_elim = $num_app_tipo_richiesti;
if ($posti_mancanti[$n_t] > ($num_elim * $num_aggiungi_letti)) $app_trovato = "NO";
} # fine if ($posti_mancanti[$n_t] and $app_incomp_cal[$n_t])
...
if ($mostra_giorni_pieni == "SI") echo "$fr_parola_La $fr_parola_settimana $fr_dal <span class=\"adate\">$inizioperiodopieno_f</span> $fr_al <span class=\"ddate\">$fineperiodopieno_f</span> $fr_est $fr_piena".".<br>";
...
$id_data_inizio_tab_disp = (floor(( (float) $idinizioperiodo_min + (float) $idfineperiodo_max) / 2) - floor((float) $num_colonne_tab_disp / 2));
...
for ($num1 = 0 ; $num1 < $num_app_tipo_richiesti ; $num1++) {
$id_app_richiesti++;
if (empty($app_richiesti_nr[$n_t][$num1])) $lista_app_richiesti_corr = $lista_app_richiesti;
else {
$lista_app_richiesti_corr = "";
foreach ($app_richiesti_nr[$n_t][$num1] as $key => $val) if ($val == "SI") $lista_app_richiesti_corr .= $key.",";
$lista_app_richiesti_corr = substr($lista_app_richiesti_corr,0,-1);
} # fine else if (empty($app_richiesti_nr[$n_t][$num1]))
$app_richiesti[$id_app_richiesti] = $lista_app_richiesti_corr;
...
elseif (!empty($interrompi_vicine_ogni[$n_t])) {
...
} # fine elseif (!empty($interrompi_vicine_ogni[$n_t]))
...
if ($fatto_libera != "SI" and $prenota_vicine == "SI" and $cerca_app_vicini == "se_poss" and isset($app_richiesti[',vicini,']) and $app_richiesti[',vicini,'] == "SI") {
if (isset($succ_non_vicino) and is_array($succ_non_vicino)) $app_richiesti[',succ_non_vicino,'] = $succ_non_vicino;
...
} # fine if ($fatto_libera != "SI" and $prenota_vicine == "SI" and $cerca_app_vicini == "se_poss" and...
...
$tabelle_lock = "";
$altre_tab_lock = array($tableprenota,$tablecostiprenota,$tablerclientiprenota,$tablenometariffe,$tableperiodi,$tableappartamenti,$tablenazioni,$tableregioni,$tablecitta,$tabledocumentiid,$tablecontratti,$tablepersonalizza,$tableutenti,$tablerelutenti,$tablebeniinventario,$tablerelinventario);
$tabelle_lock = lock_tabelle($tabelle_lock,$altre_tab_lock);
...
$testo_a_video .= " $fr_dal <span class=\"adate\">".$data_inizioperiodo_f[$n_t]."</span> $fr_al <span class=\"ddate\">".$data_fineperiodo_f[$n_t]."</span>";
if ($numpersone) {
...
if (!$dati_cat_pers['num']) $costo_tariffap_settimana = (float) $costo_tariffap_sett * (float) $numpersone;
else $costo_tariffap_settimana = (round(((float) $costo_tariffap_sett * $coeff_cat_persone['p']) / $dati_cat_pers['arrotond']) * $dati_cat_pers['arrotond']) + ((float) $costo_tariffap_sett * $coeff_cat_persone['i']);
$lista_tariffep_sett .= ",".$costo_tariffap_settimana;
} # fine if ($dati_tariffe[$tipotariffa]['moltiplica'] == "p")
$costo_tariffa_settimana_tot = (float) $costo_tariffa_settimana + $costo_tariffap_settimana;
...
if (!$dati_cat_pers['num']) {
for ($num2 = 0 ; $num2 < $dati_r2['napp'][$tipotariffa] ; $num2++) {
if (isset($numpersone_nr[$n_t][$num2]) and strcmp((string) $numpersone_nr[$n_t][$num2],"")) {
$costo_tariffap_settimana_alt = (float) $costo_tariffap_sett * (float) $numpersone_nr[$n_t][$num2];
$costo_tariffa_settimana_tot_alt = $costo_tariffa_settimana + $costo_tariffap_settimana_alt;
if (!isset($costo_tariffa_nr[$n_t][$num2])) $costo_tariffa_nr[$n_t][$num2] = 0;
$costo_tariffa_nr[$n_t][$num2] += $costo_tariffa_settimana_tot_alt;
if (strcmp(fixstr($tariffesettimanali_nr[$n_t][$num2]),"")) $tariffesettimanali_nr[$n_t][$num2] .= ",";
else $tariffesettimanali_nr[$n_t][$num2] = "";
$tariffesettimanali_nr[$n_t][$num2] .= $costo_tariffa_settimana_tot_alt;
if ($dati_tariffe[$tipotariffa]['moltiplica'] == "p") {
if (strcmp(fixstr($tariffesettimanalip_nr[$num2]),"")) $tariffesettimanalip_nr[$num2] .= ",";
$tariffesettimanalip_nr[$num2] .= $costo_tariffap_settimana_alt;
if (strcmp(fixstr($tariffesettimanalipo_nr[$num2]),"")) $tariffesettimanalipo_nr[$num2] .= ",";
else $tariffesettimanalipo_nr[$num2] = "";
$tariffesettimanalipo_nr[$num2] .= $costo_tariffap_sett;
} # fine if ($dati_tariffe[$tipotariffa]['moltiplica'] == "p")
if ($num1 == $idfineperiodo[$n_t]) {
if (!empty($tariffesettimanalip_nr[$num2])) {
$tariffesettimanali_nr[$n_t][$num2] .= ";".$tariffesettimanalip_nr[$num2];
if (!empty($tariffesettimanalipo_nr[$num2])) $tariffesettimanali_nr[$n_t][$num2] .= ";".$tariffesettimanalipo_nr[$num2];
} # fine if (!empty($tariffesettimanalip_nr[$num2]))
...
} # fine if (isset($numpersone_nr[$n_t][$num2]) and strcmp((string) $numpersone_nr[$n_t][$num2],""))
} # fine for $num2
} # fine if (!$dati_cat_pers['num'])
else {
for ($num2 = 0 ; $num2 < $dati_cat_pers['num'] ; $num2++) if (!empty(${"cat$num2"."_numpers$n_t"})) $cat_persone_tariffa_cp[$num2] = ${"cat$num2"."_numpers$n_t"};
for ($num2 = 0 ; $num2 < $dati_r2['napp'][$tipotariffa] ; $num2++) {
$coeff_cat_persone = array();
$coeff_cat_persone['i'] = 0;
$coeff_cat_persone['p'] = 0;
if (isset($numpersone_nr[$n_t][$num2]) and strcmp((string) $numpersone_nr[$n_t][$num2],"")) $numpersone_cp = $numpersone_nr[$n_t][$num2];
else $numpersone_cp = $numpersone;
for ($num3 = 0 ; $num3 < $dati_cat_pers['num'] ; $num3++) {
if (!empty($cat_persone_tariffa_cp[$num3]) and $numpersone_cp) {
...
if ($dati_cat_pers[$num3]['perc'] == "100") $coeff_cat_persone['i'] += (float) $numpersone_corr_cp;
else $coeff_cat_persone['p'] += ((float) $numpersone_corr_cp * ((float) $dati_cat_pers[$num3]['perc'] / 100));
} # fine if (!empty($cat_persone_tariffa_cp[$num3]) and $numpersone_cp)
} # fine for $num3
$costo_tariffap_settimana_alt = (round(((float) $costo_tariffap_sett * $coeff_cat_persone['p']) / $dati_cat_pers['arrotond']) * $dati_cat_pers['arrotond']) + ((float) $costo_tariffap_sett * $coeff_cat_persone['i']);
$costo_tariffa_settimana_tot_alt = $costo_tariffa_settimana + $costo_tariffap_settimana_alt;
if (!isset($costo_tariffa_nr[$n_t][$num2])) $costo_tariffa_nr[$n_t][$num2] = 0;
$costo_tariffa_nr[$n_t][$num2] += $costo_tariffa_settimana_tot_alt;
if (!isset($tariffesettimanali_nr[$n_t][$num2])) $tariffesettimanali_nr[$n_t][$num2] = "";
if (strcmp((string) $tariffesettimanali_nr[$n_t][$num2],"")) $tariffesettimanali_nr[$n_t][$num2] .= ",";
$tariffesettimanali_nr[$n_t][$num2] .= $costo_tariffa_settimana_tot_alt;
if ($dati_tariffe[$tipotariffa]['moltiplica'] == "p") {
if (!isset($tariffesettimanalip_nr[$num2])) $tariffesettimanalip_nr[$num2] = "";
if (strcmp((string) $tariffesettimanalip_nr[$num2],"")) $tariffesettimanalip_nr[$num2] .= ",";
$tariffesettimanalip_nr[$num2] .= $costo_tariffap_settimana_alt;
} # fine if ($dati_tariffe[$tipotariffa]['moltiplica'] == "p")
if ($num1 == $idfineperiodo[$n_t]) {
if (!empty($tariffesettimanalip_nr[$num2])) $tariffesettimanali_nr[$n_t][$num2] .= ";".$tariffesettimanalip_nr[$num2];
for ($num3 = 1 ; $num3 < $num_app_tipo_richiesti_orig[$n_t] ; $num3++) {
$costo_tariffa_nr[$n_t][($num2 + ($dati_r2['napp'][$tipotariffa] * $num3))] = $costo_tariffa_nr[$n_t][$num2];
$tariffesettimanali_nr[$n_t][($num2 + ($dati_r2['napp'][$tipotariffa] * $num3))] = $tariffesettimanali_nr[$n_t][$num2];
} # fine for $num3
...
if (isset($costo_tariffa_nr[$n_t][($num1 -1)]) and strcmp(fixstr($costo_tariffa_nr[$n_t][($num1 - 1)]),"")) {
...
} # fine if (isset($costo_tariffa_nr[$n_t][($num1 -1)]) and strcmp(fixstr($costo_tariffa_nr[$n_t][($num1 - 1)]),""))
...
if (empty($dati_r2['napp'][$tipotariffa])) ${"numcostiagg_transazione".$n_t} = $numcostiagg;
...
$num_costo = $dati_ca['id'][$$idcostoagg];
if (!isset($costi_agg_mostra[$$idcostoagg]) or $costi_agg_mostra[$$idcostoagg] != "SI") $$costoagg = "";
if (($mostra_costi_aggiuntivi != "SI" or $dati_ca[$num_costo]['mostra'] != "s") and $numca <= $numcostiagg_orig and empty($costi_aggiungi_promo[$$idcostoagg])) $$costoagg = "";
...
echo "<label><input type=\"checkbox\" name=\"sett$num1"."costo$numca"."_$n_t\" value=\"SI\">$fr_dal
 <span class=\"adate\">".formatta_data(risul_query($date_sett_costo,0,'datainizio'),$stile_data)."</span> $fr_al
 <span class=\"ddate\">".formatta_data(risul_query($date_sett_costo,0,'datafine'),$stile_data)."</span></label><br>";
...
aggiorna_letti_agg_in_periodi($dati_ca,$num_costo,$num_letti_agg,$idinizioperiodo[$n_t],$idfineperiodo[$n_t],$settimane_costo[$numca],"",$nummoltiplica_ca_aux,$numpersone_corr,$dati_cat_pers,fixset(${"catpers_ca".$numca."_".$n_t}));
$num_letti_agg_nr[0] = $num_letti_agg;
if (!empty($num_app_reali_costo[$n_t][$$idcostoagg])) $num_controlla_limite2 = $num_app_reali_costo[$n_t][$$idcostoagg];
...
if (!empty($diff_aggiungi_letti[$n_t][$$idcostoagg]) and $num1 >= $diff_aggiungi_letti[$n_t][$$idcostoagg]) $nummoltiplica_ca_corr = $nummoltiplica_ca_aux - 1;
...
if ($dati_ca[$num_costo]['molt_max'] == "t" and !empty($num_letti_agg['max'])) {
...
} # fine if ($dati_ca[$num_costo]['molt_max'] == "t" and !empty($num_letti_agg['max']))
...
if (controlla_num_pos(fixset(${"catpers_ca".$numca."_".$n_t})) == "NO" or ${"catpers_ca".$numca."_".$n_t} >= $dati_cat_pers['num']) $costi_aggiuntivi_sbagliati .= ",$num_costo";
else {
if ($posti_mancanti[$n_t]) $nummoltiplica_ca_max = ceil($posti_mancanti[$n_t] / $num_app_tipo_richiesti_orig[$n_t]);
...
} # fine else if (controlla_num_pos(fixset(${"catpers_ca".$numca."_".$n_t})) == "NO" or...
...
if (!empty($num_app_reali_costo[$n_t][${"idcostoagg".$numca}])) $num_controlla_limite2 = $num_app_reali_costo[$n_t][${"idcostoagg".$numca}];
else $num_controlla_limite2 = $num_controlla_limite;
for ($num1 = 0 ; $num1 < $num_controlla_limite2 ; $num1++) {
...
if ($num_app_reali_costo2) $num_app_reali_costo[$n_t][${"idcostoagg".$numca}] = $num_app_reali_costo2;
...
$costi_agg_tot_vett[$num1] = (float) 0;
$costo_escludi_perc_vett[$num1] = (float) 0;
} # fine for $num1
unset($prezzo_costo);
...
if (isset($costo_tariffa_nr[$n_t][0]) and strcmp((string) $costo_tariffa_nr[$n_t][0],"")) {
$costo_tariffa_corr = $costo_tariffa_nr[$n_t][0];
$tariffesettimanali_corr = $tariffesettimanali_nr[$n_t][0];
} # fine if (isset($costo_tariffa_nr[$n_t][0]) and strcmp((string) $costo_tariffa_nr[$n_t][0],""))
else {
$costo_tariffa_corr = $costo_tariffa;
$tariffesettimanali_corr = $tariffesettimanali;
} # fine else if (isset($costo_tariffa_nr[$n_t][0]) and strcmp((string) $costo_tariffa_nr[$n_t][0],""))
$prezzo_costo[$numca][1] = (float) calcola_prezzo_totale_costo($dati_ca,$num_costo,$idinizioperiodo[$n_t],$idfineperiodo[$n_t],$settimane_costo[$numca],$moltiplica_costo[$numca],$costo_tariffa_corr,$tariffesettimanali_corr,($costo_tariffa_corr + $costi_agg_tot),$caparra,$numpersone_corr,$dati_cat_pers,fixset(${"catpers_ca".$numca."_".$n_t}),$costo_escludi_perc,1);
...
if (strcmp(fixstr($costo_tariffa_nr[$n_t][($num1 - 1)]),"")) {
$costo_tariffa_corr = $costo_tariffa_nr[$n_t][($num1 - 1)];
$tariffesettimanali_corr = $tariffesettimanali_nr[$n_t][($num1 - 1)];
} # fine if (strcmp(fixstr($costo_tariffa_nr[$n_t][($num1 - 1)]),""))
else {
$costo_tariffa_corr = $costo_tariffa;
$tariffesettimanali_corr = $tariffesettimanali;
} # fine else if (strcmp(fixstr($costo_tariffa_nr[$n_t][($num1 - 1)]),""))
$prezzo_costo[$numca][$num1] = (float) calcola_prezzo_totale_costo($dati_ca,$num_costo,$idinizioperiodo[$n_t],$idfineperiodo[$n_t],$settimane_costo[$numca],$moltiplica_costo_nr[$numca][($num1 - 1)],$costo_tariffa_corr,$tariffesettimanali_corr,($costo_tariffa_corr + $costi_agg_tot_vett[$num1]),$caparra_reg[$n_t][$num1],$numpersone_corr,$dati_cat_pers,fixset(${"catpers_ca".$numca."_".$n_t}),$costo_escludi_perc_vett[$num1],1);
...
$num_ripeti_costo[$num_costi_agg] = fixset($num_app_reali_costo[$n_t][$idcostoagg]);
...
if (isset($costo_tariffa_nr[$n_t][0]) and strcmp((string) $costo_tariffa_nr[$n_t][0],"")) $costo_tariffa_corr = $costo_tariffa_nr[$n_t][0];
else $costo_tariffa_corr = $costo_tariffa;
$costo_tariffa_tot = $costo_tariffa_corr + $costi_agg_tot;
...
if (!empty($dati_r2['napp'][$tipotariffa]) and $num_ripeti_costo_corr) $num_ripeti_costo_corr = ceil((float) $num_ripeti_costo_corr / (float) $dati_r2['napp'][$tipotariffa]);
...
$cognome2_richiedente = stripslashes($cognome2_richiedente);
...
if ($chiedi_cognome2 == "NO") $cognome2_richiedente = "";
...
if ($chiedi_metodopagamento == "SI" and empty($metodo_pagamento) and count($metodi_pagamento_da_chiedere)) {
$metodopagamento_sb = "SI";
$inviare = "NO";
} # fine if ($chiedi_metodopagamento == "SI" and empty($metodo_pagamento) and...
...
$REMOTE_ADDR = htmlspecialchars($REMOTE_ADDR,ENT_COMPAT);
$REMOTE_ADDR = aggslashdb($REMOTE_ADDR);

$testo_email = "$fre_Riferimento: $riferimento\nIP: $REMOTE_ADDR\n\n";
if ($cognome_richiedente) {
$testo_email .= "$fre_Cognome: $cognome_richiedente";
if ($cognome2_richiedente) $testo_email .= " $cognome2_richiedente";
$testo_email .= "\n";
...
if (isset(${"cond_contr".$num_contr}) and ${"cond_contr".$num_contr} == "SI") $testo_email .= $nome_contr."\n";
} # fine if ($chiedi_campi_form_doc_cond[$num1] != "SI")
} # fine for $num1
...
if (strcmp(fixstr($valuta_pagamento),"")) {
$caparra_valuta = (float) $caparra_totale * (float) $altre_valute[$altre_valute['id'][$valuta_pagamento]]['cambio'];
$caparra_valuta = $caparra_valuta / (float) $altre_valute[$altre_valute['id'][$valuta_pagamento]]['arrotond'];
...
if (!empty($metodo_pagamento)) $testo_email .= "$fre_Metodo_di_pagamento_della_caparra: $metodo_pagamento\n";
...
if ($cognome2_richiedente) $nome_da .= " $cognome2_richiedente";
} # fine if ($cognome_richiedente)
...
if ($utente_trovato == "SI") {
$cognome_richiedente = htmlspecialchars(fixstr($cognome_richiedente),ENT_COMPAT);
$cognome2_richiedente = htmlspecialchars(fixstr($cognome2_richiedente),ENT_COMPAT);
$nome_richiedente = htmlspecialchars(fixstr($nome_richiedente),ENT_COMPAT);
$email_richiedente = htmlspecialchars(fixstr($email_richiedente),ENT_COMPAT);
$email2_richiedente = htmlspecialchars(fixstr($email2_richiedente),ENT_COMPAT);
$email_cert_richiedente = htmlspecialchars(fixstr($email_cert_richiedente),ENT_COMPAT);
$sesso = htmlspecialchars(fixstr($sesso),ENT_COMPAT);
$datanascita = htmlspecialchars(fixstr($datanascita),ENT_COMPAT);
$documento = htmlspecialchars(fixstr($documento),ENT_COMPAT);
$tipodoc = htmlspecialchars(fixstr($tipodoc),ENT_COMPAT);
$nazione = htmlspecialchars(fixstr($nazione),ENT_COMPAT);
$regione = htmlspecialchars(fixstr($regione),ENT_COMPAT);
$citta = htmlspecialchars(fixstr($citta),ENT_COMPAT);
$via = htmlspecialchars(fixstr($via),ENT_COMPAT);
$numcivico = htmlspecialchars(fixstr($numcivico),ENT_COMPAT);
$cap = htmlspecialchars(fixstr($cap),ENT_COMPAT);
$telefono = htmlspecialchars(fixstr($telefono),ENT_COMPAT);
$telefono2 = htmlspecialchars(fixstr($telefono2),ENT_COMPAT);
$telefono3 = htmlspecialchars(fixstr($telefono3),ENT_COMPAT);
$fax = htmlspecialchars(fixstr($fax),ENT_COMPAT);
$codfiscale = htmlspecialchars(fixstr($codfiscale),ENT_COMPAT);
$partitaiva = htmlspecialchars(fixstr($partitaiva),ENT_COMPAT);
$commento = htmlspecialchars(fixstr($commento),ENT_COMPAT);
$oracheckin = htmlspecialchars(fixstr($oracheckin),ENT_COMPAT);
...
$metodo_pagamento = str_replace(">","",fixstr($metodo_pagamento));
...
elseif ($cognome_richiedente) {
$mittente = $cognome_richiedente;
if ($cognome2_richiedente) $mittente .= " $cognome2_richiedente";
} # fine elseif ($cognome_richiedente)
} # fine else if ($email_richiedente)
$mittente = aggslashdb($mittente);
$testo_email = htmlspecialchars($testo_email,ENT_COMPAT);
...
${"campo_pers".$num1} = htmlspecialchars(${"campo_pers".$num1},ENT_COMPAT);
$agg_a_comm = 1;
...
$commento .= htmlspecialchars($campi_form_personalizzati[$num1],ENT_COMPAT).": ".${"campo_pers".$num1};
} # fine if ($agg_a_comm)
...
if ($chiedi_campi_form_doc_cond[$num1] != "SI") {
$num_contr = $campi_form_doc_cond[$num1];
if (isset(${"cond_contr".$num_contr}) and ${"cond_contr".$num_contr} == "SI") {
$nome_contr = htmlspecialchars(fixstr($nome_contratto[$num_contr]),ENT_COMPAT);
if (!strcmp((string) $nome_contr,"")) $nome_contr = "doc$num_contr";
$agg_a_comm = 1;
...
} # fine if (isset(${"cond_contr".$num_contr}) and ${"cond_contr".$num_contr} == "SI")
} # fine if ($chiedi_campi_form_doc_cond[$num1] != "SI")
} # fine for $num1
$campi_pers_clienti = "";
...
$dati_transazione15 .= aggslashdb("$codfiscale<d>$partitaiva<d>$valuta_pagamento<d>$cambio_pagamento<d>$arrotond_pagamento<d>$email2_richiedente<d>$email_cert_richiedente<d>$cognome2_richiedente");
$dati_transazione16 = aggslashdb(substr($dati_transazione16,1));
...
for ($num1 = 1 ; $num1 <= $num_campi_pers ; $num1++) ${"campo_pers".$num1} = htmlspecialchars(fixstr(${"campo_pers".$num1}),ENT_COMPAT);
...
else echo "$fr_dal <span class=\"adate\">".$data_inizioperiodo_f[1]."</span> $fr_al <span class=\"ddate\">".$data_fineperiodo_f[1]."</span> ";
echo "$fr_per ";
...
if ($chiedi_cognome2 != "NO") {
if (isset($cognome2_richiedente_sb)) { $apertura_tag_f = $apertura_tag_font_rosse; $chiusura_tag_f = $chiusura_tag_font_rosse; $asterisco = $fr_asterisco_rosso; }
else { $apertura_tag_f = $apertura_tag_font; $chiusura_tag_f = $chiusura_tag_font; $asterisco = $fr_asterisco_nero; }
echo "<tr><td align=\"right\">$apertura_tag_f $fr_Secondo_cognome: $chiusura_tag_f</td><td>$apertura_tag_font
 <input type=\"text\" name=\"cognome2_richiedente\" size=\"30\" value=\"$cognome2_richiedente\">";
#if ($chiedi_cognome2 == "SI") echo "&nbsp;$asterisco";
echo "$chiusura_tag_font</td></tr>";
} # fine if ($chiedi_cognome2 != "NO")
...
$caparra_valuta = (float) $caparra_totale * (float) $altre_valute[$num1]['cambio'];
$caparra_valuta = $caparra_valuta / (float) $altre_valute[$num1]['arrotond'];
$caparra_valuta = round($caparra_valuta);
$caparra_valuta = $caparra_valuta * (float) $altre_valute[$num1]['arrotond'];
...
${"costo_tariffa_".$num_ripeti} = (float) $costo_tariffa_reg[$n_t][$num1];
...
${"numero_cliente_".$num_ripeti} = "0";
if ($n_t == $num_tipologie and $num1 == ${"num_app_tipo_richiesti".$n_t}) ${"ultima_prenotazione_per_cliente_".$num_ripeti} = "1";
else ${"ultima_prenotazione_per_cliente_".$num_ripeti} = "0";
} # fine for $num1
} # fine for $n_t
...
echo "<input type=\"hidden\" name=\"contr_disp\" value=\"SI\">
<input type=\"hidden\" name=\"cognome_richiedente\" value=\"".htmlspecialchars(fixstr($cognome_richiedente))."\">
<input type=\"hidden\" name=\"cognome2_richiedente\" value=\"".htmlspecialchars(fixstr($cognome2_richiedente))."\">
<input type=\"hidden\" name=\"nome_richiedente\" value=\"".htmlspecialchars(fixstr($nome_richiedente))."\">
<input type=\"hidden\" name=\"email_richiedente\" value=\"".htmlspecialchars(fixstr($email_richiedente))."\">
<input type=\"hidden\" name=\"email2_richiedente\" value=\"".htmlspecialchars(fixstr($email2_richiedente))."\">
<input type=\"hidden\" name=\"email_cert_richiedente\" value=\"".htmlspecialchars(fixstr($email_cert_richiedente))."\">
<input type=\"hidden\" name=\"sesso\" value=\"".htmlspecialchars(fixstr($sesso))."\">
<input type=\"hidden\" name=\"annonascita\" value=\"".htmlspecialchars(fixstr($annonascita))."\">
<input type=\"hidden\" name=\"mesenascita\" value=\"".htmlspecialchars(fixstr($mesenascita))."\">
<input type=\"hidden\" name=\"giornonascita\" value=\"".htmlspecialchars(fixstr($giornonascita))."\">
<input type=\"hidden\" name=\"tipodoc\" value=\"".htmlspecialchars(fixstr($tipodoc))."\">
<input type=\"hidden\" name=\"documento\" value=\"".htmlspecialchars(fixstr($documento))."\">
<input type=\"hidden\" name=\"nazione\" value=\"".htmlspecialchars(fixstr($nazione))."\">
<input type=\"hidden\" name=\"regione\" value=\"".htmlspecialchars(fixstr($regione))."\">
<input type=\"hidden\" name=\"citta\" value=\"".htmlspecialchars(fixstr($citta))."\">
<input type=\"hidden\" name=\"via\" value=\"".htmlspecialchars(fixstr($via))."\">
<input type=\"hidden\" name=\"numcivico\" value=\"".htmlspecialchars(fixstr($numcivico))."\">
<input type=\"hidden\" name=\"cap\" value=\"".htmlspecialchars(fixstr($cap))."\">
<input type=\"hidden\" name=\"telefono\" value=\"".htmlspecialchars(fixstr($telefono))."\">
<input type=\"hidden\" name=\"telefono2\" value=\"".htmlspecialchars(fixstr($telefono2))."\">
<input type=\"hidden\" name=\"telefono3\" value=\"".htmlspecialchars(fixstr($telefono3))."\">
<input type=\"hidden\" name=\"fax\" value=\"".htmlspecialchars(fixstr($fax))."\">
<input type=\"hidden\" name=\"codfiscale\" value=\"".htmlspecialchars(fixstr($codfiscale))."\">
<input type=\"hidden\" name=\"partitaiva\" value=\"".htmlspecialchars(fixstr($partitaiva))."\">
<input type=\"hidden\" name=\"commento\" value=\"".htmlspecialchars(fixstr($commento))."\">
<input type=\"hidden\" name=\"data_oracheckin\" value=\"".htmlspecialchars(fixstr($data_oracheckin))."\">
<input type=\"hidden\" name=\"ora_oracheckin\" value=\"".htmlspecialchars(fixstr($ora_oracheckin))."\">
<input type=\"hidden\" name=\"min_oracheckin\" value=\"".htmlspecialchars(fixstr($min_oracheckin))."\">
<input type=\"hidden\" name=\"valuta_pagamento\" value=\"".htmlspecialchars(fixstr($valuta_pagamento))."\">
<input type=\"hidden\" name=\"metodo_pagamento\" value=\"".htmlspecialchars(fixstr($metodo_pagamento))."\">";
$num_campi_pers = count($campi_form_personalizzati);
for ($num1 = 1 ; $num1 <= $num_campi_pers ; $num1++) {
echo "<input type=\"hidden\" name=\"campo_pers$num1\" value=\"".htmlspecialchars(fixstr(${"campo_pers".$num1}))."\">";
} # fine for $num1
for ($num1 = 1 ; $num1 <= $num_campi_doc_cond ; $num1++) {
$num_contr = $campi_form_doc_cond[$num1];
echo "<input type=\"hidden\" name=\"cond_contr$num_contr\" value=\"".htmlspecialchars(fixstr(${"cond_contr".$num_contr}))."\">";
} # fine for $num1
...
} # fine if (!empty($menu_periodi_vett[1]))
...
if (isset($dati_ca['pmin'][$num1]["tipo_associa_".$tariffa]) and ($dati_ca['pmin'][$num1]["tipo_associa_".$tariffa] == "s" or $dati_ca['pmin'][$num1]["tipo_associa_".$tariffa] == "r")) {
...
} # fine if (isset($dati_ca['pmin'][$num1]["tipo_associa_".$tariffa]) and ($dati_ca['pmin'][$num1]["tipo_associa_".$tariffa] == "s" or...
} # fine for $num1
