...
if (strtoupper(fixstr($chiedi_numero_appartamenti_per_tipologia)) == $SI) $chiedi_num_app_tipologia = "SI";
...
if (strtoupper(fixstr($cerca_appartamenti_vicini)) == $SI) $cerca_app_vicini = "SI";
if (strtoupper(fixstr($cerca_appartamenti_vicini)) == $NO) $cerca_app_vicini = "NO";
if (strtoupper(fixstr($cerca_appartamenti_vicini)) == strtoupper(mex("se possibile",$pag))) $cerca_app_vicini = "se possibile";
if (strtoupper(fixstr($cerca_appartamenti_vicini)) == strtoupper(mex("chiedere",$pag))) $cerca_app_vicini = "chiedere";
...
if (!empty($campi_codici_promo['array_esistente'])) $num_codici_promo = (count($campi_codici_promo) - 1);
...
if (strtoupper(fixstr($mostra_richiesta_via_mail)) == $SI) $mostra_richiesta_via_mail = "SI";
else $mostra_richiesta_via_mail = "NO";
if (strtoupper(fixstr($manda_copia_richiesta_email)) == $SI) $manda_copia_richiesta_email = "SI";
else $manda_copia_richiesta_email = "NO";
$chiedi_cognome = strtoupper(fixstr($chiedi_cognome));
...
$chiedi_cognome2 = strtoupper(fixstr($chiedi_cognome2));
if (!$chiedi_cognome2) $chiedi_cognome2 = $NO;
if ($chiedi_cognome2 != $NO) $chiedi_cognome2 = "opzionale";
else $chiedi_cognome2 = "NO";
$chiedi_nome = strtoupper(fixstr($chiedi_nome));
...
$chiedi_email = strtoupper(fixstr($chiedi_email));
...
$chiedi_email2 = strtoupper(fixstr($chiedi_email2));
...
$chiedi_email_cert = strtoupper(fixstr($chiedi_email_cert));
...
$chiedi_sesso = strtoupper(fixstr($chiedi_sesso));
...
$chiedi_datanascita = strtoupper(fixstr($chiedi_datanascita));
...
$chiedi_documento = strtoupper(fixstr($chiedi_documento));
...
$chiedi_nazione = strtoupper(fixstr($chiedi_nazione));
...
$chiedi_citta = strtoupper(fixstr($chiedi_citta));
...
$chiedi_regione = strtoupper(fixstr($chiedi_regione));
...
$chiedi_telefono2 = strtoupper(fixstr($chiedi_telefono2));
...
$chiedi_telefono3 = strtoupper(fixstr($chiedi_telefono3));
...
$chiedi_partitaiva = strtoupper(fixstr($chiedi_partitaiva));
...
$chiedi_commento = strtoupper(fixstr($chiedi_commento));
...
$chiedi_oracheckin = strtoupper(fixstr($chiedi_oracheckin));
...
$chiedi_valutapagamento = strtoupper(fixstr($chiedi_valutapagamento));
if ($chiedi_valutapagamento == $SI) $chiedi_valutapagamento = "SI";
else $chiedi_valutapagamento = "NO";
$chiedi_metodopagamento = strtoupper(fixstr($chiedi_metodopagamento));
...
global $orig_prenota,$num_metodi_pagamento,$file_css_frame,$file_js_frame,$apri_nuova_finestra_da_frame,$altezza_finestra_da_frame,$larghezza_finestra_da_frame,$tema_modello,$num_codici_promo,$parola_settimane,$id_transazione,$id_utente,$lingua_mex,$utente_mess,$secondo_cognome;
$tablenometariffe_modello = $PHPR_TAB_PRE."ntariffe".$anno_modello;
...
if ($costo_aggiungi_letti != "" and ($dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['letto'] != "s" or $dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['numsett'] == "c" or $dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['mostra'] != "s" or $dati_ca[$dati_ca['id'][$costo_aggiungi_letti]]['combina'] == "s")) $continua = "NO";
...
if (!$secondo_cognome or ($chiedi_cognome2 != "opzionale" and $chiedi_cognome2 != "NO")) $chiedi_cognome2 = "NO";
...
for ($numtariffa = 1 ; $numtariffa <= $numero_tariffe ; $numtariffa++) {
if ($attiva_tariffe_consentite == "n" or (isset($tariffe_consentite_vett[$numtariffa]) and $tariffe_consentite_vett[$numtariffa] == "SI") or strtoupper(fixstr($tariffe_mostra_tr[$numtariffa])) == $SI) {
$tariffa = "tariffa".$numtariffa;
global $$tariffa;
if ($$tariffa == "SI") {
...
if ($$codice_promo and ($$tipo_codice_promo == "+" or $$tipo_codice_promo == "-") and strcmp(fixstr($dati_ca['id'][$$costo_codice_promo]),"") and $$costo_codice_promo != $costo_aggiungi_letti) {
...
if (defined('C_CARTELLA_CREA_MODELLI') and defined('C_CARTELLA_UPLOAD_IMG') and C_CARTELLA_UPLOAD_IMG != "" and @is_dir(C_CARTELLA_CREA_MODELLI."/".C_CARTELLA_UPLOAD_IMG)) $sovrascrivi_upl = 1;
else $sovrascrivi_upl = 0;
if ($file_css_frame == "https://") $file_css_frame = "";
elseif (function_exists('upload_hd_img')) upload_hd_img($file_css_frame,"css",$sovrascrivi_upl,"css");
if ($file_js_frame == "https://") $file_js_frame = "";
if (!preg_match("/^((?!php).)+\.js$/i",$file_js_frame)) $file_js_frame = "";
if ($file_js_frame and function_exists('upload_hd_img')) upload_hd_img($file_js_frame,"js",$sovrascrivi_upl,"js");
...
if (!empty($valori_tema[$num1]['pattern']) and !preg_match("/".$valori_tema[$num1]['pattern']."/i",(string) ${"valore_tema_".$num1})) ${"valore_tema_".$num1} = "";
...
if (${"valore_tema_".$num1} and (!empty($valori_tema[$num1]['img']) or !empty($valori_tema[$num1]['file'])) and function_exists('upload_hd_img')) {
$lingua_mex_orig = $lingua_mex;
$lingua_mex = $lingua_orig;
if (!empty($valori_tema[$num1]['file'])) upload_hd_img(${"valore_tema_".$num1},$num1,$sovrascrivi_upl,$valori_tema[$num1]['file']);
else upload_hd_img(${"valore_tema_".$num1},$num1,$sovrascrivi_upl);
$lingua_mex = $lingua_mex_orig;
} # fine if (${"valore_tema_".$num1} and (!empty($valori_tema[$num1]['img']) or...
} # fine for $num1
...
if (!preg_match("/^#[0-9a-f]{3,3}$/i",(string) ${"colore_tema_".$num1}) and !preg_match("/^#[0-9a-f]{6,6}$/i",(string) ${"colore_tema_".$num1})) ${"colore_tema_".$num1} = $colori_tema[$num1]['default'];
...
\$".mex("var_chiedi_cognome2",$pag)." = \"".mex("$chiedi_cognome2",$pag)."\";
...
\$var_pag[11] = 'cognome2_richiedente';
\$var_pag[12] = 'nome_richiedente';
\$var_pag[13] = 'email_richiedente';
\$var_pag[14] = 'email2_richiedente';
\$var_pag[15] = 'sesso';
\$var_pag[16] = 'giornonascita';
\$var_pag[17] = 'mesenascita';
\$var_pag[18] = 'annonascita';
\$var_pag[19] = 'documento';
\$var_pag[20] = 'nazione';
\$var_pag[21] = 'regione';
\$var_pag[22] = 'citta';
\$var_pag[23] = 'cap';
...
\$var_pag[26] = 'telefono';
\$var_pag[27] = 'telefono2';
\$var_pag[28] = 'telefono3';
\$var_pag[29] = 'fax';
\$var_pag[30] = 'email_cert_richiedente';
\$var_pag[31] = 'codfiscale';
\$var_pag[32] = 'partitaiva';
\$var_pag[33] = 'commento';
\$var_pag[34] = 'data_oracheckin';
\$var_pag[35] = 'ora_oracheckin';
\$var_pag[36] = 'min_oracheckin';
\$var_pag[37] = 'lista_tariffe_p2';
\$var_pag[38] = 'aggiungi_tipologie_p2';
\$var_pag[39] = 'chiedi_num_persone_passo_1_p2';
\$var_pag[40] = 'valuta_pagamento';
\$var_pag[41] = 'rel';
\$var_pag[42] = 'rel_sup';
\$var_pag[43] = 'id_ut_sel';
\$var_pag[44] = 'cmp';
\$var_pag[45] = 'mostra_cod';
\$var_pag[46] = 'pieno';
\$var_pag[47] = 'd';
\$var_pag[48] = 'dati_relutenti';
\$var_pag[49] = 'id';
\$var_pag[50] = 'tipodoc';
\$var_pag[51] = 'metodo_pagamento';
\$n_var_pag = 52;
\$num2 = 1;
...
\$chiedi_cognome2 = \$".mex("var_chiedi_cognome2",$pag).";
if (strtoupper(\$chiedi_cognome2) == \"$NO\") \$chiedi_cognome2 = \"NO\";
...
\$metodi_pagamento_da_chiedere = array();
...
\$chiedi_campi_form_personalizzati = array();
...
\$chiedi_campi_form_doc_condizioni = array();
...
if ($id_utente_liste != 1) includi_file("./includes/funzioni_appartamenti.php",$file);
