...
$file_esist[$fsorg] = 1;
if (substr($fsorg,-1) != "/") {
if ($fsorg != "includes/costanti.php") {
...
} # fine if ($fsorg != "includes/costanti.php")
} # fine if (substr($fsorg,-1) != "/")
...
} # fine if (substr($file_dati_conn[$num1],0,2) ==..
...
$n_tariffesett = "";
for ($num3 = 0 ; $num3 < count($tariffesett); $num3++) $n_tariffesett .= ",".round(((float) $tariffesett[$num3] / (float) $numpers),2);
...
if ($versione_corrente < "3.07") {
$aggiornato = "SI";
for ($num1 = 0 ; $num1 < $num_anni ; $num1++) {
$anno_mostra = risul_query($anni,$num1,'idanni');
$tablenometariffe = $PHPR_TAB_PRE."ntariffe".$anno_mostra;
$num_tariffe = esegui_query("select * from $tablenometariffe where idntariffe = '1' ");
$num_tariffe = risul_query($num_tariffe,0,'nomecostoagg');
$rigaimp = esegui_query("select * from $tablenometariffe where idntariffe = '6' ");
if (numlin_query($rigaimp)) {
for ($num2 = 1 ; $num2 <= $num_tariffe ; $num2++) {
$imp_prezzi_int = risul_query($rigaimp,0,'tariffa'.$num2);
if ($imp_prezzi_int) {
$imp_prezzi_per = explode(">",$imp_prezzi_int);
$num_per_importa = count($imp_prezzi_per);
$imp_prezzi_int = "";
for ($num3 = 0 ; $num3 < $num_per_importa ; $num3++) {
$imp_prezzi = explode(";",$imp_prezzi_per[$num3]);
$imp_prezzi_int .= $imp_prezzi[0].";".fixset($imp_prezzi[1]);
$tipo_imp = fixset($imp_prezzi[2]);
if ($tipo_imp == "p") $imp_prezzi_int .= ";g;0;".fixset($imp_prezzi[3]).";".fixset($imp_prezzi[4]);
else $imp_prezzi_int .= ";$tipo_imp;".fixset($imp_prezzi[3]).";0;".fixset($imp_prezzi[4]);
if (isset($imp_prezzi[5])) $imp_prezzi_int .= ";".$imp_prezzi[5];
$imp_prezzi_int .= ">";
} # fine for $num3
$imp_prezzi_int = substr($imp_prezzi_int,0,-1);
esegui_query("update $tablenometariffe set tariffa$num2 = '".aggslashdb($imp_prezzi_int)."' where idntariffe = '6' ");
} # fine if ($imp_prezzi_int)
} # fine for $num2
} # fine if (numlin_query($rigaimp))
} # fine for $num1
} # fine if ($versione_corrente < "3.07")

if ($versione_corrente < "3.08") {
$aggiornato = "SI";
$tableclienti = $PHPR_TAB_PRE."clienti";
$tableclienti_temp = $PHPR_TAB_PRE."clien";
esegui_query("alter table $tableclienti add column cognome2 varchar(70) ");
esegui_query("alter table $tableclienti rename to $tableclienti_temp ");
...
esegui_query("insert into $tableclienti select idclienti,cognome,cognome2,nome,soprannome,sesso,titolo,lingua,datanascita,cittanascita,regionenascita,nazionenascita,documento,scadenzadoc,tipodoc,cittadoc,regionedoc,nazionedoc,nazionalita,nazione,regione,citta,via,numcivico,cap,telefono,telefono2,telefono3,fax,email,email2,email3,cod_fiscale,partita_iva,commento,max_num_ordine,idclienti_compagni,doc_inviati,datainserimento,hostinserimento,utente_inserimento from $tableclienti_temp ");
esegui_query("drop table $tableclienti_temp ");
for ($num1 = 0 ; $num1 < $num_anni ; $num1++) {
$anno_mostra = risul_query($anni,$num1,'idanni');
$tableprenotacanc = $PHPR_TAB_PRE."prenotacanc".$anno_mostra;
$prenotazioni = esegui_query("select idprenota,commento from $tableprenotacanc");
$num_prenotazioni = numlin_query($prenotazioni);
for ($num2 = 0 ; $num2 < $num_prenotazioni ; $num2++) {
$commento = (string) risul_query($prenotazioni,$num2,'commento');
if (strstr($commento,">")) {
$commento = explode(">",$commento);
if (substr($commento[(count($commento) - 1)],0,11) == "hd_del_res<") {
$data_cancellazione = explode("|",substr($commento[(count($commento) - 1)],11));
if (!empty($data_cancellazione[1])) {
$data_cancellazione = $data_cancellazione[1].":00";
$idprenota = risul_query($prenotazioni,$num2,'idprenota');
esegui_query("update $tableprenotacanc set data_modifica = '$data_cancellazione' where idprenota = '$idprenota' ");
} # fine if (!empty($data_cancellazione[1]))
} # fine if (substr($commento[(count($commento) - 1)],0,11) == "hd_del_res<")
} # fine if (strstr($commento,">"))
} # fine for $num2
} # fine for $num1
$opz_eml = esegui_query("select * from ".$PHPR_TAB_PRE."contratti where tipo = 'opzeml' and testo $LIKE 'html;%' ");
$num_opz = numlin_query($opz_eml);
for ($num1 = 0 ; $num1 < $num_opz ; $num1++) {
$numc = risul_query($opz_eml,$num1,'numero');
$opz_corr = (string) risul_query($opz_eml,$num1,'testo');
esegui_query("update ".$PHPR_TAB_PRE."contratti set testo = '".aggslashdb($opz_corr.";+pt")."' where numero = '$numc' and tipo = 'opzeml' ");
} # fine for $num1
} # fine if ($versione_corrente < "3.08")
...
HOTELDRUID version ".C_PHPR_VERSIONE_TXT.", Copyright (C) 2001-2025 Marco M. F. De Santis<br>
