...
$dati_tariffe['tariffa'.$num1]['perc_importa'][$num2] = fixset($importa_prezzi[4]);
$dati_tariffe['tariffa'.$num1]['arrotond_importa'][$num2] = fixset($importa_prezzi[5]);
if (isset($importa_prezzi[6])) {
$per_imp = explode("-",$importa_prezzi[6]);
$dati_tariffe['tariffa'.$num1]['periodo_importa_i'][$num2] = $per_imp[0];
$dati_tariffe['tariffa'.$num1]['periodo_importa_f'][$num2] = $per_imp[1];
} # fine if (isset($importa_prezzi[6]))
...
$dati_tariffe['tasse_arrotond'] = (float) risul_query($arrotond_tasse,0,'valpersonalizza');
...
$costo_tariffa_corr = (float) 0;
$num_sett = 0;
$numsett = 0;
$tariffesettimanali = explode(";",$tariffesettimanali);
$tariffesettimanali = explode(",",$tariffesettimanali[0]);
$agg_sett_sconto = round((((float) $sconto * -1) / ($iddatafine - $iddataini + 1)),2);
$agg_sett_costi = round(((((float) $sconto * -1) + (float) $prezzo_costi_tot) / ($iddatafine - $iddataini + 1)),2);
for ($num1 = $iddataini ; $num1 <= $iddatafine ; $num1++) {

$costo_tariffa_sett = (float) $tariffesettimanali[$numsett];
...
$costo_base = (float) $costo_tariffa_corr;
if ($ultime_comm_base == "s") $costo_base = $costo_base + ((float) $agg_sett_sconto * $num_sett);
if ($ultime_comm_base == "c") $costo_base = $costo_base + ((float) $agg_sett_costi * $num_sett);
$commissioni_corr = ($costo_base * (float) $ultime_comm_perc) / 100;
...
$commissioni += (float) $commissioni_corr;
...
$costo_tariffa_corr += (float) $tariffesettimanali[$numsett];
$numsett++;
} # fine for $num1
...
$tariffesettimanali = explode(";",$tariffesettimanali);
$tariffesettimanali = explode(",",$tariffesettimanali[0]);
for ($num1 = 0 ; $num1 < $caparra_percent ; $num1++) $caparra += (float) $tariffesettimanali[$num1];
...
$caparra = ($costo_tariffa * (float) $caparra_percent) / 100;
...
$tipo_fisso = $dati_tariffe[$tariffa]['tipo_importa'][$num_ord];
$importa_fisso = (float) $dati_tariffe[$tariffa]['val_importa'][$num_ord];
$importa_percent = (float) $dati_tariffe[$tariffa]['perc_importa'][$num_ord];
$importa_arrotond = (float) fixset($dati_tariffe[$tariffa]['arrotond_importa'][$num_ord]);
$parte_prezzo = $dati_tariffe[$tariffa]['parte_prezzo'][$num_ord];
if ($tipo_fisso == "s" and empty($agg_vett[$tariffa][$num_ord])) {
...
} # fine if ($tipo_fisso == "s" and empty($agg_vett[$tariffa][$num_ord]))
if ($tipo_fisso == "g") $fisso = $importa_fisso;
} # fine if ($num_ord != $num_ord_prec)

$prezzo_a_p = (float) $prezzoperiodop;

if ($tipo_fisso == "s") {
$num_agg[$tariffa][$num_ord]['s']++;
$fisso = $agg_vett[$tariffa][$num_ord][$num_agg[$tariffa][$num_ord]['s']];
if ($num_agg[$tariffa][$num_ord]['s'] == 7) $num_agg[$tariffa][$num_ord]['s'] = 0;
} # fine if ($tipo_fisso == "s")

$prezzo_a = (float) $prezzoperiodo;
...
if ($parte_prezzo == "pn" or $parte_prezzo == "pf") $prezzo_a_agg = 0;
else $prezzo_a_agg = $prezzo_a;
if (($prezzo_a_agg and (strcmp((string) $prezzoperiodo,"") or $parte_prezzo == "fp" or $parte_prezzo == "2")) or ((string) $prezzo_a_agg == "0" and (string) $prezzoperiodo == "0")) esegui_query("update $tableperiodi set $tariffa = '$prezzo_a_agg' where idperiodi = '$num1'");
else esegui_query("update $tableperiodi set $tariffa = NULL where idperiodi = '$num1'");

if ($tipo_fisso == "s") {
$num_agg[$tariffa][$num_ord]['p']++;
$fisso = $agg_vett[$tariffa][$num_ord][$num_agg[$tariffa][$num_ord]['p']];
...
if (($prezzo_a_p and (strcmp((string) $prezzoperiodop,"") or $parte_prezzo == "pf" or $parte_prezzo == "2")) or ((string) $prezzo_a_p == "0" and (string) $prezzoperiodop == "0")) esegui_query("update $tableperiodi set $tariffa"."p = '$prezzo_a_p' where idperiodi = '$num1'");
...
if ($cat_pers_corr[1] > 1) $dati_cat_pers_p[$num1]['n_corr'] = fixset($dati_cat_pers[$cat_pers]['langs'][$lingua_mex]['n_p']);
else $dati_cat_pers_p[$num1]['n_corr'] = fixset($dati_cat_pers[$cat_pers]['langs'][$lingua_mex]['n_s']);
...
if ($inv) $val = (float) formatta_soldi($val) / (float) $cambio;
else $val = (float) formatta_soldi($val) * (float) $cambio;
$val = $val / (float) $arrotond;
$val = round($val);
$val = $val * (float) $arrotond;
