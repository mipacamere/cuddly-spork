...
$d_sconto = (float) risul_query($dati_prenota,0,'sconto');
if (!$d_sconto) $d_sconto = (float) 0;
$d_costo_tot = (float) risul_query($dati_prenota,0,'tariffa_tot');
if (!$d_costo_tot) $d_costo_tot = (float) 0;
...
$d_prezzo_costo_agg[$numca] = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$idinizioperiodo,$idfineperiodo,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$d_costo_tariffa,$d_tariffesettimanali,($d_costo_tariffa + $d_costo_agg_tot - $d_sconto),$d_caparra,$d_num_persone,fixset($dati_cap[$numca]['cat_pers']),0);
...
if (!empty($agg_costo) and (strcmp(fixstr($dati_ca['id'][$agg_costo]),"") or substr($agg_costo,0,1) == "c") and !$azione) {
$azione = 1;
...
} # fine if (!empty($agg_costo) and (strcmp(fixstr($dati_ca['id'][$agg_costo]),"") or...
...
if (empty($moltiplica)) $moltiplica = 0;
...
if ($dati_ca[$numca]['tipo'] != "u" or $dati_ca[$numca]['tipo_val'] != "f") $calcolabile_js[$num_costo] = 0;
...
if ($dati_cap[$numca]['moltiplica'] == "t") {
$n_prezzo_costo_agg[$numca] = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$idinizioperiodo,$idfineperiodo,$dati_cap[$numca]['settimane'],$n_moltiplica_costo[$numca],$d_costo_tariffa,$d_tariffesettimanali,1,$d_caparra,$d_num_persone,$dati_cap[$numca]['cat_pers'],0);
...
$val_costo[$num1] = (float) calcola_prezzo_totale_costo($dati_ca,$numca,$idinizioperiodo,$idfineperiodo,$settimane_costo[$num1],$moltiplica_costo[$num1],$d_costo_tariffa,$d_tariffesettimanali,1,$d_caparra,$d_num_persone,$dati_cat_pers,0);
...
$n_prezzo_costo_agg[$numca] = (float) calcola_prezzo_totale_costo($dati_cap,$numca,$idinizioperiodo,$idfineperiodo,$dati_cap[$numca]['settimane'],$n_moltiplica_costo[$numca],$d_costo_tariffa,$d_tariffesettimanali,$costo_totale_provvisorio,$d_caparra,$d_num_persone,"","");
...
$costo_totale_provvisorio = $d_costo_tariffa + $n_costo_agg_tot - $d_sconto;
$val_costo[$num1] = (float) calcola_prezzo_totale_costo($dati_ca,$numca,$idinizioperiodo,$idfineperiodo,$settimane_costo[$num1],$moltiplica_costo[$num1],$d_costo_tariffa,$d_tariffesettimanali,$costo_totale_provvisorio,$d_caparra,$d_num_persone,"","");
$n_costo_agg_tot = $n_costo_agg_tot + $val_costo[$num1];
$val_tot += $val_costo[$num1];
...
$val_entrata_valuta = (float) $val_entrata * (float) $altre_valute[$num1]['cambio'];
$val_entrata_valuta = $val_entrata_valuta / (float) $altre_valute[$num1]['arrotond'];
...
$metodo_pagamento_sel = htmlspecialchars(fixstr($metodo_pagamento_sel),ENT_COMPAT);
if (strcmp($metodo_pagamento_sel,"") and str_replace(",$metodo_pagamento_sel,",",",",$metodi_pagamento,") == ",$metodi_pagamento,") $metodo_pagamento_sel = "";
...
$val_entrata_valuta = (float) $val_entrata * (float) $altre_valute[$valuta_corr]['cambio'];
$val_entrata_valuta = $val_entrata_valuta / (float) $altre_valute[$valuta_corr]['arrotond'];
$val_entrata_valuta = round($val_entrata_valuta);
$val_entrata_valuta = $val_entrata_valuta * (float) $altre_valute[$valuta_corr]['arrotond'];
...
} # fine if (!empty($calcolabile_js[$num1]))
else {
$vett_js_id_c .= ",''";
$vett_js_nomi_c .= ",''";
$vett_js_molt_c .= ",''";
$vett_js_val_c .= ",''";
} # fine else if (!empty($calcolabile_js[$num1]))
...
$contr_escludi = array();
$escludi_pag = esegui_query("select numero from $tablecontratti where tipo = 'pag_esc' and testo $LIKE '%,pos,%' ");
